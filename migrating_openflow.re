...
その心配はおそらく無用です。なぜなら、大失敗しないためのノウハウを本章ですべて説明するからです。筆者らは、OpenFlow が登場したころから職場ネットワークで実験を始め、それこそ数え切れないほどの失敗を繰り返してきました。とにかく何度も怒られましたが、その経験からうまくやる方法をアドバイスできます。我々は既存のネットワークを穏便に OpenFlow に移行するテクニックをいくつか持っています。ちょっとした OpenFlow コントローラを書くだけで、移行の際に起こりがちなネットワーク障害を簡単に防げるのです。これはいわば、初めて自転車に乗るときのヘルメットのようなものです。
