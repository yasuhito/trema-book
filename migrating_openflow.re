...
話は 2009 年 7 月までさかのぼります。OpenFlow が登場したばかりの当時、私たちはさっそく OpenFlow コントローラを書いて小さな OpenFlow ネットワークを職場に構築してみました。「おお、ちゃんと動くじゃん!」気を良くした私たちは、こともあろうに職場ネットワークとこの OpenFlow ネットワークをいきなりつないでしまいました。まあ大丈夫だろうと楽観的に考えていたのです。

しかし結果的にはすぐにネットワーク障害が起こり、異常に気づいた管理者からお叱りのメールを受け取ることになりました。障害時のセットアップを単純化すると@<img>{buggy_controller_setup} のようになります。

//image[buggy_controller_setup][障害を起こしたときのネットワーク構成を単純化したもの][scale=0.5]

//noindent
職場ネットワーク（レガシーネットワークとします）のスイッチにはホストを 2 台つないでおり、スイッチのポート 3 番を OpenFlow スイッチのポート 1 番と接続しました。この OpenFlow スイッチは、私たちが書いた OpenFlow コントローラ (仮に BuggyController とします) で制御していました。
...
味です。われわれはすぐに OpenFlow ネットワークを切断し、原因の分析にとりか
かりました。
...
時のわれわれでは気づくことができませんでした。というわけで、レガシーネット
