...
//lead{
あとは実践あるのみ! 今までの知識を総動員し、自宅や職場のネットワークを OpenFlow 化してみましょう。
//}

あなたも OpenFlow エンジニアとしてそろそろ独り立ちするときがやってきました。本書の第 I 部では OpenFlow のプロトコルや動作モデルを学び、第 II 部では Trema でさまざまなタイプの OpenFlow コントローラを作ってきました。一人前の OpenFlow エンジニアとしてやっていくために必要な基礎知識はすでにひととおり身についたと言えます。

エンジニアとしてステップアップするために、次にやるべきことは何でしょうか？あとはひたすら実践するだけです。今まで手に入れた知識を実際に道具として使い、いま生活しているネットワーク上で OpenFlow を実際に動かしてみるのが一番です。まずは自宅のネットワークを OpenFlow で置き換えましょう。これがうまく行き物足りなくなったら、こんどは職場で小規模に OpenFlow ネットワークを作りましょう。このように徐々に規模を広げて行くのです。

実際にその環境で暮らしてみて、はじめて見えてくるニーズやアイデア、改善案があります。初めて自転車に乗ったときのことを思い出してください。補助輪をはずしただけで何とむずかしいことかと最初は思いますが、多少は怪我はしつつもあきらめずに練習を繰り返しているうち、誰でも乗りこなせるようになります。このように机上検討もほどほどに、とにかくやってみなければ見えないこともあります。

「でも、もし大失敗したら怒られるかもしれない」。普通の人はそう考えるでしょう。家のネットワークはともかく、もし職場のネットワークを止めて同僚に迷惑をかけてしまったらどうしよう……。管理者や上司に注意されたらどうしよう……。

その心配はおそらく無用です。なぜなら、大失敗しないためのノウハウを本章ですべて説明するからです。筆者らは、OpenFlow が登場したころから職場ネットワークで実験を始め、それこそ数え切れないほどの失敗を繰り返してきました。とにかく何度も怒られましたが、その経験からうまくやる方法をアドバイスできます。われわれは既存のネットワークを穏便に OpenFlow に移行するテクニックをいくつか持っています。ちょっとした OpenFlow コントローラを書くだけで、移行の際に起こりがちなネットワーク障害を簡単に防げるのです。これはいわば、初めて自転車に乗るときのヘルメットのようなものです。

まずは、筆者たちの失敗談を振り返らせてください。

== 私たちの失敗談

話は 2009 年 7 月までさかのぼります。OpenFlow が登場したばかりの当時、私たちはさっそくスイッチングハブ相当の OpenFlow コントローラを書いて小さな OpenFlow ネットワークを職場に構築してみました。「おお、ちゃんと動くじゃん!」気を良くした私たちは、こともあろうに職場ネットワークとこの OpenFlow ネットワークをいきなりつないでしまいました。まあ大丈夫だろうと楽観的に考えていたのです。

しかし結果的にはすぐにネットワーク障害が起こり、異常に気づいたネットワーク管理者からお叱りのメールを受け取ることになりました。障害時のセットアップを単純化すると@<img>{buggy_controller_setup} のようになります。
...
ネットワーク管理者から届いた障害報告メールには次のようにありました「レガシースイッチにつながったホストどうしが通信できなくなった。ネットワークを監視する Watchdog プログラムは Host Flapping 警告を出している。」
 ...
 * もう 1 つは、BuggyController が Packet In してきたスイッチポートに Packet Out していたことです。assert を入れるといった防御的プログラミングや、ユニットテストを徹底できていれば防げるバグでしたが、残念ながら当時の私たちでは気づくことができませんでした。

//noindent
というわけで、大障害を起こして始めて気付いたという最悪のパターンになってしまったわけです。
...
次のパターンは、われわれがやったようにレガシーネットワークと OpenFlow ネッ
...
残ります。というのも、われわれが失敗したように、生のトラフィックをコントロー
