...
Tremaの使用例 - Wakame-VDC

ここでは、Wakame-VDCと言うプロジェクトで、仮想ネットワークを実現するために、Tremaを活用している例をご紹介します。

== Wakame-VDCのプロジェクトの背景

Wakame-VDCは、IaaS型のクラウド構築基盤ソフトウェアです。オープンソースライセンスであるLGPL
v3に基づき公開されています。開発は任意団体であるWakame Software Foundation
(WSF)が行っています。2009年4月に株式会社あくしゅによって、Rubyで書かれた最初のコードがコミットされて以来、2012年9月現在で計22企業・団体が所属し、今もなお積極的に開発が継続されています。

この分野のオープンソースソフトウェアとしては、Eucalyptusや、OpenStack、CloudStackなどがあります。いずれのソフトウェアも、現時点では機能的に似た動作をします。データセンターの設備にインストールをして、資源の貸し出し管理をするのが目的です。サーバやネットワーク、ストレージなどの資源を効率良く切り出し、素早く利用者へ提供します。利用者が、貸し出しや、返却の要求を出す方法は、ブラウザを通じて行うことができます。また、同様の要求は、プログラムから直接呼び出せるWeb
APIからも行うことができます。この方法で、人間の作業を全てプログラムに置き換えることを可能にします。システムの運用を自動化することができるのです。IaaSは、データセンターの資源を効率良く利用できるだけでなく、運用の効率化をも図れるものとして、世界的に注目されています。
...
Wakame-VDCも開発者を募集しております。気軽にパッチを送ってください。Githubなら、Pull
Requestと言うスタイルで、それを支援してくれます。
