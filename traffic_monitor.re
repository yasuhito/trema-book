...
この章でのポイントはずばり、トラフィック情報の収集方法です。スイッチからトラフィック情報を取得するには、Flow Mod でフローエントリを書き込むときに寿命 (タイムアウト値) を設定しておき、一定時間後にフローエントリが削除されるようにしておきます。タイムアウトしてフローエントリが削除されると、これを通知する Flow Removed メッセージがコントローラに上がります。この Flow Removed メッセージにはトラフィック情報が含まれているので、コントローラで取り出してやります (@<img>{flow_removed})。

//image[flow_removed][Flow Removed メッセージから統計情報を取得する][scale=0.5]

トラフィック収集以外の部分は、ラーニングスイッチとまったく変わりません。ソースコードの差分も小さいので、前置きはこのくらいにソースコードをざっと眺めてみましょう。
...
そのほかの変更点は、Flow Mod の部分でトラフィック集計用にタイムアウトのオプション (@<tt>{:hard_timeout => 10}) を追加しています。ラーニングスイッチとの主な差分は、たったこれだけです。
