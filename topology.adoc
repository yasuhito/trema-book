...
image::shownet_topology.jpg[caption="図 14-1: "]
...
[[lldp_overview]]
.LLDP を使ってリンクを発見する
image::lldp_overview.png[caption="図 14-2: "]
...
LLDP によるリンク発見を OpenFlow で実現する方法を見ていきましょう。<<lldp_openflow,図 14-3>> のように、スイッチ 0x1 のポート 5 とスイッチ 0x3 のポート 1 が接続されていたとします。このリンクを発見するために、コントローラは次の動作をします。

[[lldp_openflow]]
.LLDP パケットと OpenFlow の仕組みを使ってリンクを発見する
image::lldp_openflow.png[caption="図 14-3: "]
...
5. コントローラは、受け取った Packet In メッセージを解析することで、リンクの発見を行う。スイッチ 0x2 からは<<lldp_packet_in,図 14-4>> の Packet In メッセージが送られてくる。この中身を見ることで、スイッチ 0x1 のポート 5 と、スイッチ 0x2 のポート 1 の間にリンクを発見できる

[[lldp_packet_in]]
.スイッチ 0x2 から送られてくる Packet In メッセージ
image::lldp_packet_in.png[caption="図 14-5: "]
...
このリンク発見方法をネットワーク中のすべてのスイッチのすべてのポートに順に適用していけば、ネットワーク全体のスイッチの接続関係、つまりトポロジを知ることができます。たとえば<<topology_before,図 14-5>>のような 3 台の OpenFlow スイッチからなるネットワークにおいて、どうやってトポロジを検出できるかを見ていきましょう。各 OpenFlow スイッチがコントローラに接続した直後の状態では、コントローラはスイッチ同士がどのように接続されているかを知りません。

[[topology_before]]
.トポロジ検出前のコントローラ
image::topology_before.png[caption="図 14-5: "]
...
[[topology_after]]
.スイッチ 0x1 から出るリンクを発見
image::topology_after.png[caption="図 14-6: "]
...
ソースコードに含まれる `triangle.conf` はスイッチ 3 台を三角形に接続したトアイアングル型のトポロジです (<<triangle_conf, 図 14-7>>)。

[[triangle_conf]]
.triangle.confのトポロジ
image::triangle_conf.png[caption="図 14-7: "]
...
image::triangle_port_down.png[caption="図 14-8: "]

たとえば<<triangle_port_down,図 14-8>>のようにスイッチ 0x1 のポート 1 番を落としてみましょう。
...
実行すると、<<graphviz_triangle,図 14-9>> のようにトポロジ画像が生成されます。

[[graphviz_triangle]]
.トポロジコントローラで生成した三角形トポロジの画像
image::graphviz_triangle.png[caption="図 14-9: "]

== トポロジコントローラのソースコード

トポロジコントローラは大きく分けて 3 つの部品からなります (<<topology_classes,図 14-10>>)。
...
[[topology_classes]]
.トポロジのクラス構成
image::topology_classes.png[caption="図 14-10: "]
