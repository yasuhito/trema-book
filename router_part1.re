 ...
 * リング型のネットワークを構成する FDDI (Fiber-Distributed Data Interface)。大学の計算機室のワークステーション間をつなげるためによく使われていた
...
//image[forward][ルータはパケットを転送するために、パケットのイーサネット部分だけを書き換える][width=12cm]
...
//image[arp_reply][ルータは ARP リクエストに対し自分の MAC アドレスを応える][width=12cm]

==== 宛先ホストの MAC アドレスを調べる

ルータがパケットを宛先ホストに送るためには、宛先ホストの MAC アドレスを調べる必要があります (@<img>{arp_request})。そこでルータは、宛先であるホスト B の MAC アドレスを調べるための ARP リクエストをホスト B へ送ります。このとき、ルータは、ホスト B の MAC アドレスを知らないので、ARP リクエストの宛先 MAC アドレスにはブロードキャストアドレス (FF:FF:FF:FF:FF:FF) を指定します。

//image[arp_request][宛先ホストの MAC アドレスを問い合わせる][width=12cm]
...
//image[router_network][ルータが複数あるネットワークでの転送][width=12cm]
...
== SimpleRouter のソースコード

シンプルルータ (@<tt>{SimpleRouter}) のソースコードは、いくつかのファイルからなります。紙面の都合上、以下ではメインのソースコード (@<list>{simple-router.rb}) を中心に説明します。残りのソースコードについては、Trema の @<tt>{src/examples/simple_router/} 以下を参照してください。
...
シンプルルータの主な動作は Packet In ハンドラから始まります。ハンドラ @<tt>{packet_in} の中身は、次のようになっています。
...
イーサネットにはルータ以外のホストが他にもつながっている可能性があるので、Packet In メッセージが上がってきたときには、まずそのパケットが自分宛かどうかを判断します (@<tt>{to_me?} メソッド)。もし自分宛でない場合にはパケットを破棄します。
...
受信パケットが ARP リクエストである場合、次の@<tt>{handle_arp_request} メソッドが呼ばれます。ここでは、ARP リプライメッセージを作って Packet Out で ARP リクエストが届いたポートに出力します。
...
受信パケットが ARP リプライである場合、ARP テーブル (@<tt>{@arp_table}) に MAC アドレスを格納します。
...
この @<tt>{forward} メソッドは、次の 5 つの処理を行ないます。
 ...
 3. インタフェースが見つかった場合、ARP テーブルから宛先 MAC アドレスを探す
...
このうち重要なのは 1 と 4 の処理です。1 で次の転送先を決める処理は、@<tt>{resolve_next_hop} メソッドにて行います。このメソッドの詳細については、次章で見ていきます。ここでは 4 の処理を詳しく見ていきましょう。

==== パケットの書き換えと転送 (Flow Mod と Packet Out)

ARP テーブルから宛先の MAC アドレスがわかると、パケットを書き換えて宛先へ出力するとともに、同様のパケットを転送するためのフローエントリを、スイッチに書き込みます。@<img>{forward} で説明したように、ルータによるパケットの転送では MAC アドレスを書き換えます。@<tt>{create_action_from} メソッドはこのためのメソッドで、送信元 MAC アドレスの書き換え、宛先 MAC アドレスの書き換え、該当するポートからの出力という 3 つのアクションを含む次の配列を作ります。Flow Mod と Packet Out メッセージの両方で、このアクションリストを使っています。
...
残念ながらシンプルルータは、ループ防止のためのTTL減算ができません。Tremaが対応するOpenFlowバージョン1.0には、そのためのアクションがないためです。本格的なルータを作りたければ、TTL減算に対応した最新バージョン1.3を使えばよいでしょう。
