...
送信元ホストは、ARP リクエストパケットを出して、ルータの MAC アドレスの問い合わせを行います。そのリクエストパケットに対して、ルータは自身の MAC アドレスを ARP リプライパケットとして応答を行います(@<img>{arp-reply})。

//image[arp-reply][ルータは、問い合わせに対し、自分の MAC アドレスを回答する]

=== 送り先の MAC アドレスを調べる

//image[arp-request][送り先ホストの MAC アドレスを問い合わせる]
