...
@<img>{forward} のホスト A がホスト B にパケットを送る場合を考えます。このときのパケットの中身は次のようになります。
...
このときルータは、受け取ったパケットの送信元 MAC アドレスにルータの MAC アドレス、宛先 MAC アドレスにホスト B の MAC アドレスを上書きし、この書き換えたパケットをホスト B へと転送します。
...
次の転送先へと正しくパケットを送るために、各ルータは、宛先と次の転送先の対応を記録したルーティングテーブルを持っています。例えば、ルータ A のルーティングテーブルは@<table>{rtable_a} のようになります。
...
パケットの宛先 IP アドレスがルータである場合、ルータ自身が応答する必要があります。今回実装したシンプルルータでは、ICMP Echo リクエスト (ping) に応答する機能だけ実装しています。詳細は省きますが、(☆ここ説明よろしくおねがいします☆)。
