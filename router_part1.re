...
実は、イーサネットと同じレベルのパケット転送方式は他にもいくつかあります。

 * 身近なところでは、ADSL や光ファイバーによるインターネット接続で使う PPP (Point to Point Protocol) がそうです。
 * 徐々に利用が減りつつありますが、WAN で使われる ATM (Asynchronous Transfer Mode) やフレームリレーもその一種です。
 * 大昔には、大学の計算機室のワークステーション間をつなげるための FDDI (Fiber-Distributed Data Interface) という技術もありました。

//noindent
このように、イーサネットはネットワークを作るための一つの選択肢にすぎません。
