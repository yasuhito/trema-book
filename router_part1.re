...
OpenFlow の世界では、L2 といえば、Ethernet です。@<chap>{openflow} で説明したマッチングルールには、MAC アドレスや VLAN ID など、Ethernet のヘッダ中に含まれる情報を使っています。この Ethernet は、現在最もメジャーな L2 技術といっても間違いではないでしょう。データセンター内で、各サーバ間をつなぐネットワークから、家庭内ネットワークまで Ethernet は、いろいろなところで使われています。
