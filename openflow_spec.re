...
スイッチはフローとして登録されていない通信を検知すると、コントローラにそのパケットおよび関連情報を通知します。これを Packet In メッセージと呼びます。

//image[packet_in][未知のパケットとその情報が Packet In メッセージとしてコントローラに上がる][width=12cm]

=== フローテーブルの更新とパケットの転送

パケットを転送する場合には、まず Packet Out という OpenFlow メッセージによって Packet In を起こしたパケット自体を正しい宛先に流してやります。これをやらないと Packet In を起こしたパケットのデータはコントローラに残ったままで、宛先には届かないからです。そして、同様の通信を今後はスイッチ側だけで処理するために、Flow Mod メッセージによってフローをスイッチのフローテーブルに書き込みます。
...
//image[buffer_id][Flow Mod のときに Buffer ID を指定することで Packet Out を同時にやる][width=12cm]
...
//image[flow_removed][フローが寿命で削除されると、フローによって転送されたパケットの統計情報が Flow Removed としてコントローラに上がる][width=12cm]
