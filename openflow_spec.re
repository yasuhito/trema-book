...
この場合は、フィールド A と B を書き換えて、ポート A と B へフォワードする、と読めます。このように、複数のフィールドを書き換えたり複数のポートにパケットを出したい場合には、アクションを複数連ねて指定します@<fn>{num_actions}。
...
Forward アクションでは指定したポートからパケットを出力します。出力先にはポート番号を指定しますが、特殊用途のために定義されているいくつかの論理ポートを使うこともできます。
 ...
 * TCP/UDP 宛先ポートの書換
 * VLAN ヘッダの除去
 * VLAN ID の書換 (VLAN ヘッダがなければ、新たに付与する)
 * VLAN プライオリティの書換 (VLAN ヘッダがなければ、新たに付与する)
