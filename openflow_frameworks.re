...
新しく Web サービスを立ち上げるには、今や Ruby on Rails@<fn>{rails} などの Web アプリケーションフレームワークが不可欠です。フレームワークを使わない Web 開発は苦痛です。何十倍ものコーディングが発生し、しかもそのほとんどは車輪の再発明に終わるからです。効率的に Web サイトを作りたいプログラマ向けに、書店の技術書コーナーにはさまざまなフレームワークの本があふれかえっています。こうしたフレームワークの上に Web サービスを構築することは、すでに常識なのです。
