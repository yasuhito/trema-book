...
普通のスイッチだけでトラフィック分離を考える際に問題になるのは、ARPリクエストなどのブロードキャストパケットです。VMは、通信をはじめる前に、ARP リクエストを使って宛先のMACアドレスを問い合わせます。ARPリクエストを出すVMは、どのVMから応答をもらえるか知らないため、ネットワーク全体に届くよう、ARPリクエストの宛先にブロードキャストアドレス(FF:FF:FF:FF:FF:FF)を指定します。たとえば、@<img>{arp_entangle}のVM Aが送ったARPリクエストは、ユーザに関係なく全てのVMに届いてしまいます。このとき、同じIPアドレスを持つVM BとDの両方がARPリプライを返すので、VM Aはどちらが正しいMACアドレスかわからず混乱します。このように、普通のスイッチでのトラフィック分離では、ブロードキャストをうまく扱うことができません。
