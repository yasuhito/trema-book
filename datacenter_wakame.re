...
IaaS で使うネットワークでは、ユーザごとのトラフィックを分離する必要があります。Wakame-VDCはどのようにして、OpenFlowに対応していない普通のスイッチでトラフィック分離を実現しているのでしょうか？
...
しかし、普通のスイッチのトラフィック分離では、ARPリクエストなどのブロードキャストの取り扱いが問題になります。ARPリクエストを出すVMは、どのVMから応答をもらえるか知らないため、ネットワーク全体に届くよう、ARPリクエストの宛先にブロードキャストアドレス(FF:FF:FF:FF:FF:FF)を指定します。たとえば、@<img>{arp_entangle}のVM Aが送ったARPリクエストは、ユーザに関係なく全てのVMに届いてしまいます。このとき、同じIPアドレスを持つVM BとDの両方がARPリプライを返すので、VM Aはどちらが正しいMACアドレスかわからず混乱します。このように、普通のスイッチでのトラフィック分離では、ブロードキャストをうまく扱うことができません。
...
各物理ホストに常駐するHVAは、同一ホスト上に配置されているエッジスイッチを、OpenFlowプロトコルを使って制御します。つまり、HVAはOpenFlowコントローラに相当し、ここがTremaを使っている部分です。

Wakame-VDCのアーキテクチャがユニークな点は、各物理ホスト上に分散配置したHVAそれぞれが、担当するエッジスイッチを制御する点です。普通のTremaアプリケーションでは、1つのコントローラでたくさんのOpenFlowスイッチを集中制御します。Wakame-VDCも集中制御であることには変わりませんが、dcmgr の指示の下動作するHVAを分散配置し、エッジスイッチの制御をそれぞれのHVAに任せている点がユニークです。
