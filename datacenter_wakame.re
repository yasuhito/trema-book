...
@<img>{translate} のように、エッジスイッチは、ホストをまたいだ ARP リクエストを、次の様に処理します。VM A が出す ARP リクエストの宛先はブロードキャストアドレスなので、関係ないホストまでパケットが届いてしまいます。そのため、エッジスイッチは、ARP パケットの宛先をホスト B に書き換えます。宛先を書き換えた ARP パケットは、物理スイッチを通過して、ホスト B に届きます。ホスト B 内のエッジスイッチは、届いたパケットの宛先を元に戻してから VM C へと ARP パケットを届けます。

//image[translate][ARP リクエストは、宛先を書き換えて、別のホストへと届けられる]

このように Wakame-VDC では、エッジスイッチの制御だけで、仮想ネットワークを実現しています。普通のスイッチは、宛先 MAC アドレスを見て転送するという一般的な処理をしているだけです。そのため、この部分を新たに OpenFlow スイッチに置き換える必要はなく、既存のネットワークをそのまま使えます。また、 ARP リクエストは最終的に宛先を書き戻して VM に届くので、VM は何も気にすることなく ARP に応答できます。非常にシンプルな仕組みで、既存インフラ上に仮想ネットワークを実現しているのが Wakame-VDC の特徴です。
...
一方、導入の際には、性能面について考慮する必要があります。Wakame-VDC では、エッジスイッチは、ユーザの VM と同じホストマシン上で動作します。CPU リソースを共有しているため、大量のパケットを処理しなければいけない状況では、ユーザ VM に影響を与える可能性があります。しかし、最近では高性能な CPU を安価に使えるので、特殊な使い方をしなければ、大抵のケースでは問題にならないようです。
