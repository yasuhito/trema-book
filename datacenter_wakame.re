...
従来、IaaSはアマゾンやグーグルなど大規模クラウド事業者が提供するサービスを借りるのが一般的でした。オンデマンドで仮想ネットワークを作り、使い終われば削除する仕組みをはじめ、他のユーザに影響を与えずに構成を変更する仕組みは各社独自のノウハウで、一般には公開されていなかったのです。このため、すでにあるサーバリソースを活かして専用のIaaSを作りたいという、いわゆるプライベートクラウドの構築は難しいものがありました。
