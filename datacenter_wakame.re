...
しかし、OpenFlowによってネットワークのAPIが整備されたことで、オープンソースのIaaSが増えつつあります。たとえば、オープンソースなIaaSの代名詞ともなったOpenStackは、ネットワークの仮想化にOpenFlowを使う仕組みを提供しており、@<chap>{sliceable_switch}で紹介したスライス機能つきスイッチと連携できます@<fn>{quantum}。今回紹介するWakame-VDCもTremaを使ったオープンソースなIaaSの一種で、OpenStackと比較して非常に軽量にIaaSを実現した、今注目のIaaSプラットフォームです。
...
ここでは詳しく説明しませんが、Wakame-VDC では ARP の偽装への対策も行なっています。シンプルな仕組みで仮想ネットワークを実現しているため、宛先や送信元を偽装することで、別のユーザの VM へとパケットを送ることができてしまいます。このことを避けるために、エッジの OpenFlow スイッチでパケットを受信した際に、宛先や送信元のチェックを行い、偽装したパケットを破棄するようにしています。
