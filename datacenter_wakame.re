...
エッジで制御を行う Wakame-VDC はOpenFlowに未対応のスイッチで構成されたネットワークに持ってきてもそのまま動く、という利点があります。スライス機能スイッチを使う場合は、ネットワーク内の各スイッチをすべて OpenFlow 対応にする必要がありました。既存のネットワークを変えずに、導入できる点が Wakame-VDC の大きな特徴です。

Wakame-VDC を導入する際には、性能面について考慮を行う必要があります。Wakame-VDC では、エッジスイッチは、ユーザの VM と同じホストマシン上で動作します。CPU リソースを共有しているため、大量のパケットを処理しなければいけない状況では、ユーザ VM に影響を与える可能性があります。しかし、最近では高性能な CPU を安価に使えるので、特殊な使い方をしなければ、大抵のケースでは問題にならないようです。
