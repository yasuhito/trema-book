...
ここまでは、仮想ネットワーク機能について説明しましたが、Wakame-VDC はそれ以外にもストレージの仮想化や Web インタフェース等 IaaS プラットフォームに必要な機能を全て備えています。全体の構成は @<img>{wakame_overview} のようになります。

//image[wakame_overview][Wakame-VDCの全体アーキテクチャ]

データセンタ全体のリソース管理は、Data Center Manager (dcmgr) 行います。Web インタフェースから指示を受けると、実際に仮想マシンの準備や、仮想ネットワークの設定などの作業指示を各エージェントへと送ります。この作業指示の送信には、メッセージキュープロトコルである AMQP を用いています。

エージェントは、それぞれの物理リソースを管理します。各エージェントは、AMQP を用いて作業指示を受け取り、実際の物理リソースに関する処理を実行します。このうち Hyper Visor Agent (HVA) と呼ばれるエージェントが、仮想マシンの起動、仮想ネットワークをセットアップを担当します。

各物理ホストに常駐する HVA は、同一ホスト上に配置されているエッジスイッチを、OpenFlow プロトコルを使って制御します。つまり、HVA は OpenFlow コントローラに相当します。Wakame-VDC は、分散する OpenFlow コントローラへ指示を出して、各コントローラが担当するエッジスイッチを設定するという動作を行います。
