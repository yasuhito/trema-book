...
一台のコンピュータ上で、リソースの管理や基本サービスの提供にOSが必要になるように、データセンターにも同じ機能を提供するソフトウェア基盤が必要です。IaaSはその一つで、ユーザやアプリケーションに対して仮想的なリソースをAPIを通じて提供します。たとえば「192.168.0.1/24のネットワークでつながったサーバが16台ほしい」というリクエストを受けると、IaaSはVMを16台ほど物理マシン上に起動し、専用の仮想ネットワークを作ってこのVM同士をつなぎます。このように、データセンターのリソースを仮想化して使いやすく提供するのがIaaSです。

従来、IaaSはアマゾンやグーグルなど大規模クラウド事業者が提供するサービスを使うのが一般的でした。オンデマンドで仮想ネットワークを作り、使い終われば削除する機能や、他のユーザに影響を与えずにこうした構成変更を行う仕組みは各社の独自ノウハウで、その技術的な詳細は未公開だったのです。このため、すでにあるサーバリソースを活かして専用のIaaSを作りたいという、いわゆるプライベートクラウドの構築はハードルが高いものでした。
...
ホストをまたいだARPリクエストは次の様に処理します(@<img>{translate})。VM Aが出すARPリクエストの宛先はブロードキャストアドレスなので、関係ないホストまでパケットが届いてしまいます。そのため、エッジスイッチは、ARPパケットの宛先をホストBに書き換えます。宛先を書き換えたARPパケットは、物理スイッチを通過して、ホストBに届きます。ホストB内のエッジスイッチは、届いたパケットの宛先を元に戻してからVM CへとARPパケットを届けます。

//image[translate][エッジスイッチでARPリクエストの宛先を書き換えて、別のホストへと届ける]

このようにWakame-VDCでは、エッジスイッチの制御だけで仮想ネットワークを実現しています。実ネットワークのスイッチは、宛先MACアドレスを見て転送するという一般的な処理をすればよいだけなので、OpenFlowスイッチに置き換える必要はありません。また、ARPリクエストは最終的に宛先を書き戻してVMに届くので、VMは何も気にすることなくARPに応答できます。このように、既存インフラやVMに影響を与えることなく仮想ネットワークを実現できるのがWakame-VDCの特徴です。
...
エッジで制御を行う Wakame-VDC はOpenFlowに未対応のスイッチで構成されたネットワークに持ってきてもそのまま動く、という利点があります。これに対して、スライス機能スイッチを使う場合は、ネットワーク内の各スイッチをすべて OpenFlow 対応にする必要がありました。

一方、導入の際には、性能面について考慮する必要があります。Wakame-VDC では、エッジスイッチは、ユーザの VM と同じホストマシン上で動作します。CPU リソースを共有しているため、大量のパケットを処理しなければいけない状況では、ユーザ VM に影響を与える可能性があります。

== Wakame-VDCの全体構成

ここまでは、仮想ネットワーク機能について説明しましたが、Wakame-VDC はそれ以外にもストレージの仮想化や Web インタフェースといった IaaS プラットフォームに必要な機能を全て備えています。全体の構成は @<img>{wakame_overview} のようになります。

//image[wakame_overview][Wakame-VDCの全体アーキテクチャ]

データセンタ全体のリソース管理は、Data Center Manager (dcmgr) が一括して行います。Web インタフェースから指示を受けると、実際に仮想マシンの準備や、仮想ネットワークの設定などの作業指示を各エージェントへと送ります。この作業指示の送信には、メッセージキュープロトコルである AMQP を用いています。
