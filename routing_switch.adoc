...
最短パスの計算でよく使われるのがダイクストラ法というアルゴリズムです。これは、出発点から目的地までの最短パスを求める汎用アルゴリズムの 1 つで、カーナビの経路検索や鉄道の乗換案内などにも使われています。
...
[[dijkstra]]
image::dijkstra.png[caption="図15-4",title="最短パスをダイクストラ法で計算する"]
...
image::routing_switch_update_shortest_path.png[caption="図15-6",title="最短パスの作り直し"]
...
image::path_manager_internals.png[caption="図15-6",title="PathManager の仕組み"]
