...
すると、ルーティングスイッチを起動したターミナルには host4 → host1 と host1 → host4 の 2 つの最短路パスを発見した、というメッセージが表示されているはずです。
...
実際にどのようなフローエントリが設定されたか見てみましょう。フローエントリの確認は、`trema dump_flows [スイッチの Datapath ID]` コマンドです。まずは host1 から host4 への最短パスである switch1, switch5, switch6 のフローテーブルをそれぞれ見てみましょう。
