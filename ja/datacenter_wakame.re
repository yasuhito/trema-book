...
1台のコンピュータ上で、リソースの管理や基本サービスの提供にOSが必要になるように、データセンターにも同じ機能を提供するソフトウェア基盤が必要です。IaaSはその1つで、ユーザやアプリケーションに対しAPIを通じて、仮想的なリソースを提供します。たとえば「192.168.0.1/24のネットワークでつながったサーバが16台ほしい」というリクエストを受けると、IaaSはVMを16台ほど物理マシン上に起動し、専用の仮想ネットワークを作ってこのVM同士をつなぎます。このように、IaaS は、データセンターのリソースを仮想化し、また使いやすくするための API を提供します。
...
Tremaを利用した本格的なIaaSプラットフォームが、Wakame-VDCです。Tremaと同じくGitHub上でオープンに開発しており、ライセンスはLGPLバージョン3のフリーソフトウェアです。開発は22企業・団体が所属する任意団体、Wakame Software Foundation(WSF)が行っています。
...
ネットワークの仮想化とは、ユーザ間のトラフィックを分離することです(@<chap>{sliceable_switch})。トラフィックの分離により、同じユーザが使うVM同士は通信を許可し、異なるユーザが使うVM間の通信を遮断します。Wakame-VDCは、これらの制御にOpenFlowを使います。
...
//image[edge_network_virtualization][スライス機能付きスイッチ(@<chap>{sliceable_switch})とWakame-VDCのネットワーク仮想化方式の違い][width=12cm]
...
このようにWakame-VDCでは、エッジスイッチの制御だけで仮想ネットワークを実現しています。ホスト間を結ぶスイッチは、宛先MACアドレスを見て転送するという一般的な処理をすればよいだけなので、OpenFlowスイッチである必要はありません。また、ARPリクエストは最終的に宛先を書き戻してVMに届くので、VMは何も気にすることなくARPに応答できます。このように、既存のネットワークやVMに影響を与えることなく仮想ネットワークを実現できるのがWakame-VDCの特長です。
