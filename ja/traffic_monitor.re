...
このオプションを設定すると、書き込まれてからの秒数がこの値に達したフローエントリを強制的に消します。つまり、この例ではFlow Modからきっかり10秒後にFlow Removedメッセージがコントローラまで上がることになります。トラフィックモニタのようにトラフィック情報を定期的に取得したい場合には、@<tt>{:hard_timeout}を使うと良いでしょう。

タイムアウトにはこの他にもアイドルタイムアウト(@<tt>{:idle_timeout})というオプションもあります(@<chap>{openflow_spec})。これは、フローエントリが参照されない時間(アイドル時間)がこの寿命に達すると消す、というものです。このオプションは主に、一定期間使われていないフローエントリを消すことでフローテーブルの容量を節約したい時に使います。
