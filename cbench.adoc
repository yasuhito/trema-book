...
OpenFlow 1.0 において指定できるマッチングフィールドは 12 種類です。`Match.new` のオプションには、以下の12種類の条件を指定できます(<<match_field_1.0,表5-2>>)。
...
[NOTE]
====
<<openflow_spec,2章>>で説明したように、OpenFlow 1.3 のマッチフィールドは 40 種類あります。しかし OpenFlow 1.3 での Match オブジェクトのオプションの指定方法は、OpenFlow 1.0 の場合と同じです。詳しくは OpenFlow 1.3 を扱う <<learning_switch13,8章>>や<<router13,14章>>を参照してください。
====
...
[NOTE]
====
<<openflow_spec,2章>>で説明したように、OpenFlow 1.3 において、フローエントリにインストラクションを指定しました。OpenFlow 1.0 では、マルチプルテーブルに対応していないのでインストラクションがありません。そのためインストラクションの代わりに、アクションを直接フローエントリに指定します。インストラクションの使い方は、<<learning_switch13,8章>>にて詳しく説明します。
====
