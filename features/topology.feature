Feature: Detect network topology
  ...
  As a Trema user
  I want to run topology_controller.rb

  Background:
    Given I cd to "../.."

  Scenario: Detect switch to switch links
    When I run `trema run topology_controller.rb -c triangle.conf` interactively
     And I run `sleep 4`
  ...
  Scenario: Kill a switch then the topology updated
    Given I run `trema run topology_controller.rb -c triangle.conf` interactively
     And I run `sleep 2`
