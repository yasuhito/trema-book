...
MAC アドレスをスライスに登録する方法では、コントローラは起動直後に、登録した MAC アドレスを持つホストがどこにいるのかを知りません。そのため、まず @<tt>{host1} から @<tt>{host2} へとパケットを送ります。これで @<tt>{host1} の位置をコントローラが学習しました。その後、@<tt>{host2} から @<tt>{host1} へパケットを送り、@<tt>{host1} の受信カウンタを見てみましょう。@<tt>{host2} からのパケットが受信できていることが確認できます。
