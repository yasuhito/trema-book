...
OpenFlow の魅力は、ネットワークをソフトウェアで自在にコントロールできる点です。本章では、他のソフトと連動させることが可能なコントローラの例として、スライス機能つきスイッチを紹介します。スライス機能つきスイッチには、Web サービスを用いてスライスを設定するための API が用意されています。この API を使えば、スライス機能つきスイッチは他のソフトウェアと連動して、動作できます。

スライス機能つきスイッチは、前章で紹介したルーティングスイッチを発展させたコントローラアプリケーションです。このコントローラを使うことで，OpenFlow ネットワーク全体をスライスに分割し，複数の L2 ネットワークを実現できます。ちょうど，L2 スイッチを複数の VLAN に分けて使うイメージです。
