...
クラウドで仮想化されるリソースには主にネットワークとサーバがありますが、このうちネットワークの仮想化は OpenFlow の得意分野です。ネットワークを OpenFlow で仮想化してやることで、ユーザーごとに専用のネットワークをいつでもオンデマンドで提供できるようになります。本章で紹介する「スライス機能つきスイッチ」は、そのような仮想ネットワーク機能を提供するソフトウェア部品の一つです。

== スライスとは何か？

スライスとはネットワークをいくつかの論理的なネットワークに分割することで、ひとつの物理ネットワークをたくさんのユーザで使えるようにするものです。たとえば IaaS のようなたくさんのユーザを限られた台数の物理サーバに集約するシステムでは、物理サーバを仮想マシンで、またネットワークをスライスで分割することでそれぞれのユーザに仮想的な専用環境 （仮想マシン＋仮想ネットワーク）を提供します。
...
ただし、VLAN にはプロトコル上 4096 個までのスライスしか作れないという制約があります。このため、オフィスなどの中小規模ネットワークではともかく、IaaS のようにユーザ数がゆうに数万のオーダーになる場合には使えません。
...
スライス機能つきスイッチが、どのようにネットワークをスライスするのか見てみましょう。@<img>{sliceable_switch} は、3 つの OpenFlow スイッチから構成されるネットワークを、2 つのスライスに分割している例です。同一のスライスに接続されたホスト同士はパケットをやりとりできますが，異なるスライスに接続されたホスト同士ではできません。このようにうまくスライスを設定することで，アプリケーションやグループ別など用途に応じて独立したネットワークを作れます。
