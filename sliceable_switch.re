...
OpenFlow の魅力は、ネットワークをソフトウェアで自在にコントロールできる点です。本書ではこれまで Trema を使い様々なタイプの OpenFlow コントローラを作る方法について紹介してきました。しかし、世の中にはさまざまな機能を持ったソフトウェアが存在しています。これらの機能を利用することが出来れば、様々な機能を容易に実現することができそうです。本章では、他のソフトと連動させることが可能なコントローラの例として、スライス機能つきスイッチを紹介します。
