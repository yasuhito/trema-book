...
スライス機能つきスイッチも、@<chap>{routing_switch} で取り上げたルーティングスイッチと同様に @<tt>{https://github.com/trema/apps/} にてソースコードが公開されています。ソースコードをまだ取得していない場合は、前章を参考に @<tt>{git} で取得しておいてください。ここでは、以下のようなディレクトリ構成になっていることが前提で、説明します。
...
次に REST API を使ってスライスの設定を行ってみます。スライスとポートを対応付けるためには、OpenFlow スイッチ内部で管理されているポート番号 (Port no) が必要です。以下のコマンドで取得してみましょう。
...
今回は、@<tt>{eth0} から @<tt>{eth3} までのポートの番号は、OpenFlow スイッチ内部ではそれぞれ 1 から 4 と認識されているのが分かります。お手持ちの環境での結果が異なる場合、以降の手順は、適宜読み替えを行って、試してみてください。
