...
クラウドで仮想化されるリソースには主にネットワークとサーバがありますが、このうちネットワークの仮想化は OpenFlow の得意分野です。ネットワークを OpenFlow で仮想化してやることで、ユーザーごとに専用のネットワークをいつでもオンデマンドで提供できるようになります。本章で紹介する「@<em>{スライス機能付きスイッチ}」は、そのような仮想ネットワーク機能を提供するソフトウェア部品の一つです。
...
一方 OpenFlow によるスライスではこの制約はありません。フローエントリによって同じスライス内にあるホスト同志のみが通信できるようにすれば、既存の VLAN の仕組みを使わなくてもフローエントリだけでスライスを実現できるからです。つまり OpenFlow を使えば、「スライス数に制限のない VLAN」を作れます。
...
スライス機能付きスイッチは次の 4 つのアプリケーションが連携して動作します。トポロジー関連のアプリ (@<tt>{topology}、@<tt>{topology_discovery}) を使うところはルーティングスイッチと同じです。
