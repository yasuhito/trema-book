...
//lead{
概念がわかったら少し細かい部分に進みましょう。「実用重視」で OpenFlow 仕様のポイントとよく使う用語を見ていきます。
//}
...
OpenFlow では VLAN ヘッダのついたパケットを扱えます。VLAN とは L2 ネットワーク (ブロードキャストが届く範囲) を、複数のネットワークに分割して使用するための仕組みです。またはこの分割されたネットワーク自体のことを VLAN と呼ぶ場合もあります。どの VLAN に所属しているかを区別するためにパケットに付与されるのが、VLAN ヘッダです。Modify-Field アクションは VLAN ヘッダの操作に必要なアクションを 3 種類用意しています。

//image[strip_vlan][VLAN ヘッダを書き換えるアクションの使い道][scale=0.6]

: VLAN ヘッダの除去
  VLAN を流れる VLAlN ヘッダ付きパケットから VLAN ヘッダを除去し、普通のパケットに戻すアクションです。
