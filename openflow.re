...
OpenFlow では VLAN ヘッダのついたパケットを扱えます。VLAN とはルーターを含まずスイッチだけで構成されるネットワーク (ブロードキャストが届く範囲のネットワーク) を、複数のネットワークに分割して使用するための仕組みです (この分割されたネットワーク自体を VLAN と呼ぶ場合もあります)。どの VLAN に所属しているかを区別するためにパケットに付与されるのが、VLAN ヘッダです。Modify-Field アクションは VLAN ヘッダの操作に必要なアクションを 3 種類用意しています。
