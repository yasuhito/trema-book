...
どちらのタイムアウトもフローを打ち込むときに値を 0 にすると、そのフローは明示的に消さない限りフローテーブルに残ります。

寿命に逹したフローはフローテーブルから消され、消されたフローの情報と、フローにしたがって処理されたパケットの統計情報がコントローラに通知されます。これを Flow Removed メッセージと呼びます。これを利用してコントローラはネットワーク全体のトラフィックを集計できます。
...
マッチングルールとは、OpenFlow スイッチがパケットを受け取ったときにアクションを起こすかどうかを決める条件です。たとえば「パケットの宛先が http サーバーだったら」とか「パケットの送信元がローカルネットワークだったら」などという条件に適合したパケットにだけ、スイッチがアクションを起こすというわけです。
...
というアクションのリストを指定します。ここで、アクションは指定された順番に実行されることに注意してください。アクションの順番を変えてしまうと、違う結果が起こります。たとえば「おにぎりを食べてから、おにぎりを作る」と最後におにぎりが残ってしまいます。同様に先ほどの例を逆にしてしまうと、まず先にパケットがフォワードされます。その後 Modify-Field が実行されても、書き換えられた後、そのパケットは破棄されるだけです。
...
OpenFlow はネットワーク仮想化の手段の一つである VLAN のパケットを扱えます。VLAN (仮想 LAN) は一枚のネットワーク (ブロードキャストが届く範囲) を薄くスライスして何枚かの仮想的な層に分離するための仕組みです。OpenFlow では仮想ネットワークを作る手段としてこの VLAN の仕組みをそのまま使えます。あるいは VLAN を使わずに別のネットワーク仮想化方式を使う場合でも、既存のネットワークから流れてくるかもしれない VLAN パケットをいったん素のパケットに戻す必要があります。いずれの場合でも OpenFlow スイッチは VLAN パケットを書き換える必要があります。Modify-Field アクションはこうした処理に必要なアクションを 3 種類用意しています。

//image[strip_vlan][VLAN 情報を書き換えるアクションの使い道][scale=0.3]
