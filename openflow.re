...
この中でも FLOOD や NORMAL は OpenFlow スイッチ機能とスイッチ本来の機能を組み合わせて使用するための論理ポートです。
...
OpenFlow では VLAN ヘッダのついたパケットを扱えます。VLAN とはルーターを含まずスイッチだけで構成されるネットワーク (ブロードキャストが届く範囲のネットワーク) を、複数のネットワークに分割して使用するための仕組みです。またはこの分割されたネットワーク自体のことを VLAN と呼ぶ場合もあります。どの VLAN に所属しているかを区別するためにパケットに付与されるのが、VLAN ヘッダです。Modify-Field アクションは VLAN ヘッダの操作に必要なアクションを 3 種類用意しています。
