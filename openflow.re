...
//lead{
OpenFlow の仕様は決して大きなものではありませんが、それでも多くのことが定義されています。この章では最低限必要なポイントと OpenFlow 特有の用語をてっとりばやく頭に入れましょう。
//}
...
本章ではこの 2 つの中でも特によく使う機能を重点的に説明します。

OpenFlow の標準化は、Stanford 大学を中心とする OpenFlow スイッチコンソーシアムにて開始され、バージョン 1.0, 1.1 の仕様が策定されました。その後、2011 年にGoogle や Facebook などにより Software Defined Network 技術の推進を行う非営利団体 Open Networking Foundation (ONF) が設立され、OpenFlow の標準化は現在この ONF にて行われています。執筆時の OpenFlow 最新バージョンは 1.3 ですが、以降の説明では対応する商用スイッチも出そろっているバージョン 1.0 をベースに説明します。なお、1.0 とそれ以降では考え方やモデルに大きな変更はありませんので、1.0 を理解していれば新しいバージョンも理解しやすいでしょう。
...
セキュアチャンネルという名前ですが、通常は普通の TCP で接続されます。コントローラとスイッチの両方が対応している場合には、よりセキュアな TLS (Transport Layer Security) を使うことができます。
...
フローがフローテーブルから消されるとき、消されたフローの情報と、フローにしたがって処理されたパケットの統計情報がコントローラに通知されます。これを Flow Removed メッセージと呼びます。このメッセージは、ネットワークのトラフィック量の集計に用いることができます。
...
というふうに自由に複数の条件を組み合わせられます。

===[column] 取間先生いわく: OSI ネットワークモデルが壊れる？

あるネットワークの経験豊富な若者がこんな事を言っていた「OpenFlow のようにレイヤをまたがって自由に何でもできるようになると、OSI@<fn>{osi} ネットワークモデルが壊れるんじゃないか？」

//footnote[osi][Open Systems Interconnection の略で国際標準化機構 (ISO) によって制定された、異機種間のデータ通信を実現するためのネットワーク構造の設計方針のこと。]
...
この場合は、フィールド A と B を書き換えて、宛先ポート A と B へフォワードする、と読めます。このように、複数のフィールドを書き換えたり複数の宛先ポートにパケットを出したい場合には、アクションを複数連ねて指定します。
 ...
 * FLOOD: パケットを既存の L2 スイッチ機能により作られたスパニングツリーに沿って出す。
 * CONTROLLER: パケットをコントローラに明示的に送り、Packet In を起こす。
 * NORMAL: パケットを既存の L2 スイッチの機能を使って転送する。
 * LOCAL: パケットをスイッチのローカルスタックに上げる。ローカルスタック上で動作するアプリケーションにパケットを渡したい場合に使う。あまり使われない。

FLOOD や NORMAL のように、OpenFlow スイッチ機能と既存の L2 スイッチ機能を組み合わせて使用するための論理ポートも定義されています。
...
OpenFlow では、VLAN ヘッダのついたパケットを扱うことができます。VLAN とは L2 ネットワーク (ブロードキャストが届く範囲) を、複数のネットワークに分割して使用するための仕組みです。また、この分割されたネットワークのことを、VLAN と呼ぶ場合もあります。どの VLAN に所属しているかを区別するためにパケットに付与されるのが、VLAN ヘッダです。Modify-Field アクションは VLAN ヘッダの操作に必要なアクションを 3 種類用意しています。
...
: VLAN ID の書き換え
  VLAN パケットが属する仮想 LAN の ID を書き換えます。たとえば VLAN ID を 3 に書き換えるといったアクションを指定できます。また、VLAN ヘッダがついていないパケットに、指定した VLAN ID が含まれる VLAN ヘッダを付与することもできます。
