...
セキュアチャンネルという名前ですが、通常は普通の TCP で接続されます。コントローラとスイッチの両方が対応している場合のみ、よりセキュアな TLS (Transport Layer Security) を使うことができます。
...
寿命に逹したフローがフローテーブルから消されるとき、消されたフローの情報と、フローにしたがって処理されたパケットの統計情報がコントローラに通知されます。これを Flow Removed メッセージと呼びます。このメッセージは、ネットワークのトラフィック量の集計に用いることができます。
