...
マッチングルールを一言で言うと、OpenFlow スイッチがパケットを受け取ったときにアクションを起こすかどうかを決めるための条件です。つまりスイッチに書き込まれたマッチングルールに適合したパケットにだけ、スイッチがアクションを起こすというわけです。

OpenFlow 1.0 では、@<table>{matching_rules} の 12 種類の条件が使えます。これらの条件はイーサネットや TCP/UDP でよく使われる値です。なお、ネットワークに詳しい人のためにそれぞれがどのレイヤに相当するかも入れてあります。
...
これに対して従来のネットワーク機器では、ネットワークの各レイヤに対応した個々のネットワーク機器が、そのレイヤで処理を行うのが当然でした。たとえばレイヤ 2 スイッチは宛先の MAC アドレスを用いてスイッチが転送先の物理ポートを特定し、そのポートにパケットを送ります。レイヤ 3 スイッチであれば、宛先の IP アドレスを使って同様の動きをします。ここが、従来のネットワーク機器と OpenFlow との違いです。
