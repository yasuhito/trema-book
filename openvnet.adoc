...
OpenVNetの利用はとても簡単です。要件は以下の2つだけです。まずは、CentOSが稼働する1台のマシンにOpenVNetの全てのサービスをインストールし、使い始めてみましょう。マシンは、物理マシンでも仮想マシンでも構いません。

- CentOS 6.6以上(CentOS6系)が稼働するマシン
- インターネット接続
...
OpenVNetは `yum` パッケージとして提供されています。リポジトリの設定ファイルである `openvnet.repo` をダウンロードし、 `/etc/yum/repos.d/` ディレクトリに配置します。
...
次に、OpenVNetで利用するミドルウェアパッケージがまとめられたリポジトリ設定ファイルである `openvnet-third-party.repo` をダウンロードし、 `/etc/yum.repos.d/` ディレクトリに配置します。
...
`openvnet` パッケージはメタパッケージで、OpenVNetの動作に必要なパッケージを一度に全てインストールすることが出来ます。

なお、OpenVNetのインストールには エンタープライズLinux用の拡張パッケージである `epel` が必要ですので、 `epel-release` パッケージをインストールしておきます。
...
RedisおよびMySQL serverパッケージをインストールします。OpenVNetは、Redisをプロセス間通信に利用し、MySQLをネットワーク構成情報の保持に利用します。
...
OpenVNetは、OpenVNetと同時にインストールされるRubyを利用しますので、環境変数PATHにそのパスを設定しておく必要があります。
...
次に、OpenVNetのサービス群( `vnmgr` 、 `webapi` 、 `vna` )を起動します。
...
続いて、 `vnctl` ユーティリティで構成情報データベースを作成します。次のコマンドで、 `vna` がどの `datapath` を管理すればよいかをOpenVNetに教えます。

```
$ vnctl datapaths add --uuid dp-test1 --display-name test1 --dpid 0x0000aaaaaaaaaaaa --node-id vna
```

`vna` と `datapath` の紐付けをができたので、 `vna` を起動してみましょう。
...
次に仮想マシンとして2つの仮想マシン( `inst1` と `inst2` )を作成し、OpenVNetの仮想ネットワークに接続してみます。仮想マシンは、どのような仮想化技術のものでも動作します。今回は、軽量なコンテナの一種であるLXCをインストールし、仮想マシンとして利用することにします。
...
また、仮想マシン作成コマンドである `lxc-create` は `rsync` を使用するため、もしまだインストールしていない場合、以下のコマンドで　`rsync` をインストールして下さい。
...
[[openvnet_connected]]
image::openvnet_connected.png[caption="図17-1",title="仮想マシンがエッジスイッチに接続した状態"]

では、最も基本的な仮想ネットワークを1つ作成をしてみましょう。

=== CLIで操作してみる

仮想ネットワークの作成なども `vnctl` で行えます。まずは、1つの仮想ネットワークを作成してみましょう。
...
次に、どのネットワークインタフェースがその仮想ネットワークに所属しているのかを `vnctl` コマンドでOpenVNetに教えます。 これは、 `vnctl interfaces` こまんどで実行できます。まずは、 `inst1` の持つネットワークインタフェースを仮想ネットワークに設定します。
...
image::openvnet_cli_simplenetwork_2.png[caption="図17-1",title="ネットワークインタフェースにIPアドレスを設定した状態"]
...
この時点ではまだ `inst1` の `eth0` にIPアドレスを設定していないため、IPアドレスが表示されません。
仮想ネットワークはDHCPサービスを有効にしていないため、IPアドレスは手動で設定する必要があります。

`inst1` のコンソールで次のコマンドを実行することで、 `eth0` にIPアドレス `10.100.0.10` を設定します。これは、 `vnctl` で `inst1` のネットワークインタフェースのIPアドレスとして設定したものです。
...
image::openvnet_cli_simplenetwork_3.png[caption="図17-1",title="ネットワークインタフェースにIPアドレスを設定"]
...
疎通できるようになったところで、従来のネットワークとOpenVNetの仮想ネットワークとの違いを1つ紹介しておきましょう。
...
先程とは異なり、疎通ができなくなったはずです。これがもし従来のネットワークだった場合、 `10.100.0.0/24` の範囲内のIPアドレスであれば疎通できます。OpenVNetは設定情報に従って厳格に制限を行うため、`inst2` のIPアドレスが `10.100.0.11` でない限り、通信を許可しません。

=== フローエントリを見る

OpenVNetはOpenFlowで仮想ネットワークをコントロールします。
OpenVNetには `vna` と共にインストールされる `vnflows-monitor` というツールが付属しており、現在のエッジスイッチのフローエントリを読みやすく整形して表示できます。

次のコマンドでフローエントリを表示できます。
...
それぞれのフローテーブルを処理単位にグループ化して表示します。それぞれのフローテーブルの下に表示される行の意味は、左から順に、以下のとおりです。
...
先ほどは複数サーバ上のDockerコンテナ間を連結するというミクロな視点でのOpenVNet利用例を見てきました。OpenVNetは複数のクラウド間を連結するというマクロな視点でも利用できます。
...
VNetEdgeはこの連結を次のように実現しています。まず、仮想ネットワークIDとVLAN IDの変換規則をOpenVNetに登録します。その後、Wakame-vdcの仮想ネットワークと、Amazon Web ServicesのVirtual Private Cloudで構築されたネットワークの間を流れるパケットがVNetEdgeのエッジスイッチを通過する際に、この２つのネットワークが同一のネットワークであるかのようにパケット転送を制御します。
