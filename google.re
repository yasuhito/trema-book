...
#@warn(具体的な回線コストをどこかからひっぱってきて載せてください。載せました。計算あってるかな？？？)
Google が他社と共同で引いた日米間約 1 万km の海底ケーブルは、総建設費が約 320 億円でした。このケーブルの回線容量は 4.8Tbps なので、1Gbps 当たりで考えると約 670 万円かかったことになります。このように長距離を結ぶ WAN 回線は非常に高価であるため、世界各地にデータセンターを持つ Google では、WAN 回線の有効活用は優先して解決すべき課題です。
...
#@warn(この図で一番重要なはずの TE サーバがまったく目立っていません。。。もっと大きくしましょう。あと TE サーバがやることはフキダシにしたほうがいいと思います)
#@warn(今までスイッチだった箇所がいきなり WAN 接続部になっているので、ここで簡単に説明が必要です。とくに、スイッチとはどこが違うのかを書けばよいでしょう)
...
#@warn(いきなりアーキテクチャに入る前に WAN 接続部の役割は何なのか、ざっくりと箇条書きで説明してください)

次に、WAN 接続部の構成について見ていきます (@<img>{wan_equip})。WAN 接続部は、複数の OpenFlow スイッチと、それらを制御する OpenFlow コントローラと BGP 処理部から構成されています。

//image[wan_equip][それぞれの WAN 接続部毎に、OpenFlow コントローラと BGP 処理部が配置されている][scale=0.5]

WAN 接続部には、大量のトラフィックを扱えるように、10Gbps Ethernet のインターフェイスを 100 ポート以上持つ大規模な OpenFlow スイッチが複数台配置されています。この OpenFlow スイッチは、Google が自ら開発したものです。なぜなら、このような仕様を持つスイッチは市場には存在しないためです。自前での装置開発にはそれなりのコストが掛かりますが、WAN の効率化により削減できるコストの方が大きいと、Google は判断したのでしょう。

BGP 処理部は、他の WAN 接続部の BGP 処理部との間で、データセンターから受け取った経路情報を交換します。この BGP 処理部には、オープンソースの経路制御ソフトウェアである Quagga を用いています。

OpenFlow コントローラは、BGP 処理部や TE サーバからの情報を元に、以下の手順で OpenFlow スイッチにフローエントリを設定します。

1. BGP 処理部から受け取った経路情報を元に、トラフィックがどのデータセンター宛なのかを決定する
2. トラフィックを流すための必要帯域を TE サーバに通知し、転送のための経路の割り当てを受ける
3. 割当を受けた経路にトラフィックが流れるよう、OpenFlow スイッチにフローエントリを設定する
