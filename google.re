...
//image[wan_equip][G-Scale を構成するスイッチは、BGP 処理部、OpenFlow コントローラ、そして OpenFlow スイッチから成る][scale=0.6]
 ...
 1. トラフィックがどのデータセンター宛なのかを決定するのが、BGP 処理部です。G-Scale がトラフィックを宛先のデータセンターに届けるためには、データセンター内にあるホストの IP アドレスを知る必要があります。そのために、BGP 処理部はデータセンターから IP アドレスに関する情報 (経路情報) を受け取り、他の BGP 処理部と交換します (詳細はコラムを参照)。このようにすべての経路情報を収集することで、パケット中の宛先アドレスからどのデータセンターに送るべきかを判断します。
