...
動作 1 を行うのが、BGP 処理部です。G-Scale がトラフィックを宛先のデータセンターに届けるためには、データセンター内にあるホストの IP アドレスを、G-Scale 中の各 WAN 接続部に教えてあげる必要があります。そのために、G-Scale では、BGP 処理部がデータセンターからアドレスに関する情報 (経路情報) を受け取り、他の WAN 接続部中の BGP 処理部に通知しています。この BGP 処理部には、オープンソースの経路制御ソフトウェアである Quagga が用いられています。
