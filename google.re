...
この OpenFlow スイッチは、Google が自ら開発したものを用いています。大量のトラフィックを扱えるように、このスイッチは 10Gbps Ethernet のインターフェイスを 100 ポート以上持ちます。このような仕様を持つスイッチは市場には存在しないため、Google は自作を行ったようです。自前での装置開発にはそれなりのコストが掛かりますが、WAN の効率化により削減できるコストの方が大きいと、Google は判断したのでしょう。
