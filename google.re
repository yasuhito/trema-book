...
ネットワーク中の全 OpenFlow スイッチを一台のコントローラで制御するルーティングスイッチ (@<chap>{routing_switch}) とは異なり、G-Scale は複数の OpenFlow コントローラが協調してネットワークを制御しています。各 OpenFlow コントローラが制御を行なっているのは、それぞれの WAN 接続部内の OpenFlow スイッチだけです。BGP や TE サーバからの情報を元にフローエントリを設定することで、G-Scale 全体として WAN の帯域の有効活用を実現しています。

複数コントローラの協調動作については、さまざまな実現方法が考えられるため、OpenFlow の仕様では規定されていません。ここで紹介した G-Scale の例も、あくまで複数 OpenFlow コントローラの協調動作の一例でしかありません。しかし、実際に運用されており、OpenFlow の協調動作における代表例と言ってもいいでしょう。
