...
さらに、データセンター間ネットワークの効率的な活用も重要な課題です。Google は世界各国に巨大なデータセンターを多数持っており、これらの間をつなぐ回線も多数有しています。これらの回線は容量が高いものほど高価になるので、必要最小限の回線を最大限に活用する必要があります。
...
それでは、Google's Software Defined WAN がどのように動作しているか見ていきましょう。Google's WAN を模式的に示したのが @<img>{interdc-network} になります。

//image[interdc-network][Google's WAN]

データセンター間ネットワークが、データセンターから受信したパケットを他のデータセンターに適切に届けるためには、以下の二つを行う必要があります。
...
データセンターがデータセンター間ネットワークからパケットを受け取るためには、データセンター内にあるホストの IP アドレスを、データセンター間ネットワークに教えてあげる必要があります。そうしなければ、データセンター間ネットワークは受け取ったパケットをどのデータセンターに送り届ければいいかを判断できません。

Google's WAN では、この目的のために BGP を用いています。BGP は、異なる組織間で、アドレスに関する情報 (経路情報と呼びます) を交換するためのプロコトルです。インターネットのように、様々な組織から構成されるネットワークを作るためには欠かすことのできないプロトコルです。BGP では、それぞれの組織を AS と呼び、AS 単位での経路情報の交換を行います。

Google's WAN では、各データセンターおよびデータセンター間ネットワーはそれぞれを一つの組織に見立て、それぞれが一つの AS として動作しています。データセンター間ネットワークは、BGP により各データセンターから経路情報をもらい、その情報を元にしてパケットの届け先を決定しています。

//image[bgp][BGP を用いた経路通知][scale=0.4]

例えば、データセンター A 内のホストには 192.168.1.0/24 のアドレスを持っていたとします @<img>{bgp}。データセンター A はこのアドレスを BGP を用いて、データセンター間ネットワークに教えます。このようにすることで、例えば宛先が 192.168.1.1 であるパケットを受け取った時に、そのパケットをデータセンター A へと送り届ければよいということを、データセンター間ネットワークは知ることができます。

=== 転送パスの決定

届け先のデータセンターが決定したら、次はパケットの転送に使うパスを決める必要があります。

Google's WAN では、データセンター間ネットワークに流入するトラフィック量を測定し、全体を調停するトラフィックエンジニアリングサーバ (TE サーバ) を導入しています。TE サーバは、各データセンターからの流入トラフィックに関する情報を集めます。そして、TE に関するポリシーを OpenFlow コントローラへと送ります。OpenFlow コントローラがフローを生成する際には、このポリシーを元にすることで、トラフィック状況に応じたパス選択を実現しています。
