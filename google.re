...
届け先のデータセンターがわかれば、次はトラフィックを転送するパスを決定します。そのためには、前章で述べた三つの課題をクリアする必要があります。

TE サーバが、ネットワーク中の各リンクの空き帯域の情報を収集します(@<img>{collect-traffic-info}。

//image[collect-traffic-info][TE サーバは、各 WAN 接続装置から空き帯域情報を収集する]

データセンターからの通信要求に対して、必要な帯域の割当を TE サーバにより行います。このように空き帯域の管理を TE サーバで行うことにより、課題 2 の空き帯域の競合の問題を回避します。

//image[assign][TE サーバが、空き帯域の中から通信に必要な帯域を割り当てる]
