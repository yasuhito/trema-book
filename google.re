...
//lead{
データセンターを一台の巨大なコンピュータと見立てると、この巨大コンピュータ同士をつなぐネットワークが必要になります。たくさんのデータセンターを持つGoogleは、OpenFlow を使って低コストでこのネットワークを作り上げてしまいました。
//}

== Googleは OpenFlow を導入ずみ!
...
Googleはインターネット上の情報を大量に収集するために、最新鋭のデータセンターをいくつも構築しました。例えばGoogleが公開している資料からは、以下のすさまじいデータを発見できます。
 ...
 * WAN 回線は容量が大きいほど高価になります。たとえば、あるキャリアで 500 km を接続するサービスの価格を比べると、 10 Gbps は 1Gbps のおよそ 1.5 倍になります。コストを下げるためには、なるべく容量の小さく安い回線を最大限まで使う必要があります。
 * WAN 回線は距離が長いほど高価になります。Googleが他社と共同で引いた日米間約 1 万km、回線容量は 4.8 Tbps の海底ケーブルは、総建設費が約 320 億円にもなりました。コストを下げるためには、あまり長いケーブルは引けません。
 ...
 * 無駄なリンク帯域の増設によるコストを避けるために、既存のリンクを使って帯域をかせぐ方法を見ました。Googleは WAN 全体の空き帯域の監視と割り当てを行う TE サーバを導入することで、複数経路を使った転送を効率化しています。
