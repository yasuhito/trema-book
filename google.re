...
: OpenFlow スイッチ
  OpenFlow コントローラからの指示に従い、実際にトラフィックを転送するのが OpenFlow スイッチです。大量のトラフィックを扱う必要があるため、10Gbps Ethernet のインターフェイスを 100 ポート以上持つ大規模な OpenFlow スイッチを複数台使います。この規模のスイッチはまだ市場には存在しないため、グーグルは 1 からこのスイッチを開発したそうです。自前での開発にはそれなりのコストが掛かりますが、WAN の効率化により削減できるコストの方が大きいとグーグルは判断したのでしょう。
 ...
 * 無駄なリンク帯域の増設によるコストを避けるために、既存のリンクを使って帯域をかせぐ方法を見ました。グーグルは WAN 全体の空き帯域の監視と割り当てを行う TE サーバを導入することで、複数経路を使った転送を効率化しています。
