...
しかし、実際に複数の経路を使ってトラフィックを流すには、次の 3 つの課題をクリアしなければなりません。
 ...
 1. すべてのリンクの空き帯域を把握するには？
 2. 空いた帯域をどのトラフィック転送に使うか？
 3. トラフィックを複数の経路にどうやって振り分ける？

//noindent
それでは、それぞれの課題を詳しく見ていきましょう。

=== 課題 1：すべてのリンクの空き帯域を把握するには？

一つ目の課題は、各リンクの空き帯域を知る必要があるという点です。@<img>{multipath} では、データセンター A からデータセンター B へは 16Gbps までトラフィックを流せると説明しました。しかし、普通は他のデータセンター間にもトラフィックがあるので、16Gbps のうち空いている帯域しか使用できません。

そこで実際にどの程度トラフィックを流せるかを知るためには、各リンクの空き帯域の情報が必要です。データセンター A は、スイッチ A がやり取りしているトラフィック量を調べることで、リンク 1 とリンク 2 の空き帯域が分かります。しかし、データセンター A から離れた場所にあるリンク 3 の空き帯域については、何らかの方法で取得しなければなりません。実際のネットワークにはもっとたくさんのリンクがあるため、すべてのリンクの空き帯域を調べる必要があります。
