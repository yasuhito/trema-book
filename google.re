...
動作 1 を行うのが、BGP 処理部です。G-Scale がトラフィックを宛先のデータセンターに届けるためには、データセンター内にあるホストの IP アドレスを、G-Scale 中の各 WAN 接続部に教えてあげる必要があります。そのために、G-Scale では、BGP 処理部が、データセンターからアドレスに関する情報 (経路情報) を受け取り、他の WAN 接続部と共有するという役目を果たしています。この BGP 処理部の実装には、オープンソースの経路制御ソフトウェアである Quagga が用いられています。

動作 2 は、OpenFlow コントローラが行います。OpenFlow コントローラは、BGP 処理部や TE サーバからの情報を元に、OpenFlow スイッチにフローエントリを設定します。

動作 3 を行うのが、OpenFlow スイッチです。大量のトラフィックを扱う必要があるため、10Gbps Ethernet のインターフェイスを 100 ポート以上持つ大規模な OpenFlow スイッチが複数台配置されています。このような仕様を持つスイッチは、市場には存在しないため、Google 自らが開発を行いました。自前での装置開発にはそれなりのコストが掛かりますが、WAN の効率化により削減できるコストの方が大きいと、Google は判断したのでしょう。
