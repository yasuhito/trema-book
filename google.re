...
それでは、Google のネットワークがどのように動作しているか見ていきましょう。
...
G-Scale では、各データセンターおよびデータセンター間ネットワーはそれぞれを一つの組織に見立て、それぞれが一つの AS として動作しています。データセンター間ネットワークは、BGP により各データセンターから経路情報をもらい、その情報を元にしてパケットの届け先を決定しています。

//image[bgp][BGP を用いた経路通知][scale=0.4]

例えば、データセンター A 内のホストには 192.168.1.0/24 のアドレスを持っていたとします @<img>{bgp}。データセンター A はこのアドレスを BGP を用いて、G-Scale に教えます。このようにすることで、例えば宛先が 192.168.1.1 であるパケットを受け取った時に、そのパケットをデータセンター A へと送り届ければよいということを、G-Scale は知ることができます。
