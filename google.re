...
Google はいくつもの最新鋭のデータセンターを使って、インターネット上のさまざまな情報を大量に収集しています。例えば Google が公開している資料からは、以下のすさまじいデータを発見できます。
...
//noindent
このような大量のデータを効率的に処理するために、Google は処理能力が高く電力効率も良いデータセンターを低いコストで実現しています。

データセンターは効率化できたが、ほかに改善できる部分は無いだろうか？Google はデータセンター同士をつなぐネットワーク (広域通信網、WAN) に目をつけました。OpenFlow を使えば、データセンター間のスループットを低コストで改善できそうだ。これが Google の狙いです。
