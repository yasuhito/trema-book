...
データセンター間のトラフィックをさばきつつ、回線コストを抑えるためには、WAN 回線の有効活用が必要です。例えば、@<img>{wan} のデータセンター B からデータセンター A へどの程度のトラフィックを流せるか考えてみます。通常の IP ルーティングでは、最短経路となるリンク 1 しか使用しないため、このリンクの帯域が上限となります。リンク帯域の上限を超えるトラフィックを扱うためには、リンク帯域の増設が必要になり、コストが増える要因となります。しかし、@<chap>{openflow_usecases} で紹介したように、スイッチ C を経由する経路もあわせて使うことで、さらなるトラフィックを流すことができます(@<img>{multipath})。
...
しかし、実際には他のデータセンター間のトラフィックも流れているため、それらのトラフィック量を知る必要があります。16Gbps のうち、他のトラフィックで使用していない帯域しか使用できないためです。データセンター B は、自身がやり取りしているトラフィック量を知ることが容易です。しかし、データセンター A とデータセンター C 間のトラフィックについては、何らかの方法で取得する必要があります。
