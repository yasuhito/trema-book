...
データセンター間のトラフィックをさばきつつ、回線コストを抑えるためには、WAN 回線の有効活用が必要です。例えば、@<img>{wan} のようなネットワークを考えてみましょう。各スイッチ間のリンク帯域は、それぞれ 10Gbps であったとします。データセンター A とデータセンター C 間のトラフィックが 4 Gbps, データセンター B とデータセンター C 間のトラフィックも同様に 4Gbps であるとします。この時、データセンター B からデータセンター A へは、どの程度のトラフィックを流すことができるでしょうか？まず、データセンター A とデータセンター B 間のリンクを使えば、10Gbps のトラフィックを流すことができます。さらにデータセンター C を経由するパスを使えば、さらに 6Gbps のトラフィックを流すことができそうです。

//image[wan][データセンター A とデータセンター B 間でやり取り可能なトラフィックの最大値は？]

このようなマルチパス転送を実現するためには、どのようにすればよいでしょうか？まず、16Gbps のトラフィックを二つのパスに分けて転送を行う必要があります。@<chap>{openflow} で説明したように、OpenFlow スイッチではヘッダの情報を見てどちらのパスにパケットを転送するかを決定することしかできません。ちょうど 100Gbps と 60Gbps となるようトラフィックを分けるためには、サービス毎のトラフィック量を知っている必要があります。
