...
OpenFlow の一番の特徴は「たくさんのスイッチをひとつのコントローラで集中管理する」というそのアーキテクチャにあります。スイッチはパケットの転送だけに集中しコントローラは全体の統括、という風にうまく分業することで、自動化やシステム連携のしやすさ、トラフィックエンジニアリングの容易さ、スイッチの低価格化、そしてベンダロックインの防止といったさまざまなメリットがあるのでした (@<chap>{whats_openflow})。

本章ではこの集中管理の一例として、スイッチの監視ツールを作ります。このツールは「今、ネットワーク中にどんなスイッチが動いているか」をリアルタイムに表示します。たくさんあるスイッチ全体が正常に動いているかを確認するのに便利です。
...
また、シンボルは "引数にメソッドを取るメソッド (いわゆる高階関数)" を使うときにも登場します。具体的な例を見ていきましょう。まず、次の @<tt>{hello} は画面に "Hello" と表示するだけのメソッドです。
...
そして、任意のメソッドを引数にとり 3 回実行してくれる @<tt>{repeat_3times} というメソッドがあったとします。もし @<tt>{hello} を 3 回実行したければ、この @<tt>{repeat_3times} に @<tt>{hello} メソッドを渡してやればいいわけです。

ここでもし次のように @<tt>{repeat_3times} の引数に @<tt>{hello} を直接渡してしまうとどうなるでしょうか。
...
//footnote[repeat_3times][@<tt>{hello} の返り値は@<ruby>{空,から} (Ruby の世界では @<tt>{nil} と表現されます) なので @<tt>{repeat_3times nil} と同じになり、@<tt>{repeat_3times} の実装によっては最悪エラーになります。]

そこでもしメソッドにメソッドを渡したいときには、引数として渡すメソッド名を @<tt>{:hello} とシンボルにし、誤って先に実行されないようにします。
...
こうすれば、シンボルを渡された @<tt>{repeat_3times} 側ではシンボルから元のメソッドを取り出して呼び出すことができます。このように、メソッドの引数にメソッド名を渡すときはシンボルを使う、と覚えてください。

=== インスタンス変数

アットマークで始まる語はインスタンス変数です。@<tt>{@x} や @<tt>{@age} や @<tt>{@switches} がそれに当たります。これらの変数はたとえば人間の歳や身長などといった属性を定義するときによく使われます。アットマークはアトリビュート (属性) を意味すると考えれば覚えやすいでしょう。

インスタンス変数は同じクラスの中のメソッド定義内であればどこからでも触れます。次の例を見てください。
...
@<tt>{Boy} クラスで定義される @<tt>{Boy} オブジェクトは、初期化したときにはそのインスタンス変数 @<tt>{@age} は 0、つまり 0 歳です。@<tt>{happy_birthday} を呼び出すたびに歳をとり @<tt>{@age} が 1 インクリメントされます。このように @<tt>{@age} は @<tt>{initialize} および @<tt>{happy_birthday} メソッドのどちらからでも操作できます。
...
//noindent
配列は空ではないので、@<tt>{false} (偽) が返ってきました。@<br>{}
...
//noindent
配列の各要素の間に指定した文字をはさんで連結 (@<tt>{join}) しています。@<br>{}

Ruby にはこのような組込みメソッドが何百種類もあります。それぞれの動作と結果は名前から大体想像できるので、心配はいりません。@<br>{}
