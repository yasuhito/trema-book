...
OpenFlow の特徴は「たくさんのスイッチをひとつのコントローラで集中制御する」という点にありました。スイッチはフローテーブルに従ったパケットの転送だけ、頭脳であるコントローラはスイッチ全体の統括、という風にうまく分業させるというわけです。これによって第 I 部で見てきたように、自動化やシステム連携のしやすさ、トラフィックエンジニアリングの容易さ、スイッチの低価格化、そしてベンダロックインの防止といったさまざまなメリットが生まれるのでした。
...
まずはざっとスイッチ監視ツールのソースコードを眺めてみましょう。このソースコードは Trema のサンプルアプリに付属する @<tt>{switch_monitor/switch-monitor.rb} でも読むことができます。今回も Ruby の文法は後まわしで先に実行してみますので、細かいところは気する必要はありません。@<chap>{openflow_framework_trema}で学んだ Ruby の品詞を頭の片隅に置きながら、ざっくりと次のコードに目を通してみてください。
...
スイッチ監視ツールのソースコードを眺めると、いくつか見慣れない品詞や構文が登場していることに気付きます。この節では順にそれぞれを紹介していきますが、最初からすべてを覚える必要はありません。もし後でわからなくなったときには見直すようにしてください。
