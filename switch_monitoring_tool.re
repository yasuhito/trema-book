...
OpenFlow の特徴は「たくさんのスイッチをひとつのコントローラで集中制御する」という点にありました。つまりスイッチはフローテーブルに従ったパケットの転送だけ、頭脳であるコントローラはスイッチ全体の統括、という風にうまく分業させるというわけです。これによって自動化やシステム連携のしやすさ、トラフィックエンジニアリングの容易さ、スイッチの低価格化、そしてベンダロックインの防止といったさまざまなメリットがあるのでした。
