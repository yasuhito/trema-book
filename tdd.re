...
私が小学生だった80年代の家庭用ゲーム開拓時代には、腕っぷしだけで稼ぐ天才ゲームプログラマ達の伝説がたくさんありました。色とりどりの小さなカセットに驚くほどたくさんの要素を詰め込んだ名作ファミコンゲームの数々。爆発的なプログラミング技術とデバッグ能力だけで何でも作ってしまう時代でした。そんな中でも、"ファイナル・ファンタジー"の伝説的プログラマ、ナーシャ・ジベリの早撃ちエピソードは有名です。「コードのみてくれやちょっとぐらいのバグは気にしねえ。誰よりも早くやってやるぜ」。彼はカウボーイのようにさっそうと現場に現れ、どんなコードでも電光石火で書いてしまったそうです。

彼のコードは信じられないほど高速に動き、ゲームは100万本以上も売れましたが、ひとつだけ困ったことがありました。彼の超絶コードは彼にしか理解できず、バグがあっても誰も修正できないのです。本当の問題は彼が現場を去った約10年後に起こりました。新しいハード向けにリメイク版を作ろうと当時の彼のコードを掘り起こしたところ、謎のテクニックだらけで誰にも動作を再現できなかったのです。「ナーシャ・カムバック!」でも彼はどこにいるのかわかりません。結局、リメイク版の発売は6年も遅れてしまいました。

== 奥義、ソフトウェアテスト

こうした悲劇を防ぐ奥義の一つがソフトウェアテストです。きちんと整備されたテストコードは、元のコードの仕様書のようなものです。しかもこの仕様書は実際に動かせるのです! たくさんのスイッチを接続したOpenFlowネットワークとコントローラの保守をまかされたとしましょう。もし前任者からテストコードをもらえなければ、ファミコン時代に逆戻りです。テストさえもらえればコード本体を理解しやすくなりますし、気楽にリファクタリングや修正ができます。とくにOpenFlowではスイッチとコントローラが複雑に絡み合い、しかもそれぞれがステートを持つので、ソフトウェアで自動化されたテストがないとやってられません。

TremaはOpenFlowコントローラ開発のためのテストツールが充実しています。アジャイル開発者の大事な仕事道具、テスト駆動開発もTremaはサポートしています。本章ではテスト駆動を使ったコントローラの開発テクニックを紹介しますが、要点をつかみやすくするため動作の単純ないわゆる「バカハブ(ダムハブ)」、つまりリピータハブを採り上げます。ではさっそく実際の流れを見て行きましょう。
...
まずは、リピータハブがどのように動くか説明しましょう。ホスト3台がリピータハブにつながったネットワークを考えてください(@<img>{repeater_hub})。ホスト1からホスト2へパケットを送信すると、リピータハブは入ってきたパケットを複製してほかのすべてのホストにばらまいてしまいます。つまり、通信に関係のないホスト3もホスト2あてのパケットを受信します。このように関係のないホストにもパケットを送ってしまうので「バカ」ハブとか「ダム」ハブと呼ばます。

//image[repeater_hub][ホスト 3 台をつなげたリピータハブの動作][width=12cm]

これをOpenFlowで実装すると@<img>{repeater_hub_openflow}のようになります。ホスト1がパケットを送信すると、スイッチからコントローラにPacket Inが起こります。ここでコントローラは「今後は同様のパケットをほかの全ポートへばらまけ(FLOOD)」というFlow Modを打ちます。また、Packet Inを起こしたホスト1からのパケットをほかのすべてのホスト(ホスト2とホスト3)に明示的にPacket Outで届けます。

//image[repeater_hub_openflow][OpenFlow版リピータハブ][width=12cm]

== どこまでテストするか？

設計も決まったので、テストを書き始める前にテスト戦略を決めます。テスト戦略とは言い換えると「どこまでテストするか？」ということです。これはむずかしい問題なので、ソフトウェアテスト界の賢人達の言葉を借りることにします。

テスト駆動開発の第一人者、ケントベックは@<href>{stackoverflow.com}の「どれだけ深くテストをすればいいか？」というトピック@<fn>{stackoverflow}に次の投稿をしています。
...
Ruby on Railsの作者として有名なDHHは、彼の勤める37signalsのブログで次のように語っています@<fn>{37signals_blog}。
...
//quote{
@<strong>{シナリオ2：}「ホスト3台(ホスト1,2,3)がスイッチにつながっているとき、ホスト1がホスト2へパケットを送ると、パケットをホスト2,3へばらまくフローエントリーがスイッチに書き込まれる」
//}
...
テストコードの最初のバージョンは@<list>{repeater_hub_test_template}のとおりです。最初の@<tt>{require}ではじまる行は、テストに必要なTremaのライブラリを読み込みます。@<tt>{describe}で始まる@<tt>{do...end}ブロックはテストの本体で、RepeaterHubコントローラのふるまいをここに記述(describe)する、という意味です。
...
//list[add_repeater_hub_class][空の@<tt>{RepeaterHub}クラスを追加してNameErrorを修正]{
require File.join( File.dirname( __FILE__ ), "spec_helper" )
...
//quote{
@<strong>{シナリオ1：}「ホスト3台(ホスト1,2,3)がスイッチにつながっているとき、ホスト1が ホスト2へパケットを送ると、ホスト2,3がパケットを受け取る」
//}
...
Tremaの仮想ネットワーク設定とまったく同じ文法ですね！ここでは、それぞれの仮想ホストが@<tt>{promisc}オプション(自分宛でないパケットを受け取る)を@<tt>{"on"}にしていることに注意してください。リピータハブはパケットがすべてのポートにばらまくので、こうすることでホストがどこからのパケットでも受信できるようにしておきます。
...
ここで分かれ道です。このテストシナリオ1だけで十分な人はRepeaterHub本体の実装に進んでください。さらに段階を踏んでテストを書いたほうがスッキリする人は、次のテストシナリオ2を実装します。

== フローエントリーのテスト

スイッチにフローエントリーができることをテストしてみましょう。さっそくテストシナリオ2をGiven,When,Thenに分解すると次のようになります。

 * Given：ホスト3台(ホスト1,2,3)がスイッチにつながっているとき、
 * When：ホスト1がホスト2へパケットを送ると、
 * Then：パケットをばらまくフローエントリーをスイッチに追加する。
...
//list[test_flow_entry][フローエントリーのテスト]{
describe RepeaterHub do
  it "は、入ってきたパケットを他のすべてのポートに転送する" do
    # (省略)
  end
  ...
  it "は、パケットをばらまくフローエントリーをスイッチに追加する" do
    network {
      vswitch( "switch" ) { dpid "0xabc" }
      vhost( "host1" ) { promisc "on" }
      vhost( "host2" ) { promisc "on" }
...
「スイッチにフローエントリーが1つあるはずが、なかった」というエラーです。では、Flow Modを打ち込むコードを@<tt>{RepeaterHub}クラスに追加して、もう一度テストしてみましょう(@<list>{tdd_sending_flow_mod})。
...
今度はテストが通りました！それでは、もう少しThenを詳細化し、フローの特徴を細かくテストしてみます(@<list>{tdd_test_src_dst})。
...
ここではホストにIPアドレスを振り、フローの@<tt>{nw_src}と@<tt>{nw_dst}がこのアドレスに正しく設定されているかをチェックしています。実行してみましょう。
...
失敗しました。フローの@<tt>{nw_src}には、パケット送信元であるhost1のIPアドレス192.168.0.1がセットされるべきですが、何もセットされていません。では、Flow Modで@<tt>{:match}を指定して、この値がセットされるようにします(@<list>{tdd_set_match_to_flow_mod})。
...
テストにパスしました！ここまでやれば、フローエントリーが正しくスイッチに書き込まれていることに確信が持てます。
...
Tremaのユニットテストフレームワークを使ってリピータハブを作りました。今回学んだことは次の2つです。

 * コントローラをテスト駆動開発する方法を学びました。TremaはRubyのユニットテストフレームワークRSpecと統合されており、仮想スイッチのフローテーブルや仮想ホストの受信パケット数などについてのテストを書けます。
 * テストをGiven、When、Thenの3ステップに分けて分析し設計する方法を学びました。それぞれのステップをRSpecのテストコードに置き換えることで、テストコードが完成します。

Tremaのサンプルディレクトリ(@<tt>{src/examples})の下にはテストコードのサンプルがいくつかあります。本格的にテストコードを書く人は参考にしてください。
...
: リファクタリング(マーチン・ファウラー著、ピアソン・エデュケーション)
  この本の最大の功績は、コードのまずい兆候(重複するコードがあるとか、長すぎるメソッドなど)を「コードの臭い」と表現したことです。粗相をした赤ちゃんのおむつのように臭うコードには改善が必要で、この本にはそのためのレシピがそろっています。この本はJavaですが、Ruby版(リファクタリング：Rubyエディション、Jay Fieldsら著、ASCII)もあります。

: reek(@<href>{https://github.com/troessner/reek})
  「コードの臭い」を検知する能力はプログラマの美意識にいくらか依存しますが、ソフトウェアで客観的に検知できるとしたらすばらしいと思いませんか。reekはRubyコードの臭いを自動的に検知して改善すべき場所を教えてくれる便利なツールです。次に採り上げるflog,flayとともに、この本のサンプルコードを書く際にとてもお世話になりました。
...
: flay(@<href>{http://ruby.sadi.st/Flay.html})
  この「Rubyサディストのためのツール その2」は、重複するコードを探して容赦なく指摘してくれます。DRYを目指すならこのツールを使って重複を完璧になくすべきです。
