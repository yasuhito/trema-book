...
私が小学生だった 80 年代の家庭用ゲーム開拓時代には、腕っぷしだけで稼ぐ天才ゲームプログラマ達の伝説がたくさんありました。色とりどりの小さなカセットに驚くほどたくさんの要素を詰め込んだ名作ファミコンゲームの数々。爆発的なプログラミング技術とデバッグ能力だけで何でも作ってしまう時代でした。そんな中でも、"ファイナル・ファンタジー" の伝説的プログラマ、ナーシャ・ジベリの早撃ちエピソードは有名です。「コードのみてくれやちょっとぐらいのバグは気にしねえ。誰よりも早くやってやるぜ」。彼はカウボーイのようにさっそうと現場に現れ、どんなコードでも電光石火で書いてしまったそうです。
...
こうした悲劇を防ぐ奥義の一つがソフトウェアテストです。きちんと整備されたテストコードは、元のコードの仕様書のようなものです。しかもこの仕様書は実際に動かせるのです! たくさんのスイッチを接続した OpenFlow ネットワークとコントローラの保守をまかされたとしましょう。もし前任者からテストコードをもらえなければ、ファミコン時代に逆戻りです。テストさえもらえればコード本体を理解しやすくなりますし、気楽にリファクタリングや修正ができます。とくに OpenFlow ではスイッチとコントローラが複雑に絡み合い、しかもそれぞれがステートを持つので、ソフトウェアで自動化されたテストがないとやってられません。
...
まずは、リピータハブがどのように動くか説明しましょう。ホスト 3 台がリピータハブにつながったネットワークを考えてください (@<img>{repeater_hub})。ホスト 1 からホスト 2 へパケットを送信すると、リピータハブは入ってきたパケットを複製してほかのすべてのホストにばらまいてしまいます。つまり、通信に関係のないホスト 3 もホスト 2 あてのパケットを受信します。このように関係のないホストにもパケットを送ってしまうので「バカ」ハブとか「ダム」ハブと呼ばます。
...
//quote{
@<strong>{シナリオ 2:} 「ホスト 3 台(ホスト 1, 2, 3)がスイッチにつながっているとき、ホスト 1 が ホスト 2 へパケットを送ると、パケットをホスト 2, 3 へばらまくフローエントリーがスイッチに書き込まれる」
//}
...
When は「○○したとき」というきっかけになる動作を記述します。ここでは、Given で定義されたホスト host1 から host2 にパケットを送ります。パケットを送るコマンドは、@<tt>{trema send_packets} でした。もちろん、テストコード中でもこれに似た API を使えます(@<list>{tdd_when})。
...
@<tt>{vhost("ホスト名")} は仮想ホストにアクセスするためのメソッドで、仮想ホストの受信パケットなどさまざまなデータを取得できます。ここでは、受信したパケットの数、つまり受信パケットカウンタ @<tt>{stats( :rx )} が 1 ということをテストしています。
...
スイッチにフローエントリーができることをテストしてみましょう。さっそくテストシナリオ 2 を Given, When, Then に分解すると次のようになります。
...
「スイッチにフローエントリーが 1 つあるはずが、なかった」というエラーです。では、Flow Mod を打ち込むコードを @<tt>{RepeaterHub} クラスに追加して、もう一度テストしてみましょう(@<list>{tdd_sending_flow_mod})。
...
テストが通ったので、最後にコードの重複部分をまとめておきましょう。同じ @<tt>{network {...\}} が重複しているので、RSpec の @<tt>{around} ブロックを使って 1 か所にまとめます(@<list>{tdd_refactoring})。
...
いよいよ完成も間近です。テストシナリオ 1 に戻り、パケットが host2 と host3 に届くことをテストします。さきほどの保留マーク (pending) を消して、再び実行してみましょう。
