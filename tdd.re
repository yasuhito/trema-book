...
テストファーストはテスト駆動開発のステップの一部なので、テスト駆動開発のほうがより大きな概念になります。テスト駆動開発では、まず最初に失敗する見込みでテストを書き (このステップがテスト・ファースト)、次にこのテストを通すためのコードを書きます。最後にコードをリファクタリングして、クリーンにします。この3ステップを数分間隔で何度も回しながら開発するのがテスト駆動開発です。
...
人によっては、思ったとおりのフローがスイッチに書き込まれたか気になる人もいるでしょう。その場合の追加のテストシナリオはこうなります。

//quote{
@<strong>{シナリオ2}：ホスト3台(ホスト1、2、3)がスイッチにつながっているとき、ホスト1がホスト2へパケットを送ると、パケットをホスト2、3へばらまくフローエントリがスイッチに書き込まれる
//}
