...
//lead{
ネットニュースをにぎわせている OpenFlow って何だろう？一体どんな仕組みで何がうれしいの？難しいネットワーク用語を使わなくても、身近なたとえ話ですっきりと理解できます。
//}
...
ソフトウエアによる横着はハッカーが最も創造性を発揮する分野の一つです。たとえば、ハッカーの巣窟として有名な MIT の AI ラボにはかつて、コンピューターからオンラインでピザを注文できる UNIX コマンドが存在しました@<fn>{xpizza}。また、RFC で標準化されているコーヒーポットプロトコルでは、遠隔地にあるコーヒーポットのコーヒーの量を監視したり、コーヒーを自動的にいれたりするための半分冗談のインターフェースを定義しています@<fn>{rfc2325}。
...
こうした「ソフトウエアで楽をする」ハックのうち、もっとも大規模な例が最新鋭のデータセンターです。クラウドサービスの裏で動くデータセンターは極めて少人数の管理者によって運用されており、大部分の管理はソフトウエアによって極限まで自動化されているという記事を読んだことがある人も多いでしょう。このようにピザやコーヒーのようなお遊びから、データセンターのように一筋縄ではいかない相手まで、ソフトウエアで「モノ」を思いどおりにコントロールするのはもっとも楽しく、そして実際に役に立つハックの一種です。

== SDN: ネットワークをソフトウエアで制御しよう

その中でもネットワークをハックする技術の 1 つが、本書で取り上げる OpenFlow です。OpenFlow を一言で言うと、ネットワークスイッチをソフトウエア制御する仕様の一つです。簡単に言えば、OpenFlow はネットワークスイッチの中身をいじるための方式の一つで、ソフトウエアでスイッチ内部の動作を自由に変更できるようにするためのものです。Software Defined Networking (SDN) という用語を聞いたことがある人もいると思いますが、OpenFlow はネットワークをソフトウエアで書いてしまおうという野心的な技術なのです。

OpenFlow の登場によって、これからはネットワークもプログラミングの対象になります。つまり今までは専門のオペレータ達が手作業で管理していたネットワークが、ついに無精なハッカー達の手にも渡るのです。「今までネットワークって手で管理してたの？ そんなの横着しちゃおうよ!」。横着とは言い換えれば自動化による効率化です。ネットワークをソフトウエアとして書くことにより、次のような「究極の自動化」も夢ではなくなります。

 * アプリケーションに合わせて勝手に最適化するネットワーク。
 * ビジネスポリシーや時間帯などによって自動変更するネットワーク。
 * 問題が発生しても自己修復するネットワーク。

本書はこの OpenFlow の説明から始まり、OpenFlow でネットワークを「ハック」するための仕組みやポイントを解説します。「OpenFlow ってどんなもので、具体的に何に使えるんだろう？」そんな素朴な疑問に答えるために、OpenFlow のメリットや活用例について、自宅や職場のような中小規模ネットワークからデータセンターのような超大規模ネットワークまでを例にとり、なるべく具体例を見ながら理解を深めていただきます。また「さっそく OpenFlow を使って便利なツールを作ってみたい!」というプログラマ向けにも、実際に動かせる実用的なコードを通じて OpenFlow プログラミングを習得できる章を設けました。

本書を読み進めるにあたってネットワークやプログラミングの深い知識は必要ありません。基本からひとつひとつ説明しますので、ネットワークの専門家やプログラマはもちろん、システムエンジニアや技術営業、そして管理職などなど OpenFlow に興味を持つ方であれば誰でもすんなり理解できるように構成してあります。ではさっそく、OpenFlow の仕組みを理解しましょう。

== OpenFlow の仕組み

ここから 3 つほどちょっとした@<bou>{たとえ}話をします。みなさんもきっとお世話になったことがあると思いますが、よくあるカスタマーサポートを思い浮かべてください。そう、テレビとかパソコンの調子が悪くなったときに、フリーダイヤルで相談するアレです。「それって、どこが OpenFlow と関係あるの？」。実は OpenFlow の基本的な仕組みはカスタマーサポートにとても良く似ているのです。

これからお話しする 3 つのストーリーが理解できれば、OpenFlow の 95% を理解できたも同然です。それではさっそくこのストーリーの主人公の@<ruby>{友太郎,ゆうたろう}君と、カスタマーサポートセンターで働く青井さん、そして上司の宮坂主任の三人に登場してもらいましょう。
...
「はい、こちらヨーヨーダインエアコン・カスタマーサポートセンターです。担当はわたくし青井がうけたまわります。ご用件は何でしょうか？」。青井さんはヨーヨーダイン・エアコン社で働く電話オペレータです。お客さんから不具合の症状を聞き出し、問題を解決できる適切なエンジニアに電話をつなぐのが仕事です (@<img>{yoyodyne_support})。
...
青井さんは手元の対応マニュアルを調べます (@<table>{manual1})。対応マニュアルには故障の症状と、問題を解決できそうなエンジニアの内線番号が書かれています。続く数字はそれぞれの症状の問い合わせ件数です。

//table[manual1][電話オペレータ用対応マニュアル]{
エアコンの症状			内線番号	問い合わせ件数
----------------------------------------------
...
OpenFlow でこの上司にあたるのがコントローラと呼ばれるソフトウエアです (@<img>{openflow_host_switch_controller})。OpenFlow でネットワークをプログラミングする場合、プログラマが書くのはこのコントローラの部分です。頭脳であるコントローラをソフトウエアとして記述することで、ネットワークを自由自在に制御できるというわけです。
...
別のお客さんからのこんな問い合わせもあります。「30 年前に買った古いエアコンをずっと使ってるんだけど、本体のブザーがピーピーとが鳴り止まなくなってしまったんです」。

青井さんが対応マニュアルを見ると、ずっと下のほうに該当する項目がありました (@<table>{manual4})。ただし「有効期限 1995/1」という注意書きがあります。とても古いエアコンなので対応期間が終わってしまっているようです。

//table[manual4][ブザーの問題はすでに有効期限切れ]{
エアコンの症状			内線番号		問い合わせ件数	有効期限
--------------------------------------------------------------------------------
温度表示が点滅する		555-2222	9 件
風が出ない				555-4444	6 件
風向きが変更できない	555-3333	4 件
...
このようにマニュアルの項目によっては有効期限が決まっており、この期限を過ぎたものについては対応できません。項目が増えすぎるのも困るので、こういう古い項目はマニュアルから消しておいたほうがいいでしょう。

ただし、消した項目の内容と問い合わせ件数はかならず上司に報告することになっています。こうした情報を集計しておけばカスタマーサポートの改善に役立つからです。たとえば問い合わせの傾向から、どこにエンジニアを集中して配置すればよいかなどが分かります。

もちろん、もし同じような問い合わせがまた来るようであればマニュアルに項目を追加しなおしてきちんと対応する必要があります。この場合は有効期限を再設定します。

==== これを OpenFlow に置換えると...

フローに有効期限を指定することで、必要が無くなったフローを自動的に削除できます。フローを消す理由は、フローが増えすぎるとフローテーブルがあふれてしまうからです@<fn>{flowtable}。フローが消えると、消されたフローとその統計情報 (フローに従って転送したパケット数など) がスイッチからコントローラへと送られます。

//footnote[flowtable][フローテーブルにはフローをどれだけ入れられるかという容量制限があります。高いスイッチほどこの容量が大きくなります。]

コントローラで集計したフローの統計情報はいろいろなことに活用できます。たとえばあるアプリケーションが発生するネットワークトラフィックを調べたり、ネットワーク全体のトラフィックを集計するといったことがコントローラ側で簡単にできます。
...
同様に、OpenFlow スイッチの制御はすべてソフトウエアであるコントローラで実現しているのでネットワーク管理の自動化が容易です。さらにコントローラが Ruby や Python、Java などよく知られた汎用言語で書いてあれば、既存のシステムやサービスなどとの連携も簡単です。さらに自動化とシステム連携を組み合わせれば、アプリケーションからの要求やビジネスポリシーの変更、問題発生などさまざまなトリガーに応じてネットワークの設定を変更するといった、一歩進んだ自動化もできます。
...
OpenFlow でもすべてのトラフィック情報はコントローラに上がってくるため、全体を見たトラフィックの最適化が可能です。フローの有効期間が切れたときにコントローラに上がる統計情報を集計することで、コントローラはネットワーク全体のトラフィックデータを集められます。そしてその情報をもとに各スイッチのフローテーブルを更新することで、全体的に見て最適となるパケット転送経路を設定できます。反対にもし個々のスイッチが判断してしまうと、うまくトラフィックを分散できません。
...
また、ポート数密度や性能を気にしなければ、OpenFlow スイッチをソフトウエアで実現するという選択肢もあります。実際すでに Open vSwitch@<fn>{openvswitch} をはじめとしたさまざまなソフトウエア実装がそろっています。
...
そこで、OpenFlow の使いどころやフローテーブルの残り容量には特に注意する必要があります。たとえば OpenFlow をインターネットのような多種多様のパケットが流れる環境につなげると、すぐにコントローラへの問い合わせが殺到しフローテーブルがいっぱいになって破綻してしまいます。しかしデータセンターなどの閉じた環境では、トラフィックの特徴や流れるパケットの種類はあらかじめ見当を付けておけます。そこで最低限のパケットのみがコントローラへ上がってくるようにうまくフローとネットワークを設計することで、スイッチが増えてもスケールさせることが可能です。

== まとめ

本章では SDN を実現するための部品である OpenFlow の仕組みを説明しました。OpenFlow はネットワークをソフトウエアとして記述できる技術です。ネットワークをソフトウエア化することによって、自動化やさまざまなシステムとの連携、また汎用ハードウエアを使うことによるコストダウンなど種々の恩恵があります。
