...
「横着」はハッカーが最も創造性を発揮する分野の一つです。たとえば、ハッカーの巣窟として有名な MIT の AI ラボにはかつて、端末からオンラインでピザを注文できる UNIX コマンドが存在しました@<fn>{xpizza}。また、RFC で標準化されているコーヒーポットプロトコルでは、遠隔地にあるコーヒーポットのコーヒーの量を監視したり、コーヒーを自動的にいれたりするための半分冗談のインターフェースを定義しています@<fn>{rfc2325}。
...
その中でもネットワークをハックする技術の 1 つが、本書で取り上げる OpenFlow です。OpenFlow を一言で言うと、ネットワークスイッチをソフトウエア制御する仕様の一つです。OpenFlow 仕様はネットワークスイッチの内部動作を変更するプロトコルを規定しており、このプロトコルを使ってスイッチを制御するソフトウエアを書くことによってネットワーク全体をソフトウェア制御できます。これを Software Defined Networking (SDN) と呼びます。

OpenFlow の登場によって、これからはネットワークもプログラミングの対象になります。つまり今までは専門のオペレータ達が手作業で管理していたネットワークが、ついに無精なハッカー達の手にも渡るのです。「今までネットワーク管理って手作業でやってたの？ そんなの横着しちゃおうよ!」。横着とは言い換えれば自動化による効率化です。ネットワーク制御をソフトウエアとして記述することにより、次のような「究極の自動化」も夢ではなくなります。
...
あらかじめ適切なマニュアルを作っておけば、カスタマーサポートセンターの業務はすべて電話オペレータが自動的にやってくれます。全体の監督は管理職で実務は電話オペレータ、というふうにきっちりと分業できるからです。そして電話オペレータが実務をやってくれている間、管理職は他の部署との協業や調整に集中できます。
