...
て有名な MIT の AI ラボにはかつて、UNIX のコマンド一発でピザを FAX 注文
する @<tt>{xpizza} コマンドが存在しました@<fn>{xpizza}。また、RFC 2325
として公開されているコーヒーポットプロトコルでは、遠隔地にあるコーヒー
ポットのコーヒーの量を監視したり、コーヒーを自動的に淹れたりするための
半分冗談のインタフェースを定義しています。
...
本書では、この OpenFlow プロトコルの説明から始まり、OpenFlow でネットワー
クを「ハック」するための仕組みやプログラミング手法を紹介します。自宅や
職場のような中小規模ネットワークから、データセンターのような超大規模ネッ
トワークまで、OpenFlow の活用例やメリットを具体的に紹介します。また実際
に動かせる実用的なコードを通じて、「OpenFlow ってどんなもので、具体的に
何に使えるの？」というよくある疑問にも答えていきます。OpenFlow やネット
ワークの基礎、プログラミングの基礎から説明しますので、ネットワークの専
門家はもちろん、普通の SE や技術営業、管理職、プログラマなどなど
OpenFlow に興味を持つ方であればすんなり理解できるように構成してあります。
ではさっそく、OpenFlow の基本としてその動作モデルを理解しましょう。
