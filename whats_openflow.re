...
ソフトウェアによる横着はハッカーが最も創造性を発揮する分野の一つです。次の3つの話は、どれもハッカーの横着文化と技術力の高さをよく表しています。
...
その中でもネットワークをハックする技術の1つが、本書で取り上げるOpenFlowです。簡単に言えば、OpenFlowはネットワークスイッチの動作を自由にいじるためのプロトコルの一つです。スイッチひとつひとつの動作をプロトコルを通じて書き換えられれば、究極的にはネットワーク全体を自由に制御できます。こうしたSoftware Defined Networking(SDN、ソフトウェアで定義されるネットワーク)を代表する野心的な技術がOpenFlowなのです。
