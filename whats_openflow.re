...
優れたプログラマが持つハッカー気質のひとつに「無精」があります。大好きなコンピュータの前から一時も離れずにどうやってジャンクフードを手に入れるか……普通の人からするとただの横着に見えるかもしれませんが、ハッカー達にとってそれはいつでも大きな問題でした。

「横着」はハッカーが最も創造性を発揮する分野の一つです。たとえば、ハッカーの巣窟として有名な MIT の AI ラボにはかつて、自分の端末からオンラインでピザを注文できる UNIX コマンドが存在しました@<fn>{xpizza}。また、RFC で標準化されているコーヒーポットプロトコルでは、遠隔地にあるコーヒーポットのコーヒーの量を監視したり、コーヒーを自動的に淹れたりするための半分冗談のインタフェースを定義しています@<fn>{rfc2325}。
...
こうした「ソフトウェアで楽をする」ハックのうち、もっとも大規模な例が最新鋭のデータセンターです。クラウドサービスの裏で動くデータセンターは極めて少人数の管理者によって運用されており、大部分の管理はソフトウェアによって極限まで自動化されているという記事を読んだことがある人も多いでしょう。このようにピザやコーヒーのようなお遊びから、データセンターのように一筋縄ではいかない相手まで、プログラムで「モノ」を思いどおりにコントロールするのはもっとも楽しく、そして実際に役に立つハックの一種です。

== SDN: ネットワークをソフトウェアで制御しよう

その中でもネットワークをハックする技術の 1 つが、本書で取り上げる OpenFlow です。OpenFlow を一言で言うと、ネットワークスイッチをソフトウェア制御する仕様の一つです。OpenFlow 仕様はネットワークスイッチの内部動作を変更するプロトコルを定義しており、スイッチを制御するソフトウェアを書くことによってネットワーク全体をプログラム制御できる世界、つまり Software Defined Network (SDN) を目指しています。

OpenFlow の登場によってこれからはネットワークもプログラミングの対象になります。つまり今までは専門のオペレータ達が手作業で管理していたネットワークが、ついにプログラマに開放されるのです。ソフトウェアによる自動化はプログラマの最も得意とするところですから、ネットワーク制御をソフトウェアとして記述することにより、次のような「究極の自動化」も夢ではなくなります。

 * アプリケーションに合わせて勝手に最適化するネットワーク。
 * ビジネスポリシーや時間帯などによって自動変更するネットワーク。
 * 障害が起こっても自己修復するネットワーク。

本書はこの OpenFlow の説明から始まり、OpenFlow でネットワークを「ハック」するための仕組みやポイントを解説します。「OpenFlow ってどんなもので、具体的に何に使えるんだろう？」そんな素朴な疑問に答えるために、OpenFlow のメリットや活用例について、自宅や職場のような中小規模ネットワークからデータセンターのような超大規模ネットワークまでを例にとり、なるべく具体例を見ながら理解を深めていただきます。また「さっそく OpenFlow で素敵なサービスを作ってみたい!」というプログラマ向けに、実際に動かせる実用的なコードを通じて OpenFlow プログラミングを習得できる章も設けました。

本書を読み進めるにあたってネットワークやプログラミングの知識は必要ありません。ネットワークの基礎やプログラミング言語の基本からひとつひとつ説明しますので、ネットワークの専門家はプログラマはもちろん、普通の SE や技術営業、管理職などなど OpenFlow に興味を持つ方であれば誰でもすんなり理解できるように構成してあります。ではさっそく、OpenFlow の仕組みを理解しましょう。
...
これからお話しする 3 つのストーリーが理解できれば、OpenFlow の 95% を理解できたも同然です。それではさっそくこのストーリーの主人公の友太郎君と、カスタマーサポートセンターで働く葵さん、そして上司の宮坂主任の三人に登場してもらいましょう。

=== ストーリー 1: エアコンが壊れた!?

今年もエアコンの活躍する季節がやってきました。ところが友太郎君のエアコンはどうも調子が悪く、取扱説明書に載っていたカスタマーサポートに問い合わせることにしました。自動音声に従ってあれこれと試してみましたがどうも埒が明きません。結局、電話オペレータが出てきました。

「はい、こちらヨーヨーダインエアコン・カスタマーサポートセンターです。担当はわたくし葵がうけたまわります。お客様のご用件は何でしょうか？」。葵さんはヨーヨーダイン・エアコン社で働く電話オペレータです。お客さんから不具合の症状を聞き出し、問題を解決できる適切なエンジニアに電話をつなぐのが仕事です。
...
マニュアルの先頭に探していた症状がみつかりました。どうやらこれはよく知られた問題のようです。「ご不便をおかけしました。担当のエンジニアにただいまお繋ぎいたします」。電話を転送してもらった友太郎君はエンジニアと話をし、無事に問題を解決できました。
...
OpenFlow スイッチが普通のスイッチと違うのは、その動作が「マニュアル化」されている点です。カスタマーサポートの例では、葵さんはマニュアルを参照して転送先の番号を調べました。OpenFlow スイッチは、パケットの転送先をフローテーブルと呼ばれるデータベースを参照して決めます。葵さんの業務がすべてマニュアル化されているのと同じく、OpenFlow スイッチの制御はすべてこのフローテーブルの中身のみによって決まります。
...
: マッチングルール
  マッチングルールは届いたパケットをどう処理するかをフローテーブルの中から探す「キー」として使われます。たとえば「温度表示が点滅する」という症状から転送先を決めたように、「パケットを送信したホストの IP アドレスが 192.168.1.100 である」といったマッチングルールからパケットの転送先を決めます。
...
いかがでしょうか？カスタマーサポートの例と OpenFlow は驚くほどよく似ていることがわかると思います。実は OpenFlow はとても単純で理解しやすい仕組みなのです。

=== ストーリー 2: またリモコンが不調!?

しばらくは友太郎君のエアコンも順調でしたが、一ヶ月後また調子悪くなってしまいました。友太郎君はふたたびカスタマーサポートへダイヤルします。「リモコンの "暖房" と "冷房" がなぜか同時に表示されてるんです」。

葵さんはいつものように手元の対応マニュアルを調べましたが、困ったことにこの症状は載っていません。どうやらまったく新しい不具合のようです。でも、葵さんは慌てずに友太郎君に次のように伝えます「すみませんが少々お待ちください。多少お時間をいただきますが対応可能なエンジニアを探します」。そして電話口には録音された "しばらくお待ちください" のメッセージとどこか軽快な音楽が流れはじめました。

//image[yoyodyne_support_miyasaka][対応マニュアルに対処法が見つからなかった場合、上司に聞いてから回答][scale=0.3]

こういう時、葵さんがいつも頼るのは上司の宮坂主任です (@<img>{yoyodyne_support_miyasaka})。「宮坂さん、こんな症状の問い合わせがあったんですが誰につなげばいいかわかりますか?」宮坂主任はこの手のことはすべて頭の中に入っています。「それだったら青木君が適任だナ」。症状を聞くやいなや内線番号を葵さんに渡してくれました。

転送先が分かった葵さんは友太郎君の待つ電話に戻ります。「大変お待たせいたしました。担当の者にお電話を転送いたします」。一度目の問い合わせと比べてかなり時間がかかってしまいましたが、これでようやく一件落着です。

葵さんは、宮坂主任から教わった青木君の内線番号をマニュアルに追加しておきます。次からの同じ問い合わせにすばやく答えられるようにするためです。
...
A. それをやると信じられないくらい遅くなります。試しに OpenFlow コントローラの開発フレームワークである Trema (本書では全体を通じて Trema を使います) を使って簡単に実験してみたところ、ソフトウェアスイッチだけで転送する場合とすべてコントローラで処理する場合を比べると、性能が 5 倍も遅くなってしまいました。もちろんこれはいい加減な実験ですが、数倍遅くなるという目安にはなります。また今回の実験ではソフトウェアスイッチを使ったのですが、ハードウェアスイッチを使うとこの差はさらに広がります。

===[/column]

=== ストーリー 3: 古いエアコンが故障

ある日、別のお客さんからこんな問い合わせがありました。「30 年前に買ったエアコンを使っているんですが、ピーピーとブザーが鳴り止まなくなってしまったんです」

葵さんが対応マニュアルを見ると、下のほうに該当する項目がありました。ただし「有効期限 1995/1」という注意書きがあります。何か問題が起こっていることは確かなのですが、なにせとても古いエアコンなので対応できるエンジニアがいないようです。
...
このようにマニュアルの項目によっては有効期限が決まっており、期限を過ぎたものは対処できません。こういう古い項目はマニュアルから消しておいたほうがいいでしょう。もちろん、消した項目は上司に報告することになっています。こういう情報はいつ役に立つかわからないからです。
...
ただし、もし同じ問合せが来たときに上司に問い合わせしていてはお客様を待たせることになります。エントリを消すかわりに@<table>{manual6}のように内線番号を空欄にしておくことで「対応できるエンジニアはいない」とはっきりさせておいても良いでしょう。
...
同様に、OpenFlow スイッチの制御はすべてソフトウェアであるコントローラで実現しているので自動化が容易です。さらにコントローラが Ruby や Python、Java などよく知られた汎用言語で書いてあれば、既存のシステムやサービスなどとの連携も簡単です。

=== トラフィックエンジニアリングしやすい

問合せ件数の情報はすべて管理職に上がってくるため、問合せの混み具合の把握や交通整理が楽です。もし特定のエンジニアに問合せが集中したりエンジニアの異動が起こっても、電話オペレータに配布するマニュアルを更新するだけで対応できます。
...
また、ポート数密度や性能を気にしなければ、OpenFlow スイッチをソフトウェアで実現するという選択肢もあります。実際すでに Open vSwitch@<fn>{openvswitch} をはじめとして様々なソフトウェア実装が揃っています。
...
そこで、OpenFlow の使いどころやフローテーブルの設計には特に注意する必要があります。たとえば OpenFlow をインターネットのような多種多様のパケットが流れる環境につなげると、すぐにコントローラへの問合せが殺到しフローテーブルがいっぱいになって破綻してしまいます。しかしデータセンターなどの閉じた環境では、トラフィックの特徴や流れるパケットの種類はあらかじめ見当を付けておくことができます。最低限のパケットのみがコントローラへ上がってくるようにうまくフローとネットワークを設計することで、スイッチが増えてもスケールさせることが可能です。

== まとめ

本章では SDN を実現するための部品である OpenFlow の仕組みを説明しました。OpenFlow はネットワークをソフトウェアとして記述できる技術です。ネットワークをソフトウェア化することによって、自動化やさまざまなシステムとの連携、また汎用ハードウェアを使うことによるコストダウンなどさまざまな恩恵があります。

ソフトウェア化による恩恵にはこの他に、ソフトウェア業界で長年培われている様々なテクニックをそのままネットワークに適用できるという面もあります。たとえばソフトウェアテストを書くことでネットワーク全体を自動的にテストすることもできますし、ソフトウェアからドキュメントを自動生成し「ネットワークの仕様書」を作ることもできます。また、近年主流のアジャイル開発手法でコントローラを開発すれば、サービスインや新機能を追加するのに必要な期間を短くすることもできるでしょう。
