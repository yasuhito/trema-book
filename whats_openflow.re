...
OpenFlow の登場によって、これからはネットワークもプログラミングの対象になります。つまり今までは専門のオペレータ達が手作業で管理していたネットワークが、ついにプログラマ達にも開放されるのです。ソフトウエアによる自動化はプログラマの最も得意とするところですから、ネットワーク制御をソフトウエアとして記述することにより、次のような「究極の自動化」も夢ではなくなります。
