...
その中でもネットワークをハックする技術の 1 つが、本書で取り上げる OpenFlow です。OpenFlow を一言で言うと、ネットワークスイッチをソフトウエア制御する仕様の一つです。OpenFlow 仕様はネットワークスイッチの内部動作を変更するプロトコルを定義しており、スイッチを制御するソフトウェアを書くことによってネットワーク全体をプログラム制御できる世界、つまり Software Defined Network (SDN) を目指しています。

OpenFlow の登場によってこれからはネットワークもプログラミングの対象になります。つまり今までは専門のオペレータ達が手作業で管理していたネットワークが、ついにプログラマに開放されるのです。ソフトウエアによる自動化はプログラマの最も得意とするところですから、ネットワーク制御をソフトウェアとして記述することにより、次のような「究極の自動化」も夢ではなくなります。
...
葵さんはマニュアルに載っていない問い合わせを上司の宮坂主任に問い合わせました。OpenFlow でこの上司にあたるのがコントローラと呼ばれるソフトウエアです。
...
A. それをやると信じられないくらい遅くなります。試しに OpenFlow コントローラの開発フレームワークである Trema (本書では全体を通じて Trema を使います) を使って簡単に実験してみたところ、ソフトウエアスイッチだけで転送する場合とすべてコントローラで処理する場合を比べると、性能が 5 倍も遅くなってしまいました。もちろんこれはいい加減な実験ですが、数倍遅くなるという目安にはなります。また今回の実験ではソフトウェアスイッチを使ったのですが、ハードウェアスイッチを使うとこの差はさらに広がります。
