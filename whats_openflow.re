...
//lead{
ネットニュースをにぎわせている OpenFlow って何だろう？どんな仕組みで一体何がうれしいの？難しいネットワーク用語を使わなくても、身近なたとえ話ですっきりと理解できます。
//}
...
今年もエアコンの活躍する季節がやってきました。ところが友太郎君のエアコンはどうにも調子が良くありません。そこで取扱説明書に載っていたカスタマーサポートに電話することにしました。自動音声に従って問題ありそうな項目をすべてチェックしてみましたが、いっこうに解決しません。結局、自動音声ではなく電話オペレータに相談することになりました。
...
フローに有効期限を指定することで、必要が無くなったフローを自動的に削除できます。フローを消す理由は、フローが増えすぎるとフローテーブルがあふれてしまうからです。フローが消えると、消されたフローとその統計情報 (フローに従って転送したパケット数など) がスイッチからコントローラへと送られます。
