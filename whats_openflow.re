...
ソフトウェアによる横着はハッカーが最も創造性を発揮する分野の一つです。次の 3 つの有名なハックは、どれもハッカーの横着文化と技術力の高さをよく表しています。
 ...
 2. コンピュータサイエンスの名門、カーネギーメロン大学にはコーク・マシンという変わったコーラ自販機があり、UNIX コマンド一発でコーラの冷え具合を確認できるようになっています@<fn>{coke_machine}。遠い自販機まで行って@<bou>{ぬるい}コーラをつかまされないようにする工夫です。
...
その中でもネットワークをハックする技術の 1 つが、本書で取り上げる OpenFlow です。OpenFlow を一言で言うと、ネットワークスイッチをソフトウェア制御する仕様の一つです。簡単に言えば、OpenFlow はネットワークスイッチの中身をいじるための方式の一つで、ソフトウェアでスイッチ内部の動作を自由に変更できるようにするためのものです。Software Defined Networking (SDN) という用語を聞いたことがある人もいると思いますが、OpenFlow はネットワークをソフトウェアで書いてしまおうという野心的な技術なのです。

OpenFlow の登場によって、これからはネットワークもプログラミングの対象になります。つまり今までは専門のオペレータ達が手作業で管理していたネットワークが、ついに無精なハッカー達の手にも渡るのです。「今までネットワークって手で管理してたの？ そんなの横着しちゃおうよ!」。横着とは言い換えれば自動化による効率化です。ネットワークをソフトウェアとして書くことにより、次のような「究極の自動化」も夢ではなくなります。
