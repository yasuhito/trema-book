...
ところでマニュアルを見ると、よく似た症状がいくつか登録されています。1 番の「温度表示が点滅する」と、2 番の「風速表示が点滅する」、3 番の「表示がおかしい」がそれです。もし「なんだか表示が変なんですけど」のようなあいまいな問い合わせが来たときには、どの内線番号に転送すればよいでしょうか？

このマニュアルの読み方として、「症状は 1 番から順に確認する」というルールがあります。たとえばお客さんの中には「なんだか表示がチカチカする」といったように症状を正確に説明できない人も少なくありません。そこでどうおかしいのかをハッキリさせるために、まずは「チカチカしているのは温度表示の部分ですか？」という 1 番の質問をします。もしそうであれば 1 番の番号に転送です。そうでなかった場合は 2 番の「チカチカするのは風速の部分ですか？」という質問です。どれもあてはまらなかった場合は、液晶自体が故障している可能性があるので 3 番です。
...
入ってきたパケットはフローテーブル内のマッチングルールと比較されますが、その順序にはカスタマーサポートと同様の簡単なルールがあります。まず最初に、あいまいさの無いマッチングルール、つまりマッチングルールで使用できる 12 種類のフィールドがすべて指定されているフロー (Exact Match) が最優先で比較されます。もし、パケットがどの Exact Match にもあてはまらなかった場合はその他のフローが比較されます。その際の順序はフローの優先度の大きい順です。

たとえば@<table>{story1_openflow} のマッチングルールではどれも 1 種類のフィールドしか指定していないため、優先度の大きい順にチェックされます。表には示していませんが 12 種類のパラメータすべてを指定したマッチングルールがあれば、これが最も最初にチェックされます。
