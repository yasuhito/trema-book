...
これはハッカーが最も創造性を発揮する分野の一つです。たとえば、ハッカーの巣窟として有名な MIT の AI ラボにはかつて、自分の端末からオンラインでピザを注文できる UNIX コマンドが存在しました@<fn>{xpizza}。また、RFC で標準化されているコーヒーポットプロトコルでは、遠隔地にあるコーヒーポットのコーヒーの量を監視したり、コーヒーを自動的に淹れたりするための半分冗談のインタフェースを定義しています@<fn>{rfc2325}。
...
風が出ない					555-4444		6 件
風向きが変更できない		555-3333		4 件
@<em>{暖房が同時に表示}		@<em>{555-5555}	@<em>{1 件}
//}
...
また、ポート数密度や性能を気にしなければ、OpenFlow スイッチをソフトウェアで実現するという選択肢もあります。実際すでに Open vSwitch などいくつかのソフトウェア実装が揃っています。
...
電話オペレータが参照するマニュアルは、管理職の指示によって担当部署が一括して作成し配布します。このため、エンジニアの異動など転送先の変更はマニュアルを更新するだけで対応できます。また、問合せ件数などのデータをすべて管理職に集めるようにしておくことで、どんな問い合わせが多いかまたはどのエンジニアに問合せが集中しているか、といった調査も簡単です。

同様に、OpenFlow スイッチが参照するフローテーブルはひとつのコントローラが管理します。このため、パケットの転送制御はフローテーブル内のフローを更新するだけで変更できます。また、フローの有効期間が切れたときにコントローラに上がる統計情報を集計することで、トラフィックエンジニアリングに役立つデータを簡単に集めることができます。
