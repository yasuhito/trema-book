...
これからお話しする 3 つのストーリーが理解できれば、OpenFlow の 95% を理解できたも同然です。それではさっそくこのストーリーの主人公の友太郎君と、カスタマーサポートセンターで働く@<ruby>{葵,あおい}さん、そして上司の宮坂主任の三人に登場してもらいましょう。
...
「はい、こちらヨーヨーダインエアコン・カスタマーサポートセンターです。担当はわたくし葵がうけたまわります。お客様のご用件は何でしょうか？」。葵さんはヨーヨーダイン・エアコン社で働く電話オペレータです。お客さんから不具合の症状を聞き出し、問題を解決できる適切なエンジニアに電話をつなぐのが仕事です (@<img>{yoyodyne_support})。
...
葵さんは手元の対応マニュアルを調べます (@<table>{manual1})。対応マニュアルには故障時の症状と、問題を解決できそうなエンジニアの内線番号が書かれています。続く数字はそれぞれの症状の問い合わせ件数です。
...
ただし、もし同じ問い合わせが来たときに上司に問い合わせしていてはお客様を待たせることになります。エントリを消すかわりに@<table>{manual6}のように内線番号を空欄にしておくことで「対応できるエンジニアはいない」とはっきりさせておいても良いでしょう。
