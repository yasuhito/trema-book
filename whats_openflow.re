...
ここから 5, 6 分ほど、ちょっとした例え話をします。みなさんもきっとお世話になったことがあると思いますが、よくあるカスタマーサポートを思い浮かべてください。そう、テレビとかパソコンの調子が悪くなったときに、フリーダイヤルで相談するアレです。「それって、どこが OpenFlow と関係あるの？」。実は OpenFlow の基本的な仕組みはカスタマーサポートにとても良く似ているのです。
...
番号	症状				内線番号	問合せ件数
------------------------------------------------------------------
1	温度表示が点滅する		555-2222-2222	8 件
2	表示が点滅する			555-1111-1111	14 件
...
ところでマニュアルを見ると、よく似た症状が登録されています。1 番の「温度表示がに点滅する」と、2 番の「表示が点滅する」がそれです。もし「なんだか表示がチカチカしてるんですけど」のようなあいまいな問い合わせが来たときには、どちらの内線番号に転送すればよいでしょうか？

このマニュアルの読み方として、「症状は 1 番から順に確認する」というルールがあります。たとえばお客さんの中には「なんだか表示がチカチカする」といったように症状を正確に説明できない人も少なくありません。そこでどうおかしいのかをハッキリさせるために、まずは「チカチカしているのは温度表示の部分だけですか？」という 1 番の質問をします。もしそうであれば 1 番の番号に転送です。そうでなかった場合は 2 番です。このように、

 * マニュアルにはあいまいさの少ない順に症状を並べる
 * オペレータは 1 番から順に症状をチェックする

//noindent
という単純なルールによって、正しい担当エンジニアに転送できるようにしているのです。
...
こういう時、葵さんがいつも頼るのは上司の宮坂主任です (@<img>{yoyodyne_support_miyasaka})。「宮坂さん、こんな症状のお問い合わせがあったんですが誰につなげばいいかわかりますか?」宮坂主任はエアコンの事なら何でも知っています。症状を聞くやいなや、適切な技術者の内線番号を葵さんに渡します。
...
3	風向きが変更できない		555-3333-3333	4 件
4	冷房と暖房が同時に表示		555-4444-4444	1 件
//}
...
3	風向きが変更できない		555-3333-3333	4 件
4	冷房と暖房が同時に表示		555-4444-4444	1 件
5	電源ランプが点かない		555-5555-5555	25 件		2008/1
//}
