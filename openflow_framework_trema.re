...
//image[trema_logo][Trema のロゴ][scale=0.7]
   ...
   ん。この本の編集者が画像編集ソフトで作ったものを気に入った作者達が、
   そのまま公式ロゴとして使っています。
...
Ruby を習得する一番の近道は、外国語の学習と同じく登場する品詞の種類を押
さえておくことです。Ruby の品詞には、たいがい見分けるための手がかりとな
る視覚的なヒントがあります。記号が使われていたり、最初の文字が大文字に
なっていたりするので、断片的なコードを見てもすぐにどんな品詞かわかります。

それぞれの品詞の見かけに注意してください。それぞれの品詞について簡単に
説明しますが、別に説明が全部理解できなくとも構いません。この節の終わる
頃には Ruby プログラムのあらゆる部分が識別できるようになっているはずです。
...
英語でも固有名詞は大文字で始めることになっています。Tokyo Tower (東京タ
ワー) みたいに。東京タワーは動かすことができません。東京タワーを何か別
なもののことだと勝手に決めることもできません。固有名詞というのはそうい
うものです。固有名詞はある特定の、通常は時間とともに変化しないものを指
しています。固有名詞と同様、Ruby の定数は一度セットすると変更できません。
定数の値を変えようとすると、Ruby は文句を言ってきます。
...
"Hello Trema!" の中身を説明しましょう。

=== コントローラクラスの定義

すべてのコントローラはクラスとして定義され、Trema の @<tt>{Controller}
クラスを継承します (@<list>{コントローラクラス定義}の 1 行目)。
@<tt>{Controller} クラスを継承するには、@<tt>{class [クラス名] < Controller} と書きます。
...
であらかじめ定義された各種イベントに対応するハンドラを定義しておくと、
イベントが発生したときに対応するハンドラが自動的に呼び出されます。たと
えば @<tt>{start} メソッド (@<list>{ハンドラ定義}の 2〜4 行目) を定義し
ておくと、コントローラの起動時にこれが Trema によって自動的に呼ばれます。
...
これで "Hello Trema!" の説明はおしまいです。このように、Trema のコント
ローラはクラスにいくつかハンドラメソッドを定義したもの、とまとめること
ができます。
