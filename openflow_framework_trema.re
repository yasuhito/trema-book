...
Trema が初めてという人のなかには、Ruby も初めてという人も多くいます。外国語の習得にも言えることですが、Ruby を習得する一番の近道は登場する品詞の種類を押さえておくことです。Ruby に出てくる名前 (構成要素) には、その品詞を見分けるための手がかりとなる視覚的なヒントがかならずあります。記号が使われていたり、最初の文字が大文字になっていたりするので、断片的なコードを見てもすぐにどんな品詞かわかります。品詞がわかれば、その Ruby コードがどんな構造かわかります。それぞれの品詞の見かけに注意してください。これからそれぞれの品詞について簡単に説明しますが、最初からすべてが理解できなくとも構いません。この節の終わる頃には "Hello, Trema!" プログラムのあらゆる部分が識別できるようになっているはずです。
...
"Hello Trema!" の例では @<tt>{class} キーワードに続く @<tt>{HelloTrema} と、@<tt>{Controller} がそれぞれ大文字で始まるので定数です。
...
勘のいい人ならすでに気付いていると思いますが、@<tt>{class} キーワードに続く定数から @<tt>{end} までで定義されるブロックがクラス定義です。Trema ではすべてのコントローラはクラスとして定義され、かならず Trema の @<tt>{Controller} クラスを継承します。クラスを継承するには、@<tt>{class クラス名 < 親クラス名} と書きます。
...
さて、こうして定義された @<tt>{HelloTrema} はどこから実行が始まるのでしょうか？ C で言う @<tt>{main()} 関数に当たるものがどこにも見あたらない気がします。その答は Trema の動作モデルであるイベントドリブンモデルにあります。Trema のコントローラは、Trema が定義する様々なイベントに反応するイベントハンドラの集合として定義できます。それぞれのイベントハンドラは、対応するイベントが発生したときに Trema によって自動的に呼び出されます。たとえば OpenFlow メッセージの到着イベントが発生したとき、もしそのメッセージに対応するハンドラメソッドがコントローラクラスに定義されていれば、Trema がそのメソッドを発見して呼んでくれます。Trema でよく使われる主なイベントをリストアップしておきましょう。
...
ハンドラメソッドの定義は、@<tt>{def} キーワードに続く名前から @<tt>{end} までで定義されるブロックです。たとえば @<tt>{HelloTrema} の例では @<tt>{start} ハンドラメソッドを定義しており、これがコントローラの起動イベント発生時、つまり @<tt>{trema run} でコントローラを起動したときに自動的に呼ばれます。@<tt>{start} ハンドラメソッド中の @<tt>{puts} は Ruby 組込みのメソッドで、C の @<tt>{puts()} と同じく文字列を標準出力へ改行付きで出力します。
...
プログラミング経験の長い読者の中には、「ハンドラメソッドを定義しただけなのに、なぜ Trema はこのメソッドを自動的にみつけられるんだろう」と不思議に思った人がおるかもしれん。確かに C などの古い言語ではコンパイル時と実行時の間にはぶ厚いカーテンが引かれており、プログラム中にどういう関数があるか (= コンパイル時情報) をプログラム自身が (= 実行時に) 取ることはむずかしい。

いっぽう、Ruby にはイントロスペクション (リフレクションや自己反映計算とも呼ぶ) と呼ばれる機能があり、クラスが自らの持つメソッドを実行時に調べることができる。たとえば Packet In メッセージが到着したとき、コントローラクラスはイントロスペクションして自分が @<tt>{packet_in} というメソッドを持っているか実行時に調べる。もしみつかればこのメソッドを呼んであげるというわけじゃ。ちなみにこの機能は親の @<tt>{Controller} クラスによってコントローラクラスに自動的に導入される。

イントロスペクションは Ruby の他にも Python や Java といった最近の言語ではサポートされておるが、それらの言語を採用するほとんどの OpenFlow フレームワークでは明示的にハンドラメソッドとイベントを結びつけなければならないようじゃ。たとえば NOX (Python 版) の場合はこう書く必要がある。
...
一方で Trema がハンドラメソッドを自動呼び出しにした理由は、それが大幅にコード量を削減してくれるからじゃ。短いコードは書きやすく、読みやすく、またメンテしやすいものじゃ。たとえば上の NOX の例でハンドラメソッドの名前を変更した場合、@<tt>{register_for_packet_in} の引数も忘れずに変更する必要がある。Trema のようにハンドラメソッドの命名規約を決めておけば、こうした手間は発生せずメンテしやすいコードとなるのじゃ。
...
このしくみには、環境構築が手軽な CruiseControl.rb (@<tt>{http://cruisecontrolrb.thoughtworks.com/}) と自作プラグインを使っているぞ。

===[/column]

== サンプルアプリ

サンプルアプリ (@<tt>{[trema]/src/examples/}) は簡単な OpenFlow アプリケーションをたくさん含んでおり、実際の API の使い方を調べるのに便利です。以下におもなサンプルアプリをまとめます (括弧内は @<tt>{[trema]/src/examples/} 内のディレクトリ名)。簡単な順に上から並べていますので、この順に読んでいくことをおすすめします。またこのうちいくつかは続く章で詳しく説明していきます。
...
Trema にはたくさんの API があり、この表に掲載したサンプルではまだまだすべてを紹介しきれていません。新しいサンプルアプリを作った人は、ぜひ github で pull リクエストを送ってください。あなたの名前が Trema プロジェクトの貢献者リスト (@<tt>{https://github.com/trema/trema/graphs/contributors}) に載るかもしれません!
...
: Why's (Poignant) Guide to Ruby (@<tt>{http://mislav.uniqpath.com/poignant-guide/})
  私は大学や職場でいろいろなプログラミング言語を勉強してきましたが、これほど読んでいて楽しい本に出会ったことはありません。この本は Ruby 界の謎の人物 _why 氏による風変りな Ruby 入門本で、まるで小説やマンガのようにリラックスして読めます。この章の Ruby の品詞の説明は、この本を参考にしました。(日本語版はこちら @<tt>{http://www.aoky.net/articles/why_poignant_guide_to_ruby/})。
