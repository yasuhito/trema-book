...
第 II 部では、OpenFlow を使ってネットワークをプログラミングする方法を紹介します。職場や自宅のような中小規模ネットワークでもすぐに試せる実用的なコードを通じて、「OpenFlow って具体的に何に使えるの？」というよくある疑問に答えていきます。実例を混じえつつ OpenFlow やネットワークの基礎から説明しますので、ネットワークの専門家はもちろん普通のプログラマもすんなり理解できるはずです。
...
Trema の特長は Rails や Sinatra などの Web フレームワークに影響を受けた、アジャイルな OpenFlow プログラミングフレームワークであることです。Trema にはコントローラの開発サイクル全体、つまりコーディングや様々なレベルでのテスト、およびリファクタリングをサポートする機能が随所にちりばめられています。こうして開発全体の面倒を見るところが Trema の「フレームワーク」たるゆえんで、他の OpenFlow コントローラやライブラリとはまったく違うところです。第 II 部では、実際に Trema フレームワークを使って OpenFlow コントローラを作っていきます。それではまず Trema をセットアップしましょう。
...
A. 作者の一人が大好きな、大阪の「とれまレコード (@<tt>{http://www.fumiyatanaka.com/})」という小さいレコードレーベルの名前から取りました。この「とれまレコード」の "とれま" の由来ですが、日本がバブルの頃に道路標識の「とまれ」を「とれま」と誤植したケースが頻発したことが由来です。ちなみにこのありえない道路標識の原因は、バブル当時に急増した日本語の苦手な外国人労働者が間違えて「とれま」と道路に描いてしまったことにあるそうです。
...
もちろんこんなに大胆な道路標識は日本中どこを探してもありません。この本の編集者が試しに画像編集ソフトで作ったところ出来が良かったので、そのまま公式ロゴになりました。
