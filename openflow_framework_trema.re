...
第 II 部では、OpenFlow を使ってネットワークをプログラミングする方法を紹介します。職場や自宅のような中小規模ネットワークでもすぐに試せる実用的なコードを通じて、「OpenFlow コントローラってどう作るの？」「OpenFlow って具体的に何に使えるの？」というよくある疑問に答えていきます。題材はなるべく実例を取り入れるようにし、また OpenFlow やネットワークの基礎から説明していくようにしましたので、ネットワークの専門家はもちろん普通のプログラマにもすんなり理解できる内容となっているはずです。
...
@<tt>{sudo} ができることを確認したら、Trema のインストールに必要なパッケージ管理ツール RubyGems と、コンパイルに必要な @<tt>{gcc} などの外部ソフトウェアを次のようにインストールします。
...
次に Trema 本体をインストールします。Trema は RubyGems を使って次のようにコマンド一発で簡単にインストールできます。
...
次のコマンドを実行すると、さきほどダウンロードしてできた @<tt>{trema/} ディレクトリに Trema がインストールされます。@<tt>{make install} のようなシステム全体へのインストールは不要であることに注意してください。

//cmd{
% ./trema/build.rb
//}

この場合、@<tt>{trema} コマンドは @<tt>{trema/} ディレクトリにインストールされます。次のコマンドで正しくインストールされたか確認してください。
