...
「分岐する」を使えば、サーバが送信したパケットを、スイッチがコピーして、複数のクライアントに届けることができます(@<img>{multicast})。この「分岐する」を使わなかった場合、サーバはクライアントそれぞれに対して、同じパケットを送らなければなりません。「分岐する」を使う場合、スイッチがパケットをコピーしてくれるので、クライアント数が増えても、サーバから送るパケットの数を増やす必要はありません。つまり、ネットワークの帯域を節約できます。

//image[multicast][パケットを複製し、複数のクライアントに届ける][scale=0.5]

「分岐する」で冗長な経路を作り、それぞれの経路に複製したパケットを流せば、ネットワークに障害が起こった場合でもパケットをロスせずに復旧できます。クライアントとサーバが通信する状況を考えてください (@<img>{fail_over})。一番左のスイッチは、複製したパケットを、2 つの経路それぞれに送ります。受け取る側は、冗長化された経路のうちどちらか一方だけから受け取るようにしておきます。このようにしておけば、もしどちらかの経路に障害が起こっても、もう一方の経路に切り替えれば、パケットを一つも落とさずに通信を続けられます。
