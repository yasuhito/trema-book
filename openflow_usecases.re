...
//image[switch][OpenFlow でスイッチを実現する][width=12cm]

これに「流量を調べる」を組み合わせると、スイッチにトラフィック集計機能を付け加えられます。フローごとにどれだけのパケットを転送したかを集計することで、コントローラでネットワーク全体のトラフィックを集計できます。

//image[traffic_switch][OpenFlow でトラフィック集計機能つきスイッチを実現する][width=12cm]
...
//image[router][OpenFlow でルータを実現する][width=12cm]
...
//image[load_balancer][OpenFlow でロードバランサを実現する][width=12cm]
...
//image[sharing_bandwidth][3 つのアプリで帯域をシェア][width=12cm]
