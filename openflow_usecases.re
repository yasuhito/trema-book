...
とにかく大量のサーバやスイッチを使うため、個々のハードウェアの信頼性はそれほど重視されません。Google データセンターのように数十万台のサーバがある環境では、そもそもすべてが故障無しに動き続けることはまったく期待できないからです。そのかわり、上のソフトウェア層で信頼性を保証します。死活監視や冗長化、障害からの自動復旧などはすべてデータセンタを管理するミドルウェアとして実装するのです。

OpenFlow はこのデータセンターのモデルにうまくマッチしています。OpenFlow スイッチのハードウェアは単純なので、量産や低コスト化が簡単です。OpenFlow コントローラは汎用サーバ上で動くので、ハイエンドなサーバーは必要ありません。ソフトウェアであるコントローラがネットワークすべてを制御するので、データセンターの制御ソフトウェア層との連携が簡単です。また、データセンターは OpenFlow のような新しい技術を採用しやすいプラットフォームです。インターネットに求められるような相互接続性は重要ではないため、独自の新技術の導入がやりやすいのです。
