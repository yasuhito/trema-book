...
OpenFlow には「ソフトウェアさえ書けば、ほぼどんなネットワーク機器にも化けられる」という特長があります。乱暴に言うと、コントローラを実装しさえすればルータやロードバランサ、ファイアウォールなど何でも作れてしまいます。もちろんこの場合、スイッチ機能の一部がソフトウェアになるので性能は落ちます。しかし、ほぼ何でもできることには変わりありません。
...
では、もともとはアカデミア発の OpenFlow が現在のように産業界で注目されるようになった理由は何でしょうか？@<chap>{whats_openflow}でも触れましたが、主な理由は次の 3 つです。
 ...
 2. OpenFlow で作ったネットワークは、既存のシステムとの連携がしやすくなります。たとえばデータセンターのジョブ管理やスケジューラ、または職場の業務システム等とコントローラをつなげば、既存システムと連携した高度なネットワーク制御のできあがりです。
 3. OpenFlow でしかできないこともあります。今までの分散型ネットワークでは苦手だった全体最適化が、OpenFlow の集中管理でできるようになるからです。ネットワーク全体を制御する神様であるコントローラが集中管理するおかげで、分散での局所最適化的な管理が全体最適化ができます。
