...
OpenFlow スイッチには「コントローラさえ書けばどんなネットワーク機器にも化けられる」という特長があります。乱暴に言うと、スイッチはもちろんルータやロードバランサ、ファイアウォールや NAT など複雑な機器も、コントローラの実装を頑張れば実現できてしまいます。もちろん、専用機と同じ機能をすべて実装するのは大変ですし、機能の一部をソフトウェアとして実装することになるので実装が悪いと性能は落ちます。しかし、ソフトウェア次第で何でもできる事に変わりはありません。
...
OpenFlow はハードウェアの利点とソフトウェアの利点をうまく組み合わせた方式です。機能の網羅性では専用機にはおよびませんが、必要な機能をソフトウェアで追加できるという柔軟性があります。また、パケットをハードウェア側のみで転送するようにうまくコントローラを実装すれば、普通のスイッチと変わらない性能を出すことだってできます。このため、実際のネットワークに接続してトラフィックを流し込むといった本格的な実験も可能です。
...
OpenFlow はこのデータセンターのモデルにうまくマッチしています。OpenFlow は制御を行うソフトウェアであるコントローラと、その命令に従うだけのハードウェアである OpenFlow スイッチにきれいに分離できます。これは、データセンターのモデル --- 全体を制御するミドルウェアと、大量のコモディティハードウェア --- と似ています。たとえば OpenFlow スイッチのハードウェアは単純なので、量産による低コスト化ができます。ネットワーク制御はソフトウェアとして実装するので、ネットワークの信頼性確保には既存のデータセンターのミドルウェア層を利用できます。

何よりも、データセンターは OpenFlow のような新しい技術を採用しやすいプラットフォームです。インターネットに求められるような相互接続性は重要ではないため、独自の新技術の導入がやりやすいのです。
...
この認証は次のように動作します。コントローラはホストからのパケットが到着すると、そのパケットが認証済ホストからのものかどうかの確認を認証サーバに依頼します。このとき、「書き換える」と「パケットを転送する」でパケットの宛先を書き換えて認証サーバに転送します。もしホストが認証できると、認証サーバはコントローラに認証できたホストを通知します。コントローラはフローを更新し、認証が済んだホストのパケットを認証済ネットワークに転送します。こうして、認証が済んだホストのみが認証済みネットワークに接続できるようになります。
...
ここまでの「帯域をめいっぱい使う」と「ネットワークを冗長化する」の具体的な例は、@<chap>{google}で紹介します。
