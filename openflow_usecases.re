...
OpenFlowはこのデータセンターのモデルにうまくマッチしています。OpenFlowは制御を行うソフトウェアであるコントローラと、その命令に従うだけのハードウェアであるOpenFlowスイッチにきれいに分離できます。これは、データセンターのモデル--全体を制御し信頼性を確保するミドルウェアと、大量のコモディティハードウェア--と似ています。たとえばOpenFlowスイッチのハードウェアは単純なので、量産による低コスト化ができます。ネットワーク制御はソフトウェアとして実装するので、ネットワークを利用するアプリケーションとの連携が容易です。ネットワークとアプリケーションがそれぞれ独立して信頼性を確保するよりも、無駄が少なく、かつ高い信頼性を実現できます。
...
OpenFlowがうまく行く具体的なユースケースを見てきました。OpenFlowではプログラミング次第で好きな機能を追加できますが、専用のネットワーク機器と同じ機能や性能を達成するのは現実的ではありません。そのかわりデータセンターに代表される、ソフトウェアでハードウェア全体を制御する世界とうまくマッチします。ネットワーク管理の自動化や最適化に必要な機能をOpenFlowで実装する、それが最も威力を発揮するOpenFlowの使い方です。
