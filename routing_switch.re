...
コントローラがスイッチ間のリンクを発見するにはどのようにすればよいでしょうか？例えば、@<img>{lldps0} のように、スイッチ 0x1 のポート 4 とスイッチ 0x2 のポート 1 が接続されていたとします。このリンクを発見するために、コントローラは以下の手順を行います。
...
あとは同様の手順を、ネットワーク中の各スイッチに対して順に行なっていきます。次は、スイッチ 2 に接続するリンクを調べてみましょう(@<img>{lldp2})。

//image[lldp2][スイッチ 0x2 のリンク発見][scale=0.4]

最後にスイッチ 0x3 に接続するリンクを同様な手順で見つければ、ネットワークの完全なトポロジー情報を取得できます。
