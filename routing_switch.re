...
ShowNet の魅力をもっともよく伝えてくれるアイテムが、Interop で毎年公開される ShowNet のトポロジ図です (@<img>{shownet_topology})。これの素晴らしいところは、ShowNet を構成するネットワーク機器同士の複雑な接続関係をたった1枚の図に収めているところです。「この部分は、いったいどんなプロトコルで動いているんだろう？」「実際の詳しいトポロジはどうなっているのかな？」ネットワークが好きな人であれば、気がつくと何時間でも眺めてしまうほどの魅力を持ったトポロジ図なのです。
...
ShowNet のようにいくつものスイッチやルータがつながるネットワークの動作では、トポロジ情報の把握が1つの鍵です。パケットが迷子になったりループしたりせずに正しく目的地まで届けられるためには、スイッチやルータ同士がどのような接続関係にあるかをお互いに把握しなければなりません。
