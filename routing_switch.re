...
//image[flow_mod][最短パスでパケットを転送するためにフローを書き込む][width=12cm]
...
ルーティングスイッチコントローラの詳しい動作は次のようになります:
 ...
 5. この最短パスに沿ってパケットが転送されるようにパス上のスイッチそれぞれに Flow Mod を送りフローを書き込む
...
//image[lldp_openflow][LLDP パケットと OpenFlow の仕組みを使ってリンクを発見する][width=12cm]
 ...
 4. LLDP パケットを受け取ったスイッチ 0x2 は、自身のフローテーブルを参照し、パケットの処理方法を調べる。このとき LLDP に対するフローはあえて設定していないため、今回受信した LLDP パケットは、Packet In としてコントローラまで戻される
...
//noindent
出力より、@<tt>{0x1} と @<tt>{0x2} のスイッチそれぞれに host2 → host1 のフローが設定されていることがわかります。
 ...
 * ネットワーク上の最短パスを計算する方法
 * LLDP でトポロジを検出する仕組みと Trema のネットワークエミュレータを使って、ルーティングスイッチの topology モジュールの動作
 * ルーティングスイッチの動作を参考に、OpenFlow によるスイッチ機能の利点
