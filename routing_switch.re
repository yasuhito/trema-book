...
ルーティングスイッチは、複数台の OpenFlow スイッチが連動して、一つの大きな L2 スイッチとして動作させるためのコントローラです (@<img>{routing_switch})。@<chap>{learning_switch} で扱ったラーニングスイッチも、普通の L2 スイッチとして動作するアプリケーションでした。ラーニングスイッチは 1 台の OpenFlow スイッチしか扱えませんでしたが、ルーティングスイッチを使うことで、複数台の OpenFlow スイッチを制御することができます。
