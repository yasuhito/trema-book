...
筆者は、ネットワークの仕事に十余年も関わっているためか、多数のスイッチ、ルータから構成される大規模なネットワークの構成図を見ると心が踊ります。``どんなプロトコルで動いているのか?'' を知りたくなったり、``いつか自分でも動かしてみたい!'' と思ったりします。

ここまでの章では一台の OpenFlow スイッチを制御する方法について学んできましたが、もちろん、OpenFlow は、複数台のスイッチからなるネットワークでも使えます。複数台の OpenFlow スイッチを制御するためには、スイッチ同士の接続情報 (トポロジー) を検出する必要があります。OpenFlow を使うことで、ネットワークのトポロジー構成を検出できます。検出したトポロジーを使うことで、ネットワーク上の最短パスでパケットを転送することができます。

本章では、一例として、ルーティングスイッチを紹介します。ルーティングスイッチが、複数の OpenFlow スイッチからなるネットワークの制御をどのように実現しているか見ていきましょう。

== 複数のスイッチを扱う

ルーティングスイッチは、複数台の OpenFlow スイッチを連動させ、一つの大きな L2 スイッチとして動作させるためのコントローラです (@<img>{routing_switch})。@<chap>{learning_switch} で扱ったラーニングスイッチも、普通の L2 スイッチ機能を実現するアプリケーションでした。ラーニングスイッチは OpenFlow スイッチを 1 台しか扱えませんでしたが、ルーティングスイッチを使うことで複数台の OpenFlow スイッチを制御することができます。

//image[routing_switch][複数台の OpenFlow スイッチを、一つの大きな L2 スイッチとして動作させる]

=== ルーティングスイッチの動作

ルーティングスイッチの基本的な動作は、ラーニングスイッチとほぼ同じです。しかし、パケットを受信したスイッチと違うスイッチまでパケットを届けなければならない点が異なっています。そのためには、入口となるスイッチから出口となるスイッチまでの最短パスを見つける必要があります。

例えば、@<img>{flow_mod} のネットワークにおいて、ホスト 1 からホスト 4 へとパケットを届ける場合について考えてみます。ホスト 1 はスイッチ 1 へ、ホスト 4 はスイッチ 6 へと、それぞれ違うスイッチに接続されています。そのため、今回はスイッチ 1 からスイッチ 5、スイッチ 6 の順にパケットを転送し、ホスト 4 へと届くようにフローを設定すればよさそうです。

//image[flow_mod][最短パスでパケットが転送されるよう、フローを設定する]

それでは、ルーティングスイッチの動作を、順に見ていきましょう。

 1. ルーティングスイッチは、 スイッチが受信したパケットを Packet In メッセージで受け取ります。@<img>{flow_mod} の例では、スイッチ 1 が受信したホスト 1 からホスト 4 へのパケットが、スイッチ 1 から送られてきます。
 2. 次に FDB を検索し、宛先であるホストが接続されているスイッチ、ポートを検索します。今回は、スイッチ 6 と、このスイッチのホスト 4 に接続しているポートが得られます。
 3. Packet In を送ってきたスイッチから出口となるスイッチまでの最短パスを計算します。この場合、スイッチ 1 からスイッチ 6 の最短パスは、スイッチ 5 を経由するパスとなります。
...
それでは、スイッチ 1 からスイッチ 6 までのパスは、どのように計算すればよいでしょうか？ルーティングスイッチでは、ダイクストラ法というアルゴリズムを用いて、二つのスイッチ間の最短パスを求めています。ダイクストラ法は、グラフ上の最短路を求めるための非常にポピュラーなアルゴリズムで、カーナビの経路探索や鉄道の乗換案内などに用いられています@<fn>{dijkstra}。

//footnote[dijkstra][ダイクストラ法は、ネットワークの世界でも広く使われており、OSPF や IS-IS 等の L3 の経路制御プロトコルで用いられています。]

//image[dijkstra][最短パスを計算する]

ダイクストラ法を用いて最短パスを見つける方法を説明します (@<img>{dijkstra})。

1. 始点となるスイッチ 1 に着目します (@<img>{dijkstra} (a))。

2. まず、スイッチ 1 から 1 ホップで行けるスイッチを見つけ出します。これはスイッチ 1 に接続するリンクの先に繋がっているスイッチを、すべてピックアップすればよさそうです。その結果、スイッチ 2 とスイッチ 5 が見つけ出せます (@<img>{dijkstra} (b))。

3. 同じようにして、今度はスイッチ 1 から 2 ホップで行けるスイッチを見つけ出します。これは、前のステップで見つけたスイッチ 2 とスイッチ 5 から、さらにその先 1 ホップで行けるスイッチを調べればよさそうです。このようにすることで、今度はスイッチ 3, 4, 6 が見つかります(@<img>{dijkstra} (c))。

4. 終点であるスイッチ 6 が、スイッチ 5 の先で見つかりましたので、最短パスは最終的にスイッチ 1 → スイッチ 5 → スイッチ 6 であることがわかります。
...
//image[lldp-packet-in][スイッチ 0x2 から送られてくる Packet In メッセージ][scale=0.5]
...
//image[modules][ルーティングスイッチの動作に必要なモジュール][scale=0.5]
...
//image[spt1][ループを含む構成では、一部のリンクを遮断する必要がある][scale=0.6]
...
//image[spt2][ルーティングスイッチでは、ネットワーク中のリンクを有効に使うことができる][scale=0.6]
