...
筆者はネットワーク研究者という仕事柄、よく大小さまざまなネットワークを目にします。その中でもいつも「すごい!」とうならされるのが、すべてのネットワークエンジニアの憧れ、ShowNet です。これはネットワーク系最大の展示会 Interop の期間中だけ構築・運用されるネットワークで、最新ネットワーク技術のいわばショーケースとも言えるものです。普段は絶対に触ることのできない、ネットワーク界の F1 マシンとも言える最新機器を集めたライブデモンストレーションは圧巻の一言です。

ShowNet の魅力をもっともよく伝えてくれるのが、Interop で毎年公開される ShowNet のトポロジ図です (@<img>{shownet_topology})。この図の素晴らしいところは、ShowNet を構成するネットワーク機器同士の複雑な接続関係をたった1枚に収めているところです。「この部分は、いったいどんなプロトコルで動いているんだろう？」「実際の詳しいトポロジはどうなっているのかな？」ネットワークが好きな人であれば、気がつくと何時間でも眺めてしまうほどの魅力を持つトポロジ図なのです。

//image[shownet_topology][2012 年 Interop の ShowNet トポロジ図。Copyright (C) 2012 Interop Tokyo ShowNet NOC Team. All Rights Reserved.][width=12cm]
...
ルーティングスイッチは、複数台の OpenFlow スイッチを連動させ1つの大きなスイッチとして動作させるためのコントローラです (@<img>{routing_switch})。同じスイッチ機能を提供するコントローラとしては@<chap>{learning_switch}で紹介したラーニングスイッチがありましたが、これとの大きな違いは制御できるスイッチの台数です。ラーニングスイッチは 1 台の OpenFlow スイッチしか扱えない一方で、ルーティングスイッチは複数台の OpenFlow スイッチを制御できます。

//image[routing_switch][いくつもの OpenFlow スイッチを1つの大きなスイッチとして動作させる][width=12cm]

複数台の OpenFlow スイッチを使い、まさにルーティングするようにパケットを転送することから、ルーティングスイッチという名がつきました。ただし、ルーティングスイッチは、ルータではなく、ラーニングスイッチと同じく、スイッチ機能を実現するコントローラであることに注意が必要です。

=== ルーティングスイッチの動作

ルーティングスイッチではたくさんのスイッチを接続するため、パケットの転送が複雑になります。多くの場合、宛先のホストまでいくつものスイッチを経由してパケットを届けなくてはならないため、宛先までの道順をスイッチに指示してやる必要があるのです。

例えば、@<img>{flow_mod} において、ホスト 1 からホスト 4 へとパケットを送信する場合を考えてみます。もしパケットを最短のパスで届けたい場合、スイッチ 1 → スイッチ 5 → スイッチ 6 → ホスト 4 の順にパケットを転送します。ルーティングスイッチはこの転送をするフローエントリを最短パス上のスイッチ 1, 5, 6 へ書き込む必要があります。

//image[flow_mod][最短パスでパケットを転送するためにフローエントリを書き込む][width=12cm]
...
最短パスの計算でよく使われるのがダイクストラ法というアルゴリズムです。これは、出発点から目的地までの最短路を求めるための汎用アルゴリズムの1つで、カーナビの経路探索や鉄道の乗換案内などにも使われています。

ダイクストラ法を使った最短パス計算のアルゴリズムは、基本的には次のようになります。まず、出発点から 1 ホップで到達できるスイッチをすべて探します。次に、見つかったスイッチから出発して 1 ホップで行けるスイッチ、つまり最初の出発点から 2 ホップで到達できるスイッチをすべて探します。これを繰り返して、出発点から 3 ホップ、4 ホップ…というように距離を広げながら次々とスイッチを探していきます。途中で目的地のスイッチに到達したら探索完了で、そこまでのパスを最短パスとして返します@<fn>{dijkstra}。

//footnote[dijkstra][ダイクストラ法はリンクに重み (距離) がある場合の最短路を求めることができるので、実際にはもう少し複雑な手順になります。ネットワーク上の最短パスではそれぞれのリンクは"重み1"として考えるので、このように単純化できます。]

//image[dijkstra][最短パスをダイクストラ法で計算する][width=12cm]
...
実際に@<img>{routing_switch} のネットワークでスイッチ 1 からスイッチ 6 までの最短パスをダイクストラ法で探索する手順は次のようになります (@<img>{dijkstra}):
 ...
 4. ゴールであるスイッチ 6 が見つかったので探索を終わる。最短パスは最終的にスイッチ 1 → スイッチ 5 → スイッチ 6 であることがわかる
...
//image[lldp_overview][LLDP を使ってリンクを発見する][width=12cm]
...
//image[lldp_openflow][LLDP パケットと OpenFlow の仕組みを使ってリンクを発見する][width=12cm]

 1. コントローラは、接続関係を調べたいスイッチの Datapath ID 0x1 とポート番号 4 を埋め込んだ Link Layer Discovery Protocol (LLDP) パケットを作る
 2. ポート 4 から出力するというアクションを含む Packet Out メッセージを作り、先ほど作った LLDP パケットをスイッチ 0x1 へと送る
 3. Packet Out を受け取ったスイッチはアクションに従い、LLDP パケットを指定されたポート 4 から出力する。その結果、LLDP パケットは、ポート 4 の先につながるスイッチ 0x2 へと到着する
 4. LLDP パケットを受け取ったスイッチ 0x2 は、自身のフローテーブルを参照し、パケットの処理方法を調べる。このとき LLDP に対するフローエントリはあえて設定していないため、今回受信した LLDP パケットは、Packet In としてコントローラまで戻される
 5. コントローラは、受け取った Packet In メッセージを解析することで、リンクの発見を行う。スイッチ 0x2 からは @<img>{lldp-packet-in} の Packet In メッセージが送られてくる。この中身を見ることで、スイッチ 0x1 のポート 4 と、スイッチ 0x2 のポート 1 の間にリンクを発見できる

//image[lldp-packet-in][スイッチ 0x2 から送られてくる Packet In メッセージ][width=12cm]
...
//image[topology_before][トポロジ検出前のコントローラ][width=12cm]
...
//image[topology_after][スイッチ 0x1 から出るリンクを発見][width=12cm]
...
//image[fullmesh][スイッチ 4 台からなるネットワーク][width=12cm]

//noindent
@<img>{fullmesh} の構成を実現する設定ファイル (@<tt>{routing_switch_fullmesh.conf})は、先ほど GitHub から取得したソースコードの中に含まれています。この設定ファイルを用いて、次のようにルーティングスイッチを起動します。
...
//noindent
それでは起動したルーティングスイッチがどのようにトポロジを検出するかを見てみましょう。
...
それでは、どのようなフローエントリが設定されたかを見てみます。設定されているフローエントリの確認は、@<tt>{trema dump_flows [表示したいスイッチの Datapath ID]} でできます。まずは host1 から host2 への最短パスであるスイッチ @<tt>{0x1, 0x2} のフローテーブルを見てみましょう。
...
//noindent
出力より、@<tt>{0x1} と @<tt>{0x2} のスイッチそれぞれに host2 → host1 のフローエントリが設定されていることがわかります。

一方で、最短パス上にないスイッチ @<tt>{0x3}, @<tt>{0x4} はパケットが通らないため、次のようにフローエントリがありません。
...
本章のはじめで説明したように、ルーティングスイッチは OpenFlow ネットワークを1台の仮想的なスイッチとして動作させるコントローラアプリケーションです。普通のスイッチを真似るだけならば、わざわざ OpenFlow を使わなくてもよいのでは？と思うかもしれません。ここでは、OpenFlow を使った場合の利点について考えてみたいと思います。
...
//image[spt1][スパニングツリーではループを避けるために一部のリンクを遮断する][width=12cm]

一方、ルーティングスイッチではコントローラがトポロジ全体を把握しているため、ループを防ぐためのリンク遮断は必要ありません。パケットの転送経路を各スイッチにフローエントリとして明示的に指示するため、ループを含むトポロジであっても問題なく動作します。このためスパニングツリーを使う場合と比べて、ネットワーク中のリンクを有効に使うことができます (@<img>{spt2})。

//image[spt2][ルーティングスイッチではネットワーク中のリンクを有効に使える][width=12cm]

=== いろいろなパス選択アルゴリズムを使える

パスの決定はコントローラで一括して行なうため、パス決定アルゴリズムを入れ替えるだけで、さまざまなパス選択を実現できます。今回、ルーティングスイッチではダイクストラ法による最短パスを使いましたが、たとえば @<img>{multipath} のようにフロー毎に異なるパスを設定することで、帯域確保のためのマルチパスを作ることも簡単にできます。

//image[multipath][OpenFlow では最短でないパスを含んだマルチパスを自由に作れる][width=12cm]

このようなマルチパスは従来の自律分散型でも実現できますが、厳しい制限があります。IETF が標準化を行う TRILL (Transparent Interconnect of Lots of Links) や IEEE が標準化を行う SPB (Shortest Path Bridges) は、マルチパス転送に対応しています。しかし、マルチパス転送を使えるのは、最短パスが複数ある場合@<fn>{ecmp}だけです。最短ではないパスは、ループを起こす可能性があるため、使用することができません。また最短パスが 1 本だけの場合にもマルチパスにできません。
...
: マスタリング TCP/IP 応用編 (Philip Miller 著、オーム社)
  とくにレイヤ 3 の経路制御プロトコルについて詳しく説明した本です。ダイクストラ法を用いた経路制御プロトコルの一つである OSPF についても説明しているので、ルーティングスイッチとの違いを比べてみるのもおもしろいでしょう。
