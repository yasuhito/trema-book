...
//image[multipath][OpenFlow では自由にマルチパスを使れる][scale=0.5]

このようなマルチパスは従来の自律分散型でも実現できますが、きつい制限があります。マルチパスを実現できる自律分散型制御には、IETF が標準化を行う TRILL (Transparent Interconnect of Lots of Links) や IEEE が標準化を行う SPB (Shortest Path Bridges) などがあります。これらがマルチパスを作れる条件は、宛先までのコストが同じでしかも最短となるパスが何本もある場合@<fn>{ecmp}に限ります。なぜなら、コストが同じ最短パスが何本もあれば、各スイッチが自律的にそれぞれの最短パスに沿ってパケットの転送を行えるからです。この場合ループは起こりません。しかし、コストが最短でないパスは作れませんし、最短パスが 1 本だけの場合にはマルチパスになりません。
