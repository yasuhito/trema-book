...
私は、多数のスイッチ、ルータから構成される大規模なネットワークの構成図を見ると、心が踊ります。``どんなプロトコルで動いているのか?'' を知りたくなったり、``いつか自分でも動かしてみたい!'' と思ったりするものです。ここまでの章では一台の OpenFlow スイッチを制御する方法について学んできましたが、もちろん、OpenFlow は、複数台のスイッチからなる大規模なネットワークでも使えます。
...
//footnote[sync][ルーティングスイッチと連動して動作するトポロジーモジュールが管理する情報に関しては、同期を行う必要があるかもしれません。仮に同期を行わなかった場合、フェイルオーバー後にトポロジー検出を再度行う必要が有るため、コントローラとしての動作を再開するまでに時間が掛かってしまう可能性があります。]
