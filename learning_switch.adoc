...
まずはラーニングスイッチのソースコード (<<learning_switch.rb,lib/learning_switch.rb>>) をざっと眺めてみましょう。 ソースコードと仮想ネットワークの設定ファイルはGitHubのtrema/learning_switchリポジトリ (https://github.com/trema/learning_switch) からダウンロードできます。とくに、`private` の行よりも上のパブリックなメソッドに注目してください。
...
[source,ruby,subs="verbatim,attributes"]
.lib/learning_switch.rb (LearningSwitch#packet_in, LearningSwitch#flow_mod_and_packet_out)
----
...
[source,ruby,subs="verbatim,attributes"]
.lib/learning_switch.rb (LearningSwitch#flow_mod_and_packet_out)
----
include::{sourcedir}/lib/learning_switch.rb[lines="24..28"]
----

このflow_modメソッドとpacket_outメソッドはそれぞれTrema::Controllerクラスのsend_flow_mod_add(<<cbench,第3章>>で紹介)およびsend_packet_out(Packet Outの送信)メソッドを次のように呼び出します。

[source,ruby,subs="verbatim,attributes"]
.lib/learning_switch.rb (LearningSwitch#flow_mod, LearningSwitch#packet_out)
----
