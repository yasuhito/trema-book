...
ラーニングスイッチのソースコードは <<learning_switch.rb,lib/learning_switch.rb>> と <<fdb.rb,lib/fdb.rb>> の 2 つからなります。まずはメインのソースコード (<<learning_switch.rb,lib/learning_switch.rb>>) をざっと眺めてみましょう。 ソースコードと仮想ネットワークの設定ファイルはGitHubのtrema/learning_switchリポジトリ (https://github.com/trema/learning_switch) からダウンロードできます。とくに、`private` の行よりも上のパブリックなメソッドに注目してください。
