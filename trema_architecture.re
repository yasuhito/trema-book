...
例を1つ挙げます。OpenFlow の仕様に Flow Cookie という便利な機能があります。この Cookie を Flow Mod を打つときのパラメータとして指定すると、たとえば同じ Cookie を持つフローエントリーどうしを 1 つのグループとしてまとめて管理するなどができます。Trema でこの Cookie を使った場合、よく調べると Flow Mod で指定した Cookie 値とスイッチのフローテーブルに実際に設定された Cookie 値が異なることがわかります。
