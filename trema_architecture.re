...
それでは、Trema Apps の中でもとくに実用的なアプリを中心に解説していきま
しょう。

=== ルーティングスイッチ

複数の OpenFlow スイッチで構成されるネットワークを 1 つのレイヤ 2 スイッ
チに仮想化します。サンプルプログラムのマルチラーニングスイッチと異なる
点は、ループのあるネットワークトポロジにも対応している点と、パケットの
転送に必ず最短パスを選択する点です。詳しくは@<chap>{routing_switch}で説
明します。

=== スライス対応スイッチ

OpenFlow ネットワーク全体を独立したスライスに分割し、複数のレイヤ 2 ス
イッチに仮想化します。これによって、アプリケーションやユーザグループな
ど用途に応じて独立した仮想ネットワークを作ることができます。ちょうど、
ネットワークを複数の VLAN に分けて使うイメージです。詳しくは
@<chap>{sliceable_switch}で説明します。
...
します。このしくみを使えばたとえば、認証していないユーザの HTTP セッショ
...
さて、第 II 部プログラミング編はこれでおしまいです。Trema を使えば
OpenFlow コントローラが簡単に開発できることを実感していただけたのではな
いでしょうか？Trema を使って、ぜひあなたも便利な OpenFlow コントローラ
を開発してみてください。
...
: C リファレンスマニュアル (S・P・ハービソン 3 世ら著、星雲社)
  C 言語でちょっとでもわからないことがあると、私達は K&R よりもこちらの
  本を手を取ります。C の JIS 規格を読むよりもわかりやすく、あらゆる事項
  が載っています。プログラミング上達のコツは、あいまいな事はすぐ調べそ
  のままにしない事です。
...
: Smalltalk ベストプラクティス(ケント・ベック著、ピアソン・エデュケーション)
  歴史あるプログラミング言語 Smalltalk の慣用句についての本ですが、
  Ruby でも十分使えます。たとえばイテレータの仮引数の名前を何にするか？
  私はこの本に従っていつでも @<tt>{each} を使うことに決めてから、これに
  ついてはもう悩まなくなりました。
...
: 「週 4 時間」だけ働く。(ティモシー・フェリス著、青志社)
  ...
  も過激な "週 4 時間" を提唱するこの本は、本業と関係の無い雑用をいかに
  うまくかわすかに 600 ページ超を割いています。とくに雑用のアウトソーシ
  ングのテクニックの数々は参考になります。
...
: 小さなチーム、大きな仕事 (ジェイソン・フリードら著、ハヤカワ新書)
  どんなプロジェクトでもプログラマ不足はいつも悩まされる問題ですが、こ
  の本は逆に小さなチームでいかに大きな成果を挙げるかに焦点を絞ります。
  問題はどの機能を付け加えるかではなく、どこを削るかにあるのです。

: リファクタリング・ウェットウェア (Andy Hunt 著、オライリー)
  プログラミングなどの知的作業に詰まったら、この本を読んで脳というハー
  ドウェアの癖を理解してください。この本は脳の取り扱い説明書です。

: フーリエの冒険 (トランスショナル・カレッジ・オブ・レックス著、ヒッポファミリークラブ)
  この本の説明は秀逸で、数学的知識ゼロの状態から最終的には FFT が理解で
  きるようになります。しかもいい加減な "図でなんとなくわかった気になる
  説明" ではなく、必要な微積分もすべてきちんと説明しているのです。この
  第 II 部も Ruby の知識ゼロから学べるように工夫しましたが、この "フー
  リエの冒険" の域に逹っしているでしょうか。

: ティッピング・ポイント (マルコム・グラッドウェル著、飛鳥新社)
  ある事柄が社会的ブームになるまでのメカニズムを解き明かします。口コミ
  がコネクター、メイヴン、セールスマンと呼ばれる種類の人々を通じてティッ
  ピング・ポイント (転換点) に逹し、爆発的流行に至る。オープンソース
  で成功するプロジェクトをやりたければ、この本は必読でしょう。
