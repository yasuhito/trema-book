...
//lead{
今まで触れてこなかった Trema のアーキテクチャについて説明します。他の章に比べ少し難易度が高いので、取っつきやすいように対話形式にしてみました。
//}
...
@<em>{取間先生}「そんなことはないぞ、ぜんぜん簡単じゃ。Switch Manager の役割はいわば inetd と同じと考えればよい。つまり Switch Manager は新しいスイッチが接続してくるのを見張っているだけで、実際のスイッチとのやりとりはスイッチごとに起動する Switch Daemon プロセスに一切合切まかせてしまうというわけじゃ」@<br>{}
...
===[column] 取間先生いわく: きれいな OpenFlow メッセージを作る
...
Switch Daemon のもうひとつの重要な役割がスイッチの仮想化です。実は、Trema ではひとつのスイッチにいくつものアプリケーションをつなげられます。このとき、アプリケーションの間でスイッチ制御を調停し、競合が起きないようにするのも Switch Daemon の大きな役割です (@<img>{switch_virtualization})。

//image[switch_virtualization][Switch Daemon はスイッチ制御を調停することでアプリ間の競合を防ぐ]

ひとつ例を挙げましょう。OpenFlow の仕様に Flow Cookie という便利な機能があります。この Cookie を Flow Mod を打つときのパラメータとして指定すると、たとえば同じ Cookie を持つフローエントリーどうしを 1 つのグループとしてまとめて管理するなどができます。Trema でこの Cookie を使った場合、よく調べると Flow Mod で指定した Cookie 値とスイッチのフローテーブルに実際に設定された Cookie 値が異なることがわかります。
...
//noindent
@<em>{友太郎}「Trema でなにがうれしいかって、仮想ネットワークの機能ですよね! おかげでノートパソコン 1 台で開発できるからすっごく楽なんですけど、あれってどういうしくみなんですか？」@<br>{}
...
仮想ホストの実体は、phost と呼ばれるユーザーレベルプロセスです (@<tt>{vendor/phost/})。これは、任意のイーサネットフレーム・UDP/IP パケットを送受信できます。@<tt>{trema run} コマンドに与えられた設定ファイル中の仮想ホスト定義 (@<list>{vswitch}) に従って、Trema は必要な数の phost プロセスを起動します。
...
酔っ払いに怒られることほどみじめなことはありません。でも友太郎君は取間先生の話が聞きたいので仕方無くコンビニエンスストアまでつきあいます。
...
//noindent
@<em>{友太郎} 「なるほど。Trema の OpenFlow API が使いやすい理由って、裏でこういう変換をたくさんやってくれているからなんですね。たしかにバイトオーダー変換とかを意識しないで書けるのってすごく楽です」@<br>{}
...
コンビニエンスストアから帰ってきましたが、深夜の二人のTrema 談義はさらに続きます。@<br>{}
...
OpenFlow ネットワーク全体を独立したスライスに分割し、複数のレイヤ 2 スイッチに仮想化します。これによって、アプリケーションやユーザーグループなど用途に応じて独立した仮想ネットワークを作れます。ちょうど、ネットワークを複数の VLAN に分けて使うイメージです。詳しくは@<chap>{sliceable_switch}で説明します。

=== リダイレクト機能付きルーティングスイッチ

ルーティングスイッチの亜種で、ユーザー認証とパケットのリダイレクト機能を付け加えたものです。基本的な動作はルーティングスイッチと同じですが、認証されていないホストからのパケットをほかのサーバーに強制的にリダイレクトします。このしくみを使えばたとえば、認証していないユーザーの HTTP セッションを強制的に特定のサイトへ飛ばすなどといったことが簡単にできます。
...
フローエントリーとパス情報を管理するアプリケーションおよび API を提供します。かなり実験的な実装なので、API は変更する可能性があります。
 ...
 * Tremashark により、Trema の内部動作を可視化できます。これは、アプリケーションや Trema の動作を詳細に確認したい場合に役立ちます。
...
ここでは第 II 部プログラミング編のまとめとして、開発全般で私たちが役に立つと思った本を紹介します。

: C リファレンスマニュアル (S・P・ハービソン 3 世ら著、星雲社)
  C 言語でちょっとでもわからないことがあると、私たちは K&R よりもこちらの本を手を取ります。C の JIS 規格を読むよりもわかりやすく、あらゆる事項が載っています。プログラミング上達のコツは、あいまいな事はすぐ調べそのままにしない事です。
...
: 「週 4 時間」だけ働く。(ティモシー・フェリス著、青志社)
  実はプログラミングで最も難しいのはプログラミングのためにまとまった時間を捻出することなのではないかと思っています。XP の "週 40 時間" よりも過激な "週 4 時間" を提唱するこの本は、本業と関係ない雑用をいかにうまくかわすかに 600 ページ超を割いています。とくに雑用のアウトソーシングのテクニックの数々は参考になります。

: レボリューション・イン・ザ・バレー (アンディ・ハーツフェルド著、オライリー)
  週 90 時間労働という極限状態でオリジナル Macintosh を作った人々のストーリー。これはもちろんまねしてはいけませんが、スティーブ・ジョブズを中心としたチームの結束力と強いこだわりを見ると誰でもやる気が出ます。開発当時のメモや写真がたくさん載っており、よくできたドキュメンタリー映画のように当時の熱気が伝わってきます。
...
: リファクタリング・ウェットウェア (Andy Hunt 著、オライリー)
  プログラミングなどの知的作業に詰まったら、この本を読んで脳というハードウエアの癖を理解してください。この本は脳の取り扱い説明書です。
