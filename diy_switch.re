...
ここで、ファームウェアのインストール用に使ったホストを LAN 側から抜き、WAN 側につなぎなおします。ファームウェアのアップデート直後には、WAN 側ポートに 192.168.1.1/24 というネットワークが設定されています@<fn>{config}。ファームウェアに設定されているコントローラの IP アドレスはデフォルトで 192.168.1.10 であるため、このアドレスを使用しましょう。この WAN 側ポートでは DHCP サーバは動作していないので、この IP アドレスを固定で設定してください。
...
もし出力が期待通りでない場合、何らかの原因でスイッチとコントローラ間の TCP コネクションに失敗している可能性があります。これを確認するためにはコントローラ用ホスト上で @<tt>{netstat} コマンドを使います。OpenFlow プロトコルではスイッチとコントローラ間の接続にはポート 6633 を使いますので、次のように @<tt>{netstat} と @<tt>{grep} で現在有効な OpenFlow のコネクションを表示します。
...
もし 2 行目の表示がされない場合、スイッチ側の OpenFlow サービスが、なんらかの理由で有効になっていない可能性があります。この場合、スイッチの OpenFlow サービスを再起動する必要があります。OpenFlow スイッチに @<tt>{telnet} でログインし、次のようにサービスを再起動してください。
...
これを頭に入れた上で出力を見ると、host1 (@<tt>{nw_src=192.168.2.1}) から host 2 (@<tt>{nw_dst=192.168.2.2}) 宛の ICMP のフローと、その逆向きのフローがそれぞれ見つかります。ここで ICMP の特殊な決まりとして、ICMP メッセージタイプ (エコー要求は 8、エコー応答は 0) は @<tt>{tp_src} のフィールドに格納することになっています (@<chap>{openflow_spec} 参照)。これを考えると一つ目のフローは ICMP エコー要求で、二つ目のフローは ICMP エコー応答であると判別できます。
...
スイッチ内で動作するファームウェア (Linux) からは eth0、eth1 の 2 つのネットワークインターフェースが見えており、それぞれ内部スイッチと WAN 側ポートに接続されています。ファームウェア上ではスタンフォード大のソフトウェアレファレンススイッチが動作しており、これが OpenFlow プロトコルをしゃべります。ルータには VLAN に対応した内部スイッチが入っており、これが VLAN タグを付けることによって 4 つの LAN 側ポートからのパケットを区別して eth0 に転送します。このように eth0 を内部スイッチが 4 つのポートに分けることで、仮想的に 4 ポートを使うことができるようになっているわけです。
