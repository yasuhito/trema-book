...
次に、無線 LAN ルータのファームウェアを先ほどダウンロードした OpenFlow 対応のものに書き換えます。ファームウェアのアップデート手順は通常のアップデート手順とまったく同じです。ルータに接続した PC からルータの管理画面を開き、さきほどダウンロードしたファームウェアを使ってアップデートしてください。"ファームウェアのアップデート中は決して電源を切らない" という約束を守れば、誰でも簡単にアップデートできるはずです。

入れ替えに成功すると、LAN 側と WAN 側ポートの役割がそれぞれ次のように変更されます。

 * LAN 側ポート: OpenFlow スイッチとして動作する。
 * WAN 側ポート: コントローラとの接続用。それ以外にも @<tt>{telnet} ログインに用いられる。
...
さていよいよ自分専用 OpenFlow スイッチを Trema とつなげてみましょう。今まで作ってきた様々なコントローラを接続し、OpenFlow スイッチとしての動作をいろいろな角度から検証してみます。セットアップとして、コントローラを動かすホスト 1 台と、通信用のホストを二台用意し @<img>{network} のように接続します。

//image[network][Trema との接続][scale=0.5]

=== Trema の起動

まずは WAN 側につないだコントローラ用ホスト上で次のようにラーニングスイッチを起動します。
...
: オープンルータ・コンペティションまとめ (@<tt>{https://sites.google.com/site/orcmatome/})
  Interop Tokyo 2012 併催のオープンルータ・コンペティションでは、計 10 チームからのさまざまなアイデアでの参戦がありました。このうち 4 チームが OpenFlow を活用したアイデアで、この 4 チームすべてが実装に Trema を使っていました。筆者の一人が運営するこのサイトには、参加各チームのスライドやソースコードへのリンクがまとめてあります。
