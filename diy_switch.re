...
あなたも仮想スイッチではそろそろ物足りなくなってきたころではないでしょうか？本格的に OpenFlow プログラミングに取り組むなら、やはり実機の OpenFlow スイッチを自宅にデンと置いて心ゆくまで一人占めしたいものです。「でも、中古ハードウェアショップにもまだ出てないんだよなあ」。自宅にサーバーラックを置き、高価な Catalyst や Summit などの中古品をマウントして愉しむ機材マニア達にかかっても、まだ実機は入手できていないようです。OpenFlow スイッチはまだまだ新しい分野の製品なので、中古市場には出回っていないからです。
...
ping コマンドは、宛先に ICMP エコー要求と呼ばれるパケットを送るコマンドです。ICMP エコー要求を受け取ったホストは、送信元ホストに対して ICMP エコー応答パケットを送り返します。この ICMP のプロトコル番号は 1 です。またエコー要求のメッセージタイプは 8、エコーリクエストのメッセージタイプは 0 と定義されています。

@<tt>{flow_dumper} の出力を見ると、ホスト 1 (@<tt>{nw_src=192.168.2.1}) からホスト 2 (@<tt>{nw_dst=192.168.2.2}) 宛の ICMP (@<tt>{nw_proto=1}) のフローと、その逆向きのフローがそれぞれ設定されていることが分かります。パケットが ICMP の場合、メッセージタイプは @<tt>{tp_src} のフィールドに格納されます(@<chap>{openflow} 参照)。一つ目のフローは @<tt>{tp_src=8} であるため、ICMP エコー要求であることが確認できます。また、二つ目のフローは @<tt>{tp_src=0} であるため、ICMP エコー応答であることが確認できます。

なおタイミングによっては、これ以外に ARP 用のフローが表示されるかもしれません。
...
: @SRCHACK 氏のブログ (@<tt>{http://www.srchack.org/})
  今回紹介した自作 OpenFlow スイッチ界の中心人物 @SRCHACK 氏のブログです。OpenFlow 対応ファームウェアを無線 LAN ルータ上で動作させるまでの試行錯誤や、自宅でサーバーラックを運用する "自宅ラック友の会" の活動の様子などをブログ形式で紹介しています。
