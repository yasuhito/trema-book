...
そんな中、事件は 2012 年の春に起きました。ある一人のエンジニアが、二千円で買える "家庭用" 無線 LAN ルータをハックし、OpenFlow スイッチに改造するツール一式を公開したのです。しかも、秋葉原の居酒屋にて有志による「無線 LAN ルータを OpenFlow 化して、Trema と接続してみよう」という勉強会を開催しました。この夢の "Trema とつながる 二千円の OpenFlow スイッチ" は Twitter でも公開され、日本中にまたたく間に広まりました。

2012 年の夏にはこの動きがさらに広がりを見せます。幕張で開催されるネットワーク業界の最大の展示会、Interop Tokyo 2012 の併催イベント、オープンルータ・コンペティションに、この激安 OpenFlow スイッチプロジェクトが参戦したのです。このオープンルータ・コンペティションとは、次世代のインターネットを担う技術者の発掘を目指したコンテストで「ソフトウェア、ハードウェアに関わらずルータを改良、または創造し、その成果を発表すること」という課題で競われます。最終審査会では予選を突破した 10 チームがそれぞれのルータを持ち寄りましたが、この激安 OpenFlow スイッチで組んだメッシュネットワーク (@<img>{orc}) でデモを披露した @SRCHACK 氏が見事準グランプリを勝ち取りました。
