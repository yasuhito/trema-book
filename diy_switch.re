...
あなたも仮想スイッチではそろそろ物足りなくなってきました。やはり実機の OpenFlow スイッチを自宅にデンと据えて、Trema で心ゆくまで OpenFlow プログラミングを楽しみたいものです。「でも、中古ハードウェアショップにもまだ出てないんだよなあ」。高価な Catalyst や Summit などの中古品を自宅で愉しむ "ネットワークマニア" にかかっても、まだ実機は入手できていないようです。なぜなら OpenFlow スイッチはまだまだ新しい分野の製品なので、中古市場には出回っていないのです。
...
そんな中、事件は 2011 年の年末に起きました。ある一人のエンジニアが、二千円で買える "家庭用" 無線 LAN ルータをハックし、OpenFlow スイッチに改造するツール一式を公開したのです。しかも、秋葉原の居酒屋にて有志による「無線 LAN ルータを OpenFlow 化して、Trema と接続してみよう」という勉強会を開催しました。この夢の "Trema とつながる 二千円の OpenFlow スイッチ" は Twitter でも公開され、日本中にまたたく間に広まりました。

2012 年にはこの動きがさらに広がりを見せます。幕張で開催されるネットワーク業界の最大の展示会、Interop Tokyo 2012 の併催イベント、オープンルータ・コンペティションでこの激安 OpenFlow スイッチプロジェクトが準グランプリを獲得したのです。このオープンルータ・コンペティションとは、次世代のインターネットを担う技術者の発掘を目指したコンテストで「ソフトウェア、ハードウェアに関わらずルータを改良、または創造し、その成果を発表すること」という課題で競われます。最終審査会では予選を突破した 10 チームがそれぞれのルータを持ち寄りましたが、この激安 OpenFlow スイッチで組んだメッシュネットワーク (@<img>{orc}) でデモを披露した @SRCHACK 氏が見事準優勝を勝ち取りました。

//image[orc][オープンルータ・コンペティション会場での無線 LAN ルータを改造して作った OpenFlow メッシュネットワーク][scale=0.12]
...
こうしてできた OpenFlow スイッチの内部構成は次のようになっています。スイッチ内で動作する Linux からは eth0、eth1 の 2 つのネットワークインタフェースが見えており、それぞれ内部スイッチと WAN 側ポートに接続されています。内部スイッチは VLAN に対応したレイヤ 2 スイッチで、タグを付けることによって 4 つの LAN 側ポートからのパケットを区別します。このため、論理的には 4 ポートですが OpenFlow としては 1 つのポートとして見えることに注意してください。
...
以下のコマンドでスイッチとコントローラ間に TCP コネクションが張られているかを確認しましょう。次のように表示されていれば成功です。
...
もしこのように表示されない場合、OpenFlow サービスが有効になっていない可能性があります。OpenFlow スイッチに @<tt>{telnet} でログインし、次のようにサービスを再起動してください。
...
: OpenFlow 対応ファームウェア作者の SRCHACK 氏の Blog (@<tt>{http://www.srchack.org/})
