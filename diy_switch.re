...
出力の最初の 3 行により、ルーター内で起動している OpenFlow スイッチの実装は、スタンフォード大学で作成されたソフトウエアレファレンススイッチのバージョン 1.0.0 であることが分かります。また、@<tt>{eth0.1} から @<tt>{eth0.4} までの OpenFlow 用ポート 4 つと、ローカルポート @<tt>{tap0} が定義されていることがわかります。なお@<chap>{openflow_spec}でも触れましたがこのローカルポート @<tt>{tap0} は特殊な用途向けですのでユーザーが普段使うことはありません。
...
これを頭に入れた上で出力を見ると、ホスト 1 (@<tt>{nw_src=192.168.2.1}) からホスト 2 (@<tt>{nw_dst=192.168.2.2}) 宛の ICMP のフローと、その逆向きのフローがそれぞれ見つかります。ここで ICMP の特殊な決まりとして、ICMP メッセージタイプ (エコー要求は 8、エコー応答は 0) は @<tt>{tp_src} のフィールドに格納することになっています (@<chap>{openflow_spec} 参照)。これを考えると一つ目のフローは ICMP エコー要求で、二つ目のフローは ICMP エコー応答であると判別できます。
