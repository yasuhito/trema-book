...
そんな中、事件は 2012 年の春に起きました。"自宅ラック派" エンジニアの一人、@SRCHACK.ORG 氏@<fn>{srchack}が二千円で買える家庭用無線 LAN ルータ (@<img>{akihabara}) をハックし、OpenFlow スイッチに改造するツール一式を公開したのです。これは劇的な出来事で、OpenFlow を試したいけどもスイッチが高くて躊躇している人々を電器店に走らせるきっかけとなりました。さらに同氏とその仲間達は秋葉原にて「無線 LAN ルータを OpenFlow 化して、Trema と接続してみよう」勉強会を開催し、集まった一人ひとりに OpenFlow スイッチ自作法を伝授したのです。これらの情報は Twitter でも公開され、日本各地からの OpenFlow スイッチ自作レポートが続々と集まりはじめました。

//footnote[srchack][@<tt>{http://www.srchack.org/}]

//image[akihabara][OpenFlow 改造が可能な家庭用無線 LAN ルータ。もちろん秋葉原などでこのように普通に売っている]

2012 年の夏にはこの動きがさらに広がりを見せます。幕張で開催されるネットワーク業界の最大の展示会、Interop Tokyo 2012 の併催イベント、オープンルータ・コンペティション@<fn>{orc}に、@SRCHACK.ORG 氏による自作 OpenFlow スイッチプロジェクトが参戦したのです。このオープンルータ・コンペティションとは、次世代のネットワーク技術者の発掘を目的としたコンテストで「ソフトウェア、ハードウェアに関わらずまったく新しいルータを改良、または創造する」という課題で競われます。最終審査会では予選を突破した 10 チームがそれぞれのルータを持ち寄りましたが、@SRCHACK.ORG 氏の自作 OpenFlow スイッチによるメッシュネットワーク (@<img>{orc_srchack}) が見事準グランプリを勝ち取りました。Interop のような業界が注目する場で受賞したことによって、OpenFlow スイッチの自作がますます注目を集めることになったのです。
...
この家庭用無線 LAN ルータはどのようにして OpenFlow 化されているのでしょう。その鍵は無線 LAN ルータのファームウェアにあります。最近の無線 LAN ルータは Linux ベースのファームウェアで動作しており、比較的オープンなハードウェア構成を持っています。そこで、そのファームウェアを Linux ベースのオープンなファームウェアと載せ替えることにより、メーカーの保証は受けられませんが様々なソフトウェアを動かすことができるようになります。@SRCHACK.ORG 氏の開発した OpenFlow 化ツールは、Linux ベースのファームウェアに OpenFlow ソフトウェアスイッチなどを組み込んだものです。
...
ファームウェアを公開している @SRCHACK.ORG 氏のサイト (@<tt>{http://openflow.inthebox.info/}) から、手持ちの無線 LAN ルータに対応するファームウェアをダウンロードしてください。いくつかのバージョンの OpenFlow プロトコルに対応したファームウェアが用意されていますが、動作確認のためには Trema が対応しておりコントローラ実装も豊富な OpenFlow 1.0 用ファームウェアが良いでしょう。
...
ここで、ファームウェアのインストール用に使った PC を LAN 側から抜き、WAN 側につなぎなおします。ファームウェアのアップデート直後には、WAN 側ポートに 192.168.1.1/24 というネットワークが設定されています@<fn>{config}。ファームウェアに設定されているコントローラの IP アドレスはデフォルトで 192.168.1.10 であるため、このアドレスを使用しましょう。この WAN 側ポートでは DHCP サーバは動作していないので、この IP アドレスを固定で設定してください。
...
滅多にあることではありませんが、ファームウェアのアップデートに失敗するといわゆる「文鎮」状態になってしまいます。幸い、配布元の @SRCHACK.ORG 氏のサイトに純正ファームウェアに戻す方法が記載されていますので、もし文鎮化した場合にはこちらを参考にしてみてください。
...
どのエントリが ping によって生成されたフローかを理解するために、ping が送受信するパケットの中身を説明しましょう。ping コマンドは、宛先に ICMP エコー要求と呼ばれるパケットを送るコマンドです。ICMP エコー要求を受け取ったホストは、送信元ホストに対して ICMP エコー応答パケットを送り返します。この ICMP のプロトコル番号は 1 なので、@<tt>{nw_proto=1} のフローが ping に対応します。
...
自作 OpenFlow 用ファームウェアはオープンに開発が行われています (@<tt>{http://openflow.inthebox.info/})。もし新しい不具合や改善方法をみつけた方はぜひフィードバックを送ってください。

== まとめ

この章では個人でも手に入る安価な無線 LAN ルータを OpenFlow スイッチに改造しました。また、実際に Trema とつなげて動作確認を行いました。

 * @SRCHACK.ORG 氏公開の OpenWRT ベースのファームウェアを使うと、一部の無線 LAN ルータを OpenFlow スイッチに改造することができます。
...
: @SRCHACK.ORG 氏のブログ (@<tt>{http://www.srchack.org/})
  今回紹介した自作 OpenFlow スイッチ界の中心人物 @SRCHACK.ORG 氏のブログです。OpenFlow 対応ファームウェアを無線 LAN ルータ上で動作させるまでの試行錯誤や、自宅でサーバーラックを運用する "自宅ラック友の会" の活動の様子などをブログ形式で紹介しています。
