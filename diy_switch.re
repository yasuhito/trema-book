...
あなたも仮想スイッチではそろそろ物足りなくなってきました。やはり実機のOpenFlow スイッチを自宅にデンと据えて、Trema で心ゆくまで OpenFlow プログラミングを楽しみたいものです。「でも、中古でも出てないんだよ」。高価な Catalyst や Summit などの中古品を自宅で愉しむ "ネットワークマニア" にかかっても、まだ実機は入手できていないようです。なぜなら OpenFlow スイッチは新しい分野の製品なので、中古市場には出回っていないのです。
...
@<table>{openflow_switch} は 2012 年現在の OpenFlow スイッチの価格比較です。これを見ると、安くなってきたとは言え最安でも 32 万円。とても個人では手は出ません。やはり、コモディティに降りてくるまであと数年は指をくわえて待つしかないのでしょうか。

そんな中、事件は 2011 年の年末に起きました。ある一人のエンジニアが、二千円で買える "家庭用" 無線 LAN ルータをハックし、OpenFlow スイッチに改造するファームウェアを公開したのです。しかも、秋葉原の居酒屋 "村役場"にて有志による「無線 LAN ルータを OpenFlow 化して、Trema と接続してみよう」という勉強会が開催されました。この夢の "Trema とつながる 二千円の OpenFlow スイッチ" は Twitter でも公開され、日本中にまたたく間に広まりました。

== 無線 LAN ルータを OpenFlow 化しよう

無線 LAN ルータには、LAN 側に有線のポートが 4 ポートついており、L2 スイッチとして動作しています。この部分を OpenFlow スイッチとして動作させるようにしてみましょう@<fn>{musen}。

いまどきの無線 LAN ルータの OS は、Linux で動作しています。Linux 上で動作する OpenFlow スイッチのソフトウェア実装がありますので、これが無線 LAN ルータ上でも動作させればよさそうです。しかし、無線 LAN ルータのメーカーは、自由にソフトウェアを動かす目的で Linux を採用しているわけではありません。

OpenWRT は、そんな制約を超えるために無線 LAN ルータのファームウェアを入れ替えてしまおうというプロジェクトです。このファームウェアを使えば、その上で自由にソフトウェアを動かすことができます。

@SRCHACK さんは、この OpenWRT をベースに OpenFlow スイッチのソフトウェア実装を組み込んだファームウェアを開発しました。対応機種は @<table>{wifirouter} のとおりです。本章では、このうち WHR-G301N を使って OpenFlow スイッチを作ってみます。
...
毎年、業界最大の展示会 Interop Tokyo が幕張メッセにて開催されています。2012 年の Interop Tokyo では、Trema が大活躍しました。Trema ベースで開発された情報通信研究機構 (NICT) の RISE Controller が、Best of Show Award (ShowNet プロダクト部門) のグランプリを受賞しました(@<img>{trema_interop})。
...
また Interop と併催で、次世代のインターネットを担う技術者の発掘を目指したオープンルータ・コンペティションが開催されました。「ソフトウェア、ハードウェアに関わらず OpenRouter を改良、または創造し、その成果を発表すること」という課題に対し、一次選考を通過した 10 チームによりその成果が競われました。

//image[orc][最終審査会場で動作する無線ルータベースの OpenFlow スイッチ][scale=0.12]

その結果、本章で取り上げた @SRCHACK さんが、見事準グランプリ (NEC 賞) を受賞しました (@<img>{orc})。また他にも 3 チームにて OpenFlow に関連した成果発表が行われ、これらのチームすべてが Trema を使用していました。各チームの発表内容は、以下のサイトで確認できます。

 * オープンルータ・コンペティション公式 ( @<tt>{http://www.interop.jp/2012/orc/} )
 * まとめサイト ( @<tt>{https://sites.google.com/site/orcmatome/} )
...
事前に @SRCHACK さんのサイトから、手持ちの機種に対応するファームウェアと、アップデート方法について記載された手順書をダウンロードしておいてください。異なるいくつかのバージョンの OpenFlow プロトコルに対応したファームウェアが用意されていますが、本書で扱っている OpenFlow 1.0 に対応したものを用いてください。

 * 無線 LAN ルータ OpenFlow 対応ファームウェア ( @<tt>{http://www.srchack.org/orc12/} )

まず、ファームウェアを入れたホストを、無線 LAN ルータと接続します。この時、@<img>{update} (A) のように無線 LAN ルータの LAN 側ポートに接続してください。LAN 側ポートは 4 ポートありますが、どのポートに接続してもよいです。ホストの IP アドレスは、無線 LAN ルータの DHCP サーバ機能により、自動的に割り当てられます。

//image[update][無線 LAN ルータとホストとの接続][scale=0.5]

次に、ファームウェアを OpenFlow 対応のものに書き換えます。ダウンロードしておいた手順書に従い、アップデートを行なってください。``ファームウェアのアップデート中は決して電源を切らないでください'' というお約束を守れば、簡単にアップデートできるはずです。

OpenFlow 対応ファームウェアをいれた無線 LAN ルータでは、各ポートの役割が変わってしまっていることに注意が必要です。LAN 側の 4 ポートは OpenFlow スイッチとして動作するポートとなります。また WAN 側ポートは、コントローラとの接続に用いる管理用のポートになります。WHR-G301N では Internet と記載されている青いポートが、WAN 側ポートです。コントローラとの接続以外にも @<tt>{telnet} での接続などにも用いられます。

ファームウェアのアップデート直後には、WAN 側ポートに 192.168.1.1/24 というアドレスが設定されています。そのため、@<img>{update} (B) のように、ホストを WAN 側ポートに繋ぎ直してください。この WAN 側ポートでは DHCP サーバ機能は動作していないので、ホストに固定で IP アドレスを設定してください。OpenFlow スイッチ側に事前に設定されているコントローラの IP アドレスが 192.168.1.10 であるため、このアドレスを使用しましょう。ホストにアドレスを設定した後、OpenFlow スイッチに@<tt>{telnet} を使って @<tt>{root} で接続してみます。
...
上記のように接続ができたでしょうか？接続ができたら、OpenFlow スイッチの設定ファイルを見てみましょう。設定ファイルは、@<tt>{/etc/config} 配下にあります。IP アドレス等、ネットワーク関連の設定は、@<tt>{network} ファイルに記載されています (@<list>{config_network})。WAN 側ポートは OpenFlow スイッチ上では @<tt>{eth1} に対応しているので、IP アドレスを変更したい場合には @<tt>{eth1} の @<tt>{ipaddr} の値を変更してください。変更後の値を反映させるためには、再起動が必要です。
...
OpenFlow 関連の設定は、@<tt>{openflow} ファイルに記載されています(@<list>{config_openflow})。OpenFlow スイッチのポートとして使用する LAN 側の 4 ポートはそれぞれ @<tt>{eth0.1, eth0.2, eth0.3, eth0.4} に対応しており、@<tt>{ofports} オプションの部分に指定されます。また OpenFlow コントローラの IP アドレス、ポート番号は @<tt>{ofctl} の値として指定されるので、変更したい場合はこのオプションの値を変更してください。変更した設定は、以下のコマンドで反映させることができます。
...
以下のコマンドでスイッチとコントローラ間に TCP コネクションが張られているかを確認しましょう。
...
TCP コネクションはスイッチ側からコントローラへと接続されます。もし、接続がされてないければ、OpenFlow スイッチに @<tt>{telnet} でログインして、@<tt>{openflow} 機能の再起動を行ってください。
 ...
 * 二千円の無線 LAN ルータを OpenFlow スイッチに改造し、ラーニングスイッチを使った動作確認を行いました。
 * @<tt>{show_description} コマンドを用意して、スイッチの情報を取得する方法について学びました。
 * @<tt>{flow_dumper} コマンドを使い、実際に設定されているフローの確認を行いました。

今回はラーニングスイッチを使いましたが、他のアプリケーションを用いることももちろん可能です。もし、今回の OpenFlow スイッチを二台以上用意できるなら、ルーティングスイッチ (@<chap>{routing_switch}) を使うことがお勧めです。
...
: OpenFlow 対応ファームウェア作者のSRCHACK 氏の Blog (@<tt>{http://www.srchack.org/})
  無線 LAN ルータで Linux が動くのであれば、OpenFlow も動くんじゃないか？今回紹介したプロジェクトを始めたとき、SRCHACK さんはそんな風に思ったに違いありません。
