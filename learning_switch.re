...
//lead{
データセンターのような複雑に入り組んだネットワークも、ケーブルを抜いてバラバラにしてみると、大量のスイッチやサーバというシンプルな部品からできていると分かります。いわばネットワークの "原子" とも言えるこのスイッチを OpenFlow で作ってみましょう。
//}

OpenFlow の世界では、コントローラとしてソフトウェア実装したスイッチをよくラーニングスイッチと呼びます。なぜ、ラーニング (学習) スイッチと呼ぶのでしょうか？それは、スイッチが次のように動くからです。

 1. 学習: 飛んできたパケットに含まれる、ホストのネットワーク上での位置情報を保存する
 2. 転送: 今まで学習してきた位置情報を使って、パケットを宛先のホストまで転送する

この「学習し、転送する」というラーニングスイッチの考えかたはとても応用が効きます。たとえば第 III 部で紹介するいくつかのデータセンターネットワークも、基本はラーニングスイッチと同じく「学習し、転送する」という動作をします。このように、自宅ネットワークであろうが最新鋭のデータセンターであろうが、その基本的な考えかたは同じなのです。
...
//noindent
ちょうど、和食の基本である@<ruby>{出汁,だし}をマスターすればおいしい吸い物や煮物などを作れるように、ラーニングスイッチをベースに機能を付け加えていけば、さまざまな OpenFlow アプリケーションの出来上がりです。
