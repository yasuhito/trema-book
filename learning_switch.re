...
//lead{
データセンターを動かす複雑怪奇なネットワークも、いったん解きほぐしてバラバラにしてみると、たくさんのスイッチとネットワークケーブルという単純な要素に分解できます。いわばネットワークの原子とも言えるこのスイッチ、どうすれば OpenFlow で作れるでしょうか？
//}
...
OpenFlow の学習には、ラーニングスイッチの実装は避けては通れません。ちょうど、和食の基本である@<ruby>{出汁,だし}がうまく取れればおいしい吸い物や煮物を作れるように、ラーニングスイッチが作れればそれをベースに複雑なネットワークを OpenFlow で実装できるからです。たとえば、@<chap>{router_part1}、@<chap>{router_part2}で紹介するルータはラーニングスイッチにパケット書き換え機能を加えたものです。また、第 III 部で紹介するいくつかのデータセンターネットワークも、ラーニングスイッチを改造してたくさんの OpenFlow スイッチに対応できるようにしたものです。

スイッチの基本機能はとても単純なので、OpenFlow で簡単に作れます。高価なハードウェアスイッチの高機能ぶりに惑わされてはいけません。基本は「学習し、転送する」これだけです。最低限の仕組みが分かってしまえばこれを OpenFlow で実装するのはワケもないことです。
...
スイッチのポート 1 番と 4 番に、ホスト 1 と 2 をそれぞれ接続しています。また、それぞれのホストのネットワークカードには図に示した MAC アドレスを割り振っているとします。

//indepimage[host1to2][パケットの送信元 MAC アドレスとスイッチのポート番号を FDB に学習する][scale=0.5]

まずホスト 1 がホスト 2 へパケットを送信すると、そのパケットはスイッチのポート 1 番に到着します。パケットが到着すると、スイッチはパケットの送信元 MAC アドレスとパケットの入ってきたポートの番号を調べ、「ポート 1 番には MAC アドレスが 00:11:11:11:11:11 のホストがつながっている」と学習します。この学習した「MAC アドレス + ポート番号」の組をためておくデータベースのことを、フォワーディングデータベース (FDB) と呼びます。この情報は、以降のパケットの転送に使います。

//indepimage[host1to2_flood][パケットの宛先 MAC アドレスからスイッチのポート番号が FDB にみつからないため、パケットをばらまく][scale=0.5]

学習が終わるとスイッチはパケットの宛先 MAC アドレスを調べ、送出先のポート番号を探そうとします。しかし、ホスト 2 の MAC アドレスとポート番号はまだ学習していないので分かりません。そこで、スイッチはパケットをポート 1 以外のすべてのポートにばらまき、最終的にポート 4 につながるホスト 2 にパケットが届きます。このように学習が不十分な段階では、無駄なトラフィックが発生してしまいます。

//indepimage[host2to1][同様にホスト 2 の MAC アドレスとポート番号を学習し、FDB の情報からパケットをホスト 1 に届ける][scale=0.5]

この状態で逆にホスト 2 がホスト 1 へパケットを送信すると、スイッチは「ポート 4 番には MAC アドレスが 00:22:22:22:22:22 のホストがつながっている」と学習します。また、パケットの宛先 MAC アドレスと今までの学習データを照らし合わせ、出力先のポート番号を探します。ここで、最初のステップで「ポート 1 = MAC アドレス 00:11:11:11:11:11」と学習しているので、出力ポートは 1 と決定できます。最初のステップとは違いパケットはポート 1 のみに転送されるので、無駄なトラフィックが発生しません。
...
このように、スイッチはホストの MAC アドレスとポート番号をパケット到着のタイミングで学習しつつ、これを使って賢くパケットを送り届けるわけです。ここまでの仕組みがわかれば、スイッチの機能を実現するコントローラ (ラーニングスイッチ) を Trema で実装することは簡単です。ではさっそく実装を見て行きましょう。
