...
//lead{
データセンターのような複雑に入り組んだネットワークも、ケーブルを抜いてバラバラにしてみると、大量のスイッチやサーバなどのシンプルな部品からできていると分かります。いわばネットワークの "原子" とも言えるこのスイッチを OpenFlow で作ってみましょう。
//}

OpenFlow の世界では、コントローラとしてソフトウェア実装したスイッチをよくラーニングスイッチと呼びます。なぜ、ラーニング (学習) スイッチと呼ぶのでしょうか？それは、スイッチが次のように動くからです。

 1. 学習: ホストから出たパケットに含まれる、ホストのネットワーク上での位置情報を学習する
