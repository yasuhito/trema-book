...
<tt>{vhost} は、仮想ホストの設定です。この設定ファイルでは @<tt>{host1, host2} という名前の二つの仮想ホストを起動します。また、@<tt>{link} は仮想リンクで、指定した仮想スイッチと仮想ホストを接続できます。ここでは、仮想スイッチ @<tt>{lsw} と、仮想ホスト @<tt>{host1, host2} をそれぞれ仮想リンクで接続しています。
