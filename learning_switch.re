...
スイッチはホストから届いたパケットを宛先のホストまで転送するために、スイッチにつながる各ホストの位置情報をためておくデータベースを持っています。これをフォワーディングデータベース (FDB) と呼び、「ホストの MAC アドレス + ポート番号」の組を保持します。

//image[switch_network][スイッチ 1 台とホスト 2 台からなるネットワークと、スイッチの FDB の内容][scale=0.5]

ここでホスト 2 がホスト 1 へパケットを送信すると、スイッチは次のようにパケットをホスト 1 まで転送します (@<img>{host2to1})。

//image[host2to1][FDB の情報からパケットをホスト 1 に届ける][scale=0.5]

 1. 届いたパケットの宛先 MAC アドレス (00:11:11:11:11:11) を見て、この MAC アドレスを持つホストがつながるポート番号を FDB から探します。
 2. FDB には「MAC アドレス 00:11:11:11:11:11 = ポート 1」と学習しているので、ポート 1 にパケットを出力します。

ここまでの仕組みがわかれば、スイッチの機能を実現するコントローラ (ラーニングスイッチ) を OpenFlow で実現するのは簡単です。
...
ラーニングスイッチのソースコード (@<list>{learning-switch.rb}) で新しく登場した構文はたった一つだけです。@<tt>{start} ハンドラの中で登場した FDB の本体 @<tt>{{\}} がそれですが、カンの良い読者ならこれはハッシュテーブルでは？と気付いているでしょう。
...
//noindent
などなどマッチングルールが定義する 12 個の条件 (詳しくは@<chap>{openflow_spec}を参照) がすべてまったく同じ、というのが Exact Match です。
