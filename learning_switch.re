...
学習が終わるとスイッチはパケットの宛先 MAC アドレスを調べ、送出先のポート番号を探します。しかし、ホスト 2 の MAC アドレスとポート番号はまだ学習していないので分かりません。そこで、スイッチはパケットをポート 1 以外のすべてのポートにばらまき、最終的にポート 4 につながるホスト 2 にパケットが届きます。このように学習が不十分な段階では、無駄なトラフィックが発生してしまいます。
...
このように、スイッチはホストの MAC アドレスとポート番号をパケット到着のタイミングで学習しつつ、これを使って賢くパケットを送り届けるわけです。ここまでの仕組みがわかれば、スイッチの機能を実現するコントローラ (ラーニングスイッチ) を OpenFlow で実装するのは簡単です。
