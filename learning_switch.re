...
ラーニングスイッチのソースコード (@<list>{learning-switch.rb}) で新しく登場した構文はたった一つだけです。@<tt>{start} ハンドラの中で登場した FDB の本体 @<tt>{{\}} がそれですが、カンの良い読者ならこいつが何者か、うすうす感付いているかもしれません。
...
//emlist{
animals = { "ア" => "アルマジロ", "イ" => "イノシシ, "ウ" => "ウシ" }
//}

たとえば「ウ」で始まる動物は何だろう？と辞書で調べたくなったら、次のようにして辞書を引きます。

//emlist{
animals[ "ウ" ] #=> "ウシ"
//}

この辞書を引くときに使う言葉 (この場合、"ウ") を@<em>{キー}と言います。そして、みつかった定義 (この場合、"ウシ") を@<em>{バリュー}と言います。キー & バリューは語呂も良い名コンビ、キーが現れるときはいつもバリューといっしょ、と覚えてください。
...
ハッシュテーブルは Ruby で使える高性能辞書なので、言葉 (文字列) だけでなく好きなオブジェクトを格納できます。たとえば、FDB では MAC アドレスをキーとして、ポート番号をバリューにします。

そうそう、言い忘れていましたが空のハッシュテーブルは @<tt>{{\}} です。FDB は最初は空のデータベースなので、@<tt>{{\}} として初期化されていました。
