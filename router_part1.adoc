...
ルータがパケットを受け取るためには、ホストはルータの MAC アドレスを知る必要があります。このためにホスト A は、パケットを送る前にルータの MAC アドレスを ARP リクエストで調べ、これを宛先 MAC アドレスとしてパケットに指定します。ルータは ARP リクエストを受け取ると、自身の MAC アドレスを ARP リプライとして返します (<<router_arp_reply,図 12-2>>)。
...
従来のネットワーク専用機器をソフトウェアで実装した、いわゆる NFV の例としてシンプルなルータの仕組みを学びました。
