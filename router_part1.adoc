...
`send_later` メソッドでは、ARP が未解決なパケットを宛先 IP アドレスごとにキュー (`queue`) に入れます。キューにはパケットのデータに加え、ルータでの書き換えに必要なアクションの一部も入ります。この時点では宛先 MAC アドレスはまだ分からないので、`SetEtherDestinationAddress` アクションは入っていません。キューへの追加後に ARP リクエストを送ることで宛先の MAC アドレスを解決します。

[source,ruby,subs="verbatim,attributes"]
.UnresolvedPacketQueue#send_later (lib/unresolved_packet_queue.rb)
----
include::{sourcedir}/lib/unresolved_packet_queue.rb[lines="7..15"]
----

キューにためたパケットを転送するのは ARP リプライが Packet In したタイミングです。`packet_in_arp_reply` の最後に呼び出している `maybe_flush_unsent_packets` がこの処理を行います。

[source,ruby,subs="verbatim,attributes"]
.UnresolvedPacketQueue#maybe_flush_unsent_packets (lib/unresolved_packet_queue.rb)
----
include::{sourcedir}/lib/unresolved_packet_queue.rb[lines="17..28"]
----

ここでは MAC アドレスが解決したパケットそれぞれに対して、宛先 MAC アドレスを書き換える `SetEtherDestinationaddress` アクションを追加したアクションリストを作り Packet Out しています。
