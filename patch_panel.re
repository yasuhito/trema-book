...
//lead{
実際のネットワーク管理に役立って、SDN の入門にもなるのがこの OpenFlow で作ったインテリジェントなパッチパネルです。その上ソースコードも簡単とくれば、試さない手はありません。
//}

== 便利なインテリジェント・パッチパネル

スイッチやサーバが増えてくると、ネットワークケーブルの配線は悲惨なまでにごちゃごちゃになります。からみあったケーブルによって見通しが悪くなるのはもちろん、気にせずさらに増築を続けてしまうと、最悪の場合は配線のやり直しとなります。よくある原因は、スイッチとスイッチ、またはスイッチとサーバ同士を直接つないでしまうという配線方法です。これでは、つなぐ物を増やせば増やすほどごちゃごちゃになるのは当然です。

これを解消するのがパッチパネルという装置です (@<img>{patch_panel})。パッチパネルの仕組みはシンプルで、ケーブルを挿すためのコネクタが並んでいるだけです。これによって配線をいったんパッチパネルで中継できるので、配線がすっきりし拡張性が向上します。また、うまく設計すると配線の変更をパッチパネルの前面だけで行えるようになるので、メンテナンス性も向上します。
...
実は、インテリジェント・パッチパネルは SDN の一種です。なぜならば、パッチパネルにつながっているホストの所属するネットワークを、ソフトウェア的に切り替えられるからです (→ @<chap>{openflow_usecases})。これは、物理ネットワークの上にそれぞれ独立したネットワークをいくつも作れるという意味で、最も単純なネットワーク仮想化に他なりません。

こうしたインテリジェント・パッチパネルは OpenFlow を使うと簡単に実装できます。パッチパネルでの中継のように、パケットを曲げて別の場所につなぐというのは、フローが得意とすることの一つだからです。この章では、さっそくこうしたコントローラを実装して使ってみましょう。
