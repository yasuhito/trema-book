...
//noindent
ここまで分かればしめたものです。あらかじめ仕組みが分かっているので、ソースコードを読むのは簡単ですね。
...
@<tt>{PatchPanel} コントローラを実行すると、この設定ファイルを読み込み仮想パッチを OpenFlow スイッチに反映します。今回のように実機の OpenFlow スイッチとコントローラを接続するのも、@<tt>{trema run} コマンドできます。実機を使う場合には、次のように @<tt>{-c} オプションなしでコントローラを実行します。これによって、起動したコントローラに実機の OpenFlow スイッチが接続します。
 ...
 * @<tt>{@patch} は設定ファイルから読み込んだパッチング情報を入れておくインスタンス変数です。たとえばポート 1 番と 10 番、および 2 番と 11 番をパッチングする場合、この中身は @<tt>{[[1,10],[2,11]]} となります。このように、パッチでつなぐポートのペアを要素に持つ配列からなる配列となります。
 * @<tt>{File.open("ファイル名")} はファイルを読み込むメソッドで、続く @<tt>{each_line} は読み込んだファイルの一行一行に対してループ処理するためのイテレータです。つまり、ここでは設定ファイルを一行ずつループで処理しています。一時変数 @<tt>{each} には、それぞれの行の内容が文字列として入ります。
...
このようにフローエントリを追加するには、フローエントリに必要な要素であるマッチングルールとアクションを指定します。それぞれの詳しい API を見て行きましょう。
...
これらアクションの利用方法も、続くいくつかの章で具体的な使い方を見て行きます。API の詳しい内容については Trema Ruby API を参照してください。
...
このように Flow Mod にはたくさんのパラメータがありますので、必要になったらこの章まで立ち戻ってください。
