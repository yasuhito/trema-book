...
Ruby の正規表現は Perl や他の言語の正規表現とほとんど変わりません。スラッシュ (@<tt>{/}) ではさまれた文字列が正規表現で、その中に正規表現のメタ文字を書きます。マッチさせるには @<tt>{=~} に続いて文字列を渡します。マッチしなかった場合には @<tt>{nil} が返るので、次のように @<tt>{if} の条件に指定することでマッチしたかどうかを判定できます。
