...
サーバやスイッチが増えてくると配線が悲惨なまでにごちゃごちゃになります。何がどこにつながっているのかわからなくなりますし、さらにラックが増えたりすれば最悪の場合、配線のやりなおしになります。ネットワーク構成が最初から決まっていることはほとんどないので、スイッチとスイッチ、またはスイッチとサーバ同士を直接つなげていく配線方法をとっている限り、そのうちごちゃごちゃになるのは当たり前です。

これを解消するのがパッチパネルという装置です。パッチパネルの仕組みはシンプルで、ケーブルのインプットとアウトプットを中継するためのポートが並んでいるだけです。パッチパネルを使うと配線をいったん中継できるので、配線がきれいに整理できます。また、うまく設計すると配線の変更をパッチパネルだけで行えるようになるので、メンテナンス性も向上します。

パッチパネルをソフトウェアで実現したのがソフトウェアパッチパネルです。もしパッチパネルでメンテナンス性が向上できたとしても、パッチパネルを変更するためにサーバ室まで足を運ぶのは面倒です。そこで、パッチパネルの配線をネットワーク経由でソフトウェア的に変更できるようにすることで、さらにメンテナンス性が上がります。
