...
ルータは、その判断のために、自身のインタフェースに割り当てられた IP アドレスを使います。そのため、ルータのインタフェースには、ネットワーク中のホストとネットワークアドレスが同じになるように、IP アドレスを割り当てる必要があります。@<img>{router_address} で、ルータ B のインタフェースには、ホスト X, Y, Z と同じネットワークアドレスになるよう、例えばアドレス 192.168.1.254 を割り当てます。

//image[router_address][ルータのインタフェースには、ネットワーク内のホストとネットワークアドレスが同じとなるよう、IP アドレスを割り当てる]

@<img>{router_address} のルータ B が、ホスト X (192.168.1.1) 宛のパケットを受け取った場合について考えます。ルータ B は、パケットの宛先アドレスを参照し、ネットワークアドレスが同じインタフェースを探します。この例では、192.168.1.254 というアドレスがついたインタフェースが見つかります。あとは、このインタフェースを通じて、ARP リクエストによる MAC アドレス問い合わせを行い、ホスト X 宛にパケットを出力します。
...
@<img>{default_route} のように、インターネットに接続するネットワークでのルーティングテーブルについて考えてみましょう。先ほどインターネット上のホスト数が膨大だという話をしましたが、宛先ホストをネットワーク単位にまとめたとしても、インターネットでは数十万のルーティング情報を扱う必要があります。しかし、この図のように、インターネットへの出口が一か所だけの場合、デフォルトルートを使えば、これらを一つのエントリにまとめられます。このことにより、ルーティングテーブル中のエントリ数を大きく減らせます。

また、@<img>{default_route} 中のルータ C につながるネットワーク (172.16.3.0/24) のような例外があっても、問題ありません。もし 172.16.3.0/24 宛のパケットがルータ A に届いた場合、ロンゲストマッチによりルータ C へのエントリが選択されます。それ以外のパケットは、デフォルトルートによってルータ B へ転送され、インターネットへと送られます。

== ソースコード

=== パケットを書き換えて転送する(再)

パケットを書き換えて、転送を行う @<tt>{forward} メソッドを、もう一度見ていきましょう。@<chap>{router_part1} でこのメソッドが行う 5 つの処理について説明しましたが、そのうち、次転送先の決定と、出力インタフェースの決定について見ていきます。
...
この判定は、@<tt>{Interfaces} クラスの @<tt>{find_by_prefix} メソッドで行います。このメソッドは、ルータが次の転送先と同一のネットワークアドレスを持つインタフェースを持っているかを調べます。もし、該当するインタフェースがない場合、ルータはそのパケットを転送できないので、パケットを破棄して転送処理を終えます。
