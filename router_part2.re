...
@<img>{default_route} のように、インターネットにつながっている場合のルータ A の経路表について考えてみましょう。
...
このメソッドでは、まず経路表 (@<tt>{@routing_table}) を @<tt>{lookup} メソッドで検索します。このとき経路表中で見つかった次転送先を、戻り値として返します。もし、見つからなかった場合は、宛先アドレス (@<tt>{daddr}) をそのまま返します。これは、@<img>{router_address} で説明したように、宛先ホストとルータが直接接続している可能性があるからです。
