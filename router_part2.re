...
@<img>{router_address} のルータ B が、ホスト X (192.168.1.1) 宛のパケットを受け取った場合について考えます。ルータ B は、パケットの宛先アドレスを参照し、ネットワークアドレスが同じインターフェイスを探します。この例では、192.168.1.254 というアドレスがついたインターフェイスが見つかります。あとは、このインターフェイスを通じて、ARP リクエストによる MAC アドレス問い合わせを行い、ホスト X 宛にパケットを出力します。
...
パケットの宛先 IP アドレスに複数のエントリが該当する場合はどうなるでしょうか？@<img>{longest_match} のルータ A がホスト X (172.16.3.1) にパケットを送る場合について考えてみましょう。ルータ A が持つルーティングテーブルは、ルータ B につながる 3 つのネットワーク宛のエントリはまとめることで、図のように 2 つのエントリnにすることができます。しかし、このようにまとめてしまうと、宛先 172.16.3.1 のパケットは、どちらのエントリにもマッチしてしまいます。ルータは、どちらか正しいエントリを選択しなければいけません。
...
@<img>{default_route} のように、インターネットに接続するネットワークでのルーティングテーブルについて考えてみましょう。先ほどインターネット上のホスト数が膨大だという話をしましたが、宛先ホストをネットワーク単位にまとめたとしても、インターネットでは数十万のルーティング情報を扱う必要があります。しかし、この図のように、インターネットへの出口が一か所だけの場合、デフォルトルートを使えば、これらを一つのエントリにまとめることができます。このことにより、ルーティングテーブル中のエントリ数を大きく減らせます。
...
その後 @<tt>{forward} メソッドへ戻り、決定した次の転送先がルータのインターフェイスに接続しているかを判定します。
...
この判定は、@<tt>{Interfaces} クラスの @<tt>{find_by_prefix} メソッドで行います。このメソッドは、ルータが次の転送先と同一のネットワークアドレスを持つインターフェイスを持っているかを調べます。もし、該当するインターフェイスがない場合、ルータはそのパケットを転送できないので、パケットを破棄して転送処理を終えます。
