...
@<img>{router_address} のルータ B が、ホスト X (192.168.1.1) 宛のパケットを受け取った場合について考えます。ルータ B は、パケットの宛先アドレスを参照し、ネットワークアドレスが同じインターフェイスを探します。この例では、192.168.1.254 というアドレスがついたインターフェイスが見つかります。あとは、@<chap>{router_part1} で説明した ARP による MAC アドレスの問い合わせを、このインターフェイスに対して行います。
