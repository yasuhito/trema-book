...
いよいよシンプルルータを動かしてみましょう。いろいろなパケットの送受信を行うために、今回は仮想ホストではなく実際のホストを使用します。シンプルルータとして動作させるホストも含めて、合計で3台のホストを用意し、@<img>{network}のように接続してください。
...
シンプルルータ用のホスト上では、仮想ネットワーク機能を使って仮想スイッチを一台起動します。次の設定ファイルを@<tt>{simple-router.conf}として用意してください。
 ...
 * こうしてまとめられたエントリは、ネットワークアドレスが同じ他のエントリとまとめることで、さらにエントリ数を減らせます。
 * パケットの宛先にマッチするエントリがルーティングテーブルに複数ある場合は、ロンゲストマッチ、つまりネットマスク長が最も長いエントリを優先します。
