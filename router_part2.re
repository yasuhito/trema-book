...
@<img>{router_network} では、ルータが宛先ホストに直接接続していない場合について説明しましたが、つながっているかいないかの判断はどのように行なっているのでしょうか？

ルータは、その判断のために、自身のインタフェースに割り当てられた IP アドレスを使います。インタフェースに割り当てる IP アドレスには、ネットワーク中のホストとネットワークアドレスが同じ IP アドレスを用います。@<img>{router_address} で、ルータ B のインタフェースには、ホスト X, Y, Z と同じネットワークアドレスになるよう、例えばアドレス 192.168.1.254 を割り当てます。

//image[router_address][ルータのインタフェースには、ネットワーク内のホストとネットワークアドレスが同じとなるよう、IP アドレスを割り当てる]

ここで @<img>{router_address} のルータ B が、ホスト X (192.168.1.1) 宛のパケットを受け取った場合について考えます。ルータ B は、パケットの宛先アドレスを参照し、ネットワークアドレスが同じインタフェースを探します。この例では、192.168.1.254 というアドレスがついたインタフェースが見つかります。あとは、このインタフェースを通じて、ARP リクエストによる MAC アドレス問い合わせを行い、ホスト X 宛にパケットを出力します。
...
パケットの宛先 IP アドレスに複数のエントリが該当する場合はどうなるでしょうか？@<img>{longest_match} のルータ A がホスト X (172.16.3.1) にパケットを送る場合について考えてみましょう。ルータ A が持つルーティングテーブルは、ルータ B につながる 3 つのネットワーク宛のエントリはまとめることで、図のように 2 つのエントリにできます。しかし、このようにまとめてしまうと、宛先 172.16.3.1 のパケットは、どちらのエントリにもマッチしてしまいます。ルータは、どちらか正しいエントリを選択しなければいけません。
...
@<img>{default_route} のように、インターネットに接続するネットワークでのルーティングテーブルについて考えてみましょう。先ほどインターネット上のホスト数が膨大だという話をしました。しかし、宛先ホストをネットワーク単位にまとめたとしても、インターネットでは数十万のルーティング情報を扱う必要があります。この図のようにインターネットへの出口が一か所だけの場合、デフォルトルートを使えば、これらを一つのエントリにまとめられます。このことにより、ルーティングテーブル中のエントリ数を大きく減らせます。

@<img>{default_route} 中のように、インターネットとは別にネットワーク (172.16.3.0/24) があっても、デフォルトルートを使うことに問題はありません。172.16.3.0/24 宛のパケットがルータ A に届いた場合、ネットマスク長が長い方の、ルータ C へのエントリが選択されます。それ以外のパケットは、デフォルトルートによってルータ B へ転送され、インターネットへと送られます。

== ソースコード

=== パケットを書き換えて転送する(再)

パケットを書き換えて、転送を行う @<tt>{forward} メソッドを、もう一度見ていきましょう。@<chap>{router_part1} でこのメソッドが行う 5 つの処理について説明しましたが、そのうち、次の転送先の決定と、出力インタフェースの決定について見ていきます。
...
このメソッドでは、まずルーティングテーブル (@<tt>{@routing_table}) を @<tt>{lookup} メソッドで検索します。このときルーティングテーブル中で見つかった次の転送先を返します。もし、見つからなかった場合は、宛先アドレス (@<tt>{daddr}) をそのまま返します。これは、@<img>{router_address} で説明したように、宛先ホストとルータが直接接続している可能性があるからです。

その後 @<tt>{forward} メソッドへ戻り、決定した次の転送先がルータのインタフェースに接続しているかを判定します。
...
この判定は、@<tt>{Interfaces} クラスの @<tt>{find_by_prefix} メソッドで行います。このメソッドは、次の転送先と同一のネットワークアドレスであるインタフェースを、ルータが持っているかを調べます。もし、該当するインタフェースがない場合、ルータはそのパケットを転送できないので、パケットを破棄して転送処理を終えます。
...
ルーティングテーブル作成時に呼ばれる @<tt>{initalize} メソッドでは、@<tt>{ADDR_LEN + 1} (= 33) 個のハッシュからなる配列 (@<tt>{@db}) を用意します。ロンゲストマッチを実現するためには、ネットマスク長ごとに分けて経路を管理しなければなりません。そのため、このような実装になっています。
...
ルータが動作するためには、インタフェースへのアドレス設定とルーティングテーブルを設定する必要があります。シンプルルータでは、これらの設定を @<tt>{simple_router.conf} に記述します(@<list>{simple_router.conf})。
...
インタフェースの設定では、そのインタフェースの MAC アドレス (@<tt>{:hwaddr})、IP アドレス (@<tt>{:ipaddr})、ネットマスク長 (@<tt>{:masklen}) と、このインタフェースが OpenFlow スイッチのどのポート (@<tt>{:port}) に対応しているかを指定します。
...
はじめの何回かは@<tt>{ping}の応答が返って来ず、その後返ってきます。実際のルータでは、ARPが未解決のパケットを受信した時に、そのパケットをキューにためておき、ARPが解決した後転送を行います。しかし、シンプルルータではこの仕組みを実装していないため、通信の始めではこのように応答が帰ってこないパケットが発生します。ただし、いったんARPが解決されてしまえば、後の通信は通常通りに進みます。
