...
@<img>{default_route} のように、インターネットに接続するネットワークでのルーティングテーブルについて考えてみましょう。インターネット上のホスト数は膨大なので、宛先ホストをネットワーク単位にまとめたとしても、インターネット用に数十万のルーティング情報を扱う必要があります。でも、この図のようにインターネットへの出口が一か所だけの場合、デフォルトルートを使えば、インターネット用のエントリを一つにまとめられます。これによって、ルーティングテーブル中のエントリ数を大きく減らせます。
...
パケットの書き換えと転送を行う @<tt>{forward} メソッドをもう一度見ていきましょう。@<chap>{router_part1} で説明したこのメソッドが行う 5 つの処理のうち、次の転送先と出力インタフェースを決める方法を見ていきます。
