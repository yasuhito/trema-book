...
//image[router_network][同じイーサネット上にあるホストを一つの宛先としてまとめる]
...
この時、ホストX(192.168.1.1)宛のパケットを受け取ったルータAは次のように動作します。ルーティングテーブルのエントリ192.168.1.0/24と、宛先の192.168.1.1の上位24ビットを比較すると一致します。そこで、ルーティングテーブルから次の転送先はルータBだとわかります。ホストY,Z宛も同様に処理できるので、このエントリ1つでホスト3台分の宛先をカバーできます。
...
@<img>{default_route}のように、インターネットに接続するネットワークでのルーティングテーブルについて考えてみましょう。インターネット上のホスト数は膨大なので、宛先ホストをネットワーク単位にまとめたとしても、インターネット用に数十万のルーティング情報を扱う必要があります。しかし、@<img>{default_route}のようにインターネットへの出口が1か所だけの場合、デフォルトルートを使えば、インターネット用のエントリを1つにまとめられます。これによって、ルーティングテーブル中のエントリ数を大きく減らせます。
...
ルータが動作するためには、インターフェースへのアドレス設定とルーティングテーブルを設定する必要があります。シンプルルータでは、これらの設定を @<tt>{simple_router.conf} に記述します(@<list>{simple_router.conf})。
