...
このメソッドでは、まずルーティングテーブル (@<tt>{@routing_table}) を @<tt>{lookup} メソッドで検索し、見つかった次の転送先を返します。もし、次の転送先が見つからなかった場合は、宛先アドレス (@<tt>{daddr}) をそのまま返します。これは、@<img>{router_address} で説明したように、宛先ホストとルータが直接接続している可能性があるからです。
...
この判定は、@<tt>{Interfaces} クラスの @<tt>{find_by_prefix} メソッドで行います。このメソッドでは、次の転送先と同一のネットワークアドレスであるインターフェースを、ルータが持っているかを調べます。もし、該当するインターフェースがない場合、ルータはそのパケットを転送できないので、パケットを破棄して転送処理を終えます。

=== ルーティングテーブルの実装

次にルーティングテーブル(@<tt>{RoutingTable})のソースコード(@<list>{routing-table.rb})を見ていきます。
